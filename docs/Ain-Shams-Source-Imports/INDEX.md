...
Readable source index: [corpus-source-index.json](evidence/corpus-source-index.json), 1,362 readable source IDs.
...
| ASU_Y1 | `ASU-LOCO` | Skeletal muscle physiology, collagen/vitamin-D/purine/creatine biochemistry, arthropod/helminth parasitology (2026-08-27/28 passes) | Native-text Physiology MCQ paper (physiology, **now fully authored**) + scanned Biochemistry and Parasitology-Practical MCQ papers (image scans, no text layer — see note) | pending-live 20 physiology + 5 biochemistry concept overlays + 12 article overlays; concept 9 new (msk/fnd) + 12 new (inf); article 5 new (msk/fnd) + 2 new (inf); resource 3; question **90/90 physiology MCQs (all)** across 7 files (37+13+13+13+14) | `medical:batch` 0 errors on every file; `medical:simulate` confirms every pending-live overlay as `created:0` (genuine update) and, run across all 7 physiology question files together, `created: 90, rejected: 0, errors: []` (no cross-file id collisions); `medical:audit` 0 errors on these ids beyond documented evidence-stage and cross-directory gaps; `medical:duplicate-keys` 0 canonical-key collisions tree-wide. **Physiology MCQ authoring is complete.** OWED: evidence layer (claims/citations/spans) for all physiology records above. **NEEDS OMAR / needs-OCR**: all 88 biochemistry + 25 parasitology MCQs — `pdftotext` and `pdffonts` both confirm these two source PDFs have **zero extractable text layer** (pure CamScanner image scans, not OCR'd at all, despite the manifest's own `textLayer` field and this module's original triage note describing them as "OCR'd clean" — that referred to human-eye legibility of the rendered page images, not a machine text layer). Authoring from them safely requires the same visual, page-by-page render-and-read-by-eye pass Alexandria's AU-MED-106 lane used for its own unreadable-scan bank, not a quick re-run of `pdftotext`. See `coverage/ASU-LOCO-triage.md` and the CLAIMS.md Done row for full detail. |
...
| ASU_Y2 | `ASU-CNS-2` | Cerebellar tumours (medulloblastoma, hemangioblastoma) — this module's first authored question content | Native-text Pathology MCQ bank, printed "Answer: X" keys, not the embedded-answer format used by this university's CNS-3 papers | resource 1 catalogue row; concept 11; article 11; question **11/13 (2 within-file duplicates held)** | `medical:batch` 0 errors on question/concept/article/resource files; `medical:simulate` across all 4 files together: `created: 34, rejected: 0, errors: []`; `medical:audit` 0 errors. Claims are `needs_evidence`; citations are non-counting local curriculum support. Same-source-PDF duplication check against ASU-CNS-3's manifest run first (see `coverage/ASU-CNS-2-triage.md` "Cross-module note") after the first candidate paper turned out to already be authored under CNS-3. **NEEDS-OMAR**: the source paper's own CNS-infection block (items 14-58, ~44 items) is un-triaged next frontier; item 38 there ("NOT a common CNS complication of HIV... Answer: C [Toxoplasmosis]") looks medically wrong and needs source verification before keying. |
...
| [`question/`](question/) | MCQ import batches | — |
| [`pending-live/`](pending-live/) | Sparse overlay updates onto ids that exist only in another lane's unimported batch — never import until the named target file is live | [pending-live/INDEX.md](pending-live/INDEX.md) |
