# Item

## id
SPN-IMM-INNATE-BASICS-01

## article_id
ART-ASU-IMM-INNATE-BASICS

## section_id
art-asu-imm-innate-basics-mechanism

## text
PRR-PAMP recognition, ingestion into a phagosome, and intracellular killing, chiefly through the oxidative burst

## claim_ids
CLM-IMM-PHAGOCYTOSIS-STEPS-01
CLM-IMM-PRR-PAMP-01

## citation_ids
CIT-IMM-PHAGOCYTOSIS-STEPS-01
CIT-IMM-PRR-PAMP-01

---

# Item

## id
SPN-IMM-INFLAMMATION-CYTOKINES-01

## article_id
ART-ASU-IMM-INFLAMMATION-CYTOKINES

## section_id
art-asu-imm-inflammation-cytokines-definition

## text
Inflammation is the sequence of vascular and cellular reactions triggered when tissue damage and the entry of pathogenic organisms activate resident and recruited defence mechanisms at the affected site.

## claim_ids
CLM-IMM-PROINFLAMMATORY-CYTOKINE-01
CLM-IMM-MACROPHAGE-ACTIVATION-01

## citation_ids
CIT-IMM-PROINFLAMMATORY-CYTOKINE-01
CIT-IMM-MACROPHAGE-ACTIVATION-01

---

# Item

## id
SPN-IMM-CELLS-INNATE-01

## article_id
ART-ASU-IMM-CELLS-OF-INNATE-IMMUNITY

## section_id
art-asu-imm-cells-of-innate-immunity-mechanism

## text
the immediate mechanism is mast-cell degranulation releasing histamine, the defining feature of immediate (type I) hypersensitivity

## claim_ids
CLM-IMM-EOSINOPHIL-MARKER-01
CLM-IMM-TYPE-I-HYPERSENSITIVITY-01

## citation_ids
CIT-IMM-EOSINOPHIL-MARKER-01
CIT-IMM-TYPE-I-HYPERSENSITIVITY-01

---

# Item

## id
SPN-IMM-ANTIGENS-MHC-01

## article_id
ART-ASU-IMM-ANTIGENS-AND-MHC

## section_id
art-asu-imm-antigens-and-mhc-mechanism

## text
are encoded by the highly polymorphic HLA-A, HLA-B and HLA-C loci on the short arm of chromosome 6

## claim_ids
CLM-IMM-EPITOPE-DEFINITION-01
CLM-IMM-MHC1-GENETICS-01

## citation_ids
CIT-IMM-EPITOPE-DEFINITION-01
CIT-IMM-MHC1-GENETICS-01

---

# Item

## id
SPN-IMM-TCELL-BIOLOGY-01

## article_id
ART-ASU-IMM-T-CELL-BIOLOGY

## section_id
art-asu-imm-t-cell-biology-mechanism

## text
Full T-cell activation needs a second, costimulatory signal delivered by B7 on the antigen-presenting cell engaging CD28 on the T cell

## claim_ids
CLM-IMM-TCELL-MHC-RECOGNITION-01
CLM-IMM-COSTIMULATION-B7-CD28-01
CLM-IMM-CTLA4-INHIBITORY-01

## citation_ids
CIT-IMM-TCELL-MHC-RECOGNITION-01
CIT-IMM-COSTIMULATION-B7-CD28-01
CIT-IMM-CTLA4-INHIBITORY-01



---

# Item

## id
SPN-IMM-HUMORAL-IMMUNITY-01

## article_id
ART-ASU-IMM-HUMORAL-IMMUNITY

## section_id
art-asu-imm-humoral-immunity-key-determinants

## text
An antibody's antigen-binding site is formed by the variable region of both the heavy and the light chain together, and the hypervariable region within that variable domain confers the antibody's specificity

## claim_ids
CLM-IMM-HUMORAL-EXTRACELLULAR-ROLE-01
CLM-IMM-TI-ANTIGEN-PLASMACELLS-01
CLM-IMM-ANTIBODY-VARIABLE-REGION-01
CLM-IMM-CLASSSWITCH-IDIOTYPE-01
CLM-IMM-HYBRIDOMA-FUSION-01
CLM-IMM-IGM-COMPLEMENT-ROLE-01
CLM-IMM-CD40-CD40L-CLASSSWITCH-01
CLM-IMM-ISOTYPE-FUNCTION-PAIRING-01
CLM-IMM-C1Q-BINDING-ISOTYPES-01
CLM-IMM-IGALPHA-IGBETA-SIGNALING-01
CLM-IMM-BCELL-MATURATION-01
CLM-IMM-BCR-VS-TCR-CHO-01
CLM-IMM-THYMUS-DEPENDENT-FEATURES-01
CLM-IMM-NEONATAL-PASSIVE-ANTIBODY-01

## citation_ids
CIT-IMM-HUMORAL-EXTRACELLULAR-ROLE-01
CIT-IMM-TI-ANTIGEN-PLASMACELLS-01
CIT-IMM-ANTIBODY-VARIABLE-REGION-01
CIT-IMM-CLASSSWITCH-IDIOTYPE-01
CIT-IMM-HYBRIDOMA-FUSION-01
CIT-IMM-IGM-COMPLEMENT-ROLE-01
CIT-IMM-CD40-CD40L-CLASSSWITCH-01
CIT-IMM-ISOTYPE-FUNCTION-PAIRING-01
CIT-IMM-C1Q-BINDING-ISOTYPES-01
CIT-IMM-IGALPHA-IGBETA-SIGNALING-01
CIT-IMM-BCELL-MATURATION-01
CIT-IMM-BCR-VS-TCR-CHO-01
CIT-IMM-THYMUS-DEPENDENT-FEATURES-01
CIT-IMM-NEONATAL-PASSIVE-ANTIBODY-01

