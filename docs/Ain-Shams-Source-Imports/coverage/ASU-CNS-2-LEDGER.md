| cluster | authored | held | remaining | total |
|---|---:|---:|---:|---:|
| cerebellar-tumors-mcq | 11 | 2 | 0 | 13 |
| infections-mcq | 39 | 6 | 0 | 45 |

## Held
- cerebellar-tumors-q12 — within-file duplicate of this batch's own item 1 — item 12 ("A pathologist identifies a cerebellar tumor with 'Homer Wright rosettes.' This histologic feature is characteristic of which tumor? Answer: B [Medulloblastoma (classical)]") restates the identical Homer Wright rosettes → medulloblastoma fact already captured by cerebellar-tumors-q01
- cerebellar-tumors-q13 — within-file duplicate of this batch's own item 6 — item 13 ("A frontal section shows a well-circumscribed reddish mass in the vermis bulging into the fourth ventricle. Answer: B [Hemangioblastoma]") restates the identical gross-appearance fact already captured by cerebellar-tumors-q06
- infections-q38-hold — printed key (C, Toxoplasmosis) contradicts standard knowledge — toxoplasmosis IS a common CNS complication of AIDS; the stem's own phrase "directly mentioned in the text" suggests it is scoped to an unprovided source textbook. Needs Omar/source verification before keying; not keyed editorially per this lane's triage note.
- infections-q43-hold — within-file duplicate of item 25 (infections-q13) — restates the identical Cowdry type A intranuclear inclusion body -> HSV encephalitis fact already captured.
- infections-q50-hold — within-file duplicate of item 20 (infections-q07) — restates the identical well-circumscribed/firm/avascular mass -> tuberculoma fact already captured.
- infections-q52-hold — within-file duplicate of item 39 (infections-q26) — restates the identical viral meningitis CSF glucose (normal/not markedly decreased) fact already captured, framed as a negative-exception question.
- infections-q54-hold — within-file duplicate of item 31 (infections-q19) — restates the identical tuberculous meningitis CSF protein elevation fact already captured.
- infections-q55-hold — within-file duplicate of item 28 (infections-q16) — restates the identical perivascular mononuclear infiltrates -> viral encephalitis fact already captured.

## Remaining
(none)
