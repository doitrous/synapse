# Item

## id
CLM-FND-AMIODARONE-01

## concept_id
CON-FND-C96DA44323A6B8

## subject
Amiodarone: mechanism and adverse effects

## predicate
is

## object
Amiodarone is a class III anti-arrhythmic that prolongs the effective refractory period and action potential duration by blocking potassium channels, and also blocks calcium channels and alpha- and beta-adrenergic receptors.

## display_text
Amiodarone is a class III anti-arrhythmic that prolongs the effective refractory period and action potential duration by blocking potassium channels, and also blocks calcium channels and alpha- and beta-adrenergic receptors.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-ATHEROSCLEROSIS-RISK-FACTORS-01

## concept_id
CON-CVS-F44EBF69D14F3B

## subject
Risk factors for atherosclerosis

## predicate
is

## object
Atherosclerosis is predisposed to by hyperlipidaemia (raised LDL, lowered HDL), hypertension, smoking, diabetes mellitus, advancing age, male sex, obesity, stress, family history and lack of exercise.

## display_text
Atherosclerosis is predisposed to by hyperlipidaemia (raised LDL, lowered HDL), hypertension, smoking, diabetes mellitus, advancing age, male sex, obesity, stress, family history and lack of exercise.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-MALIGNANT-HYPERTENSION-01

## concept_id
CON-CVS-8BFFDE83CC4617

## subject
Malignant hypertension

## predicate
is

## object
In malignant hypertension, death occurs, in order of frequency, from renal failure, heart failure, coronary heart disease and cerebral haemorrhage.

## display_text
In malignant hypertension, death occurs, in order of frequency, from renal failure, heart failure, coronary heart disease and cerebral haemorrhage.

## risk_class
clinical_non_treatment

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-MSK-SMOOTH-MUSCLE-HISTOLOGY-01

## concept_id
CON-MSK-FA3FAEDF08014F

## subject
Smooth muscle: histological features

## predicate
is

## object
Smooth muscle fibres are spindle-shaped, non-striated, non-branched cells with a single centrally placed nucleus and no T-tubule system.

## display_text
Smooth muscle fibres are spindle-shaped, non-striated, non-branched cells with a single centrally placed nucleus and no T-tubule system.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-FIBRINOUS-PERICARDITIS-01

## concept_id
CON-CVS-D7459E7855802A

## subject
Fibrinous pericarditis

## predicate
is

## object
Fibrinous pericarditis is the commonest type of pericarditis, producing a bread-and-butter gross appearance, and follows rheumatic fever, lobar pneumonia, tuberculosis, uraemia or myocardial infarction.

## display_text
Fibrinous pericarditis is the commonest type of pericarditis, producing a bread-and-butter gross appearance, and follows rheumatic fever, lobar pneumonia, tuberculosis, uraemia or myocardial infarction.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-FATTY-STREAK-01

## concept_id
CON-CVS-E87E4CDC5DE594

## subject
The fatty streak

## predicate
is

## object
The fatty streak, the earliest grossly visible lesion of atherosclerosis, is a flat yellow intimal lesion composed microscopically of foam cells, occurring characteristically at the aortic valve and thoracic aorta.

## display_text
The fatty streak, the earliest grossly visible lesion of atherosclerosis, is a flat yellow intimal lesion composed microscopically of foam cells, occurring characteristically at the aortic valve and thoracic aorta.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-LONG-TERM-BP-REGULATION-01

## concept_id
CON-CVS-A531FD56A171D7

## subject
Long-term regulation of arterial blood pressure

## predicate
is

## object
Long-term arterial blood pressure is regulated mainly by renal and endocrine control of ECF volume, through the renin-angiotensin system, atrial natriuretic peptide and vasopressin.

## display_text
Long-term arterial blood pressure is regulated mainly by renal and endocrine control of ECF volume, through the renin-angiotensin system, atrial natriuretic peptide and vasopressin.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-VARICOSE-VEINS-01

## concept_id
CON-CVS-F048262BCB7EED

## subject
Varicose veins

## predicate
is

## object
A varicose vein is a superficial vein permanently dilated and tortuous, most often in the leg, from sustained raised venous pressure combined with valve incompetence.

## display_text
A varicose vein is a superficial vein permanently dilated and tortuous, most often in the leg, from sustained raised venous pressure combined with valve incompetence.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-DVT-THROMBOPHLEBITIS-01

## concept_id
CON-CVS-92BB03F3D57E33

## subject
Deep vein thrombosis and thrombophlebitis

## predicate
is

## object
Deep vein thrombosis arises from Virchow's triad — venous stasis (immobility, bed rest, varicose veins), hypercoagulability (pregnancy, malignancy, oral contraceptives) and endothelial injury (trauma, surgery).

## display_text
Deep vein thrombosis arises from Virchow's triad — venous stasis (immobility, bed rest, varicose veins), hypercoagulability (pregnancy, malignancy, oral contraceptives) and endothelial injury (trauma, surgery).

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-DVT-THROMBOPHLEBITIS-02

## concept_id
CON-CVS-92BB03F3D57E33

## subject
Deep vein thrombosis and thrombophlebitis

## predicate
is

## object
Thrombophlebitis is a deep vein thrombosis whose thrombosed segment is also inflamed, presenting with leg pain, swelling, warmth and redness over a palpable venous cord.

## display_text
Thrombophlebitis is a deep vein thrombosis whose thrombosed segment is also inflamed, presenting with leg pain, swelling, warmth and redness over a palpable venous cord.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-ELASTIC-LAMINA-STAIN-01

## concept_id
CON-CVS-0AD4BCDFB88035

## subject
Elastic-lamina histologic stain (Orcein)

## predicate
is

## object
Orcein is the special stain used to visualise elastic laminae in the arterial wall, since routine H&E does not resolve them clearly.

## display_text
Orcein is the special stain used to visualise elastic laminae in the arterial wall, since routine H&E does not resolve them clearly.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.65

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-GIT-ABDOMINAL-ESOPHAGUS-SUPPLY-01

## concept_id
CON-GIT-1817A46BEDC8BD

## subject
Abdominal oesophagus: blood supply from the left gastric artery

## predicate
is

## object
The abdominal part of the oesophagus is supplied by oesophageal branches of the left gastric artery, a branch of the coeliac trunk, rather than by the descending thoracic aorta's own oesophageal branches which supply the thoracic segment.

## display_text
The abdominal part of the oesophagus is supplied by oesophageal branches of the left gastric artery, a branch of the coeliac trunk, rather than by the descending thoracic aorta's own oesophageal branches which supply the thoracic segment.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-B85BC8B6AAB51A

## concept_id
CON-INF-B85BC8B6AAB51A

## subject
Staphylococcus aureus and acute infective endocarditis

## predicate
is

## object
Staphylococcus aureus is the most common cause of acute infective endocarditis and classically produces a fulminant illness, including right-sided (tricuspid) disease in intravenous drug users.

## display_text
Staphylococcus aureus is the most common cause of acute infective endocarditis and classically produces a fulminant illness, including right-sided (tricuspid) disease in intravenous drug users.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-5CE506A0EE2F49

## concept_id
CON-INF-5CE506A0EE2F49

## subject
Embolic pattern of acute Staphylococcus aureus endocarditis

## predicate
is

## object
Staphylococcal sepsis followed by acute valve destruction and embolic phenomena to distant organs (brain, spleen) is the characteristic pattern of acute Staphylococcus aureus endocarditis.

## display_text
Staphylococcal sepsis followed by acute valve destruction and embolic phenomena to distant organs (brain, spleen) is the characteristic pattern of acute Staphylococcus aureus endocarditis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-5D7344B6B2EC3C

## concept_id
CON-INF-5D7344B6B2EC3C

## subject
Viridans streptococci: dextran-mediated valve adherence

## predicate
is

## object
Viridans streptococci adhere to damaged heart valves by synthesising dextran, a glucose-derived biofilm, which anchors the organism to a pre-existing fibrin-platelet vegetation.

## display_text
Viridans streptococci adhere to damaged heart valves by synthesising dextran, a glucose-derived biofilm, which anchors the organism to a pre-existing fibrin-platelet vegetation.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-DDBD3BC0EBA46E

## concept_id
CON-INF-DDBD3BC0EBA46E

## subject
Blood cultures in the diagnosis of infective endocarditis

## predicate
is

## object
Serial blood cultures drawn from separate sites before starting any antibiotic are the best initial diagnostic investigation for suspected infective endocarditis.

## display_text
Serial blood cultures drawn from separate sites before starting any antibiotic are the best initial diagnostic investigation for suspected infective endocarditis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-9E294C0B1846C2

## concept_id
CON-INF-9E294C0B1846C2

## subject
Janeway lesions in infective endocarditis

## predicate
is

## object
Janeway lesions are painless, macular, embolic skin lesions on the palms and soles caused by septic micro-emboli in infective endocarditis.

## display_text
Janeway lesions are painless, macular, embolic skin lesions on the palms and soles caused by septic micro-emboli in infective endocarditis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-E7C02DE64681D8

## concept_id
CON-INF-E7C02DE64681D8

## subject
Empirical antimicrobial cover in suspected infective endocarditis

## predicate
is

## object
Empirical antimicrobial therapy for suspected infective endocarditis covers Staphylococcus aureus/MRSA, Viridans streptococci and enterococci, but has no role against viral pathogens such as Coxsackievirus B.

## display_text
Empirical antimicrobial therapy for suspected infective endocarditis covers Staphylococcus aureus/MRSA, Viridans streptococci and enterococci, but has no role against viral pathogens such as Coxsackievirus B.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-96FA06BB80E670

## concept_id
CON-INF-96FA06BB80E670

## subject
Antibiotic prophylaxis before dental procedures in endocarditis risk

## predicate
is

## object
A single dose of oral amoxicillin given before a dental procedure is the recommended antibiotic prophylaxis for a patient at elevated risk of infective endocarditis.

## display_text
A single dose of oral amoxicillin given before a dental procedure is the recommended antibiotic prophylaxis for a patient at elevated risk of infective endocarditis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-921BB840F59775

## concept_id
CON-INF-921BB840F59775

## subject
Staphylococcus epidermidis biofilm pathogenesis on prosthetic devices

## predicate
is

## object
Staphylococcus epidermidis causes prosthetic-valve endocarditis chiefly through glycocalyx (biofilm) formation on the device surface, not through tissue invasion or potent exotoxins.

## display_text
Staphylococcus epidermidis causes prosthetic-valve endocarditis chiefly through glycocalyx (biofilm) formation on the device surface, not through tissue invasion or potent exotoxins.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-B5EFF86198FE83

## concept_id
CON-INF-B5EFF86198FE83

## subject
Coxsackievirus B as the leading cause of viral myocarditis

## predicate
is

## object
Coxsackievirus B is the most common viral cause of myocarditis, owing to its specific tropism for cardiac myocytes.

## display_text
Coxsackievirus B is the most common viral cause of myocarditis, owing to its specific tropism for cardiac myocytes.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-605E48CC7E4DE4

## concept_id
CON-INF-605E48CC7E4DE4

## subject
Diagnostic features of myocarditis

## predicate
is

## object
Myocarditis is diagnosed from a combination of elevated cardiac enzymes and a heart-failure clinical presentation.

## display_text
Myocarditis is diagnosed from a combination of elevated cardiac enzymes and a heart-failure clinical presentation.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-9D42060DD85811

## concept_id
CON-INF-9D42060DD85811

## subject
Pleuritic chest pain in acute pericarditis

## predicate
is

## object
Acute pericarditis classically presents with chest pain that worsens with inspiration and coughing, reflecting irritation of the pain-sensitive parietal pericardium.

## display_text
Acute pericarditis classically presents with chest pain that worsens with inspiration and coughing, reflecting irritation of the pain-sensitive parietal pericardium.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-710B2DE024DB74

## concept_id
CON-INF-710B2DE024DB74

## subject
Streptococcus pneumoniae in purulent pericarditis

## predicate
is

## object
Streptococcus pneumoniae is a recognised cause of purulent (infective) pericarditis, typically by direct spread from an adjacent pneumonic focus or haematogenous seeding.

## display_text
Streptococcus pneumoniae is a recognised cause of purulent (infective) pericarditis, typically by direct spread from an adjacent pneumonic focus or haematogenous seeding.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-ABCFBB77F6B9F5

## concept_id
CON-INF-ABCFBB77F6B9F5

## subject
Stool isolation of Coxsackievirus B in acute myocarditis

## predicate
is

## object
When an enterovirus is isolated from stool in a patient with acute myocarditis, Coxsackievirus B is the most likely agent recovered.

## display_text
When an enterovirus is isolated from stool in a patient with acute myocarditis, Coxsackievirus B is the most likely agent recovered.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-7A4574086BD96A

## concept_id
CON-INF-7A4574086BD96A

## subject
Supportive management of myocarditis

## predicate
is

## object
Most causes of myocarditis, particularly viral myocarditis, have no specific cure; management is supportive, directed at heart failure and its complications.

## display_text
Most causes of myocarditis, particularly viral myocarditis, have no specific cure; management is supportive, directed at heart failure and its complications.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-4C3F7A81008817

## concept_id
CON-INF-4C3F7A81008817

## subject
Antistreptolysin O titer in the diagnosis of rheumatic fever

## predicate
is

## object
The antistreptolysin O (ASO) titer is a specific serologic marker of past streptococcal infection; a titer above about 200 IU/mL supports the diagnosis of rheumatic fever.

## display_text
The antistreptolysin O (ASO) titer is a specific serologic marker of past streptococcal infection; a titer above about 200 IU/mL supports the diagnosis of rheumatic fever.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-8D4ADBA83C4DA9

## concept_id
CON-INF-8D4ADBA83C4DA9

## subject
Penicillin timing and rheumatic carditis

## predicate
is

## object
Penicillin given after the onset of rheumatic fever eradicates any remaining streptococcal infection but does not reverse rheumatic carditis that has already become established.

## display_text
Penicillin given after the onset of rheumatic fever eradicates any remaining streptococcal infection but does not reverse rheumatic carditis that has already become established.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-A34594329B91AD

## concept_id
CON-INF-A34594329B91AD

## subject
Laboratory differentiation of Viridans streptococci from Streptococcus pneumoniae

## predicate
is

## object
Viridans streptococci are distinguished from Streptococcus pneumoniae by being optochin-resistant and insoluble in bile, whereas S. pneumoniae is optochin-sensitive and bile-soluble.

## display_text
Viridans streptococci are distinguished from Streptococcus pneumoniae by being optochin-resistant and insoluble in bile, whereas S. pneumoniae is optochin-sensitive and bile-soluble.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-33D503C0F8E835

## concept_id
CON-INF-33D503C0F8E835

## subject
Suckling-mouse model of Coxsackievirus B infection

## predicate
is

## object
In the suckling-mouse laboratory model, Coxsackievirus B produces myositis together with encephalitis, distinguishing it from Coxsackievirus A.

## display_text
In the suckling-mouse laboratory model, Coxsackievirus B produces myositis together with encephalitis, distinguishing it from Coxsackievirus A.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-7FF5FE97568DA6

## concept_id
CON-INF-7FF5FE97568DA6

## subject
Routes by which pathogens reach the pericardium

## predicate
is

## object
Pathogens reach the pericardium by haematogenous spread, direct extension from an adjacent thoracic structure, or spread from an infected myocardium -- not by inhalation of contaminated air.

## display_text
Pathogens reach the pericardium by haematogenous spread, direct extension from an adjacent thoracic structure, or spread from an infected myocardium -- not by inhalation of contaminated air.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-F4CDAB96CB55F9

## concept_id
CON-INF-F4CDAB96CB55F9

## subject
Faecal-oral transmission of Coxsackievirus

## predicate
is

## object
Coxsackievirus is transmitted by the faecal-oral route, like other enteroviruses.

## display_text
Coxsackievirus is transmitted by the faecal-oral route, like other enteroviruses.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-AB05714C0C8981

## concept_id
CON-INF-AB05714C0C8981

## subject
Primary replication site of Coxsackievirus

## predicate
is

## object
The primary site of Coxsackievirus multiplication is the lymphoid tissue of the gastrointestinal tract, prior to a secondary viraemia that disseminates it to distant organs.

## display_text
The primary site of Coxsackievirus multiplication is the lymphoid tissue of the gastrointestinal tract, prior to a secondary viraemia that disseminates it to distant organs.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-2608E0642A91AF

## concept_id
CON-INF-2608E0642A91AF

## subject
Disease associations of Coxsackievirus B

## predicate
is

## object
Coxsackievirus B is associated with pleurodynia, myocarditis, and has been implicated as a possible environmental trigger for type 1 diabetes mellitus.

## display_text
Coxsackievirus B is associated with pleurodynia, myocarditis, and has been implicated as a possible environmental trigger for type 1 diabetes mellitus.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-1FF0B557692436

## concept_id
CON-INF-1FF0B557692436

## subject
Bacterial causes of pericarditis and the exception of S. pyogenes

## predicate
is

## object
Streptococcus pyogenes is not a typical direct causative organism of infective pericarditis, unlike Staphylococcus aureus, Streptococcus pneumoniae and Mycobacterium tuberculosis.

## display_text
Streptococcus pyogenes is not a typical direct causative organism of infective pericarditis, unlike Staphylococcus aureus, Streptococcus pneumoniae and Mycobacterium tuberculosis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-8B8271500495E1

## concept_id
CON-INF-8B8271500495E1

## subject
Fever pattern in subacute versus acute infective endocarditis

## predicate
is

## object
High spiking fever with rigors is not typical of subacute infective endocarditis, which instead presents with a low-grade, indolent fever; the spiking pattern is characteristic of acute endocarditis.

## display_text
High spiking fever with rigors is not typical of subacute infective endocarditis, which instead presents with a low-grade, indolent fever; the spiking pattern is characteristic of acute endocarditis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-D756F46B795F18

## concept_id
CON-INF-D756F46B795F18

## subject
Taxonomic classification of Coxsackievirus

## predicate
is

## object
Coxsackievirus is classified within the family Picornaviridae, a family of small, non-enveloped, positive-sense single-stranded RNA viruses.

## display_text
Coxsackievirus is classified within the family Picornaviridae, a family of small, non-enveloped, positive-sense single-stranded RNA viruses.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-F82B27A0C7E03F

## concept_id
CON-INF-F82B27A0C7E03F

## subject
Non-specific inflammatory markers in rheumatic fever

## predicate
is

## object
Erythrocyte sedimentation rate and C-reactive protein are non-specific acute-phase markers used as supportive evidence in rheumatic fever, unlike the specific antistreptolysin O titer.

## display_text
Erythrocyte sedimentation rate and C-reactive protein are non-specific acute-phase markers used as supportive evidence in rheumatic fever, unlike the specific antistreptolysin O titer.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-963A049A93D8FA

## concept_id
CON-INF-963A049A93D8FA

## subject
Toxin-mediated myocarditis in diphtheria

## predicate
is

## object
Myocarditis, caused by systemic absorption of diphtheria exotoxin, is a recognised serious complication of Corynebacterium diphtheriae infection.

## display_text
Myocarditis, caused by systemic absorption of diphtheria exotoxin, is a recognised serious complication of Corynebacterium diphtheriae infection.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-FD94F6ED59BCBF

## concept_id
CON-INF-FD94F6ED59BCBF

## subject
Staphylococcus aureus and Mycobacterium tuberculosis as leading causes of pericarditis

## predicate
is

## object
Staphylococcus aureus and Mycobacterium tuberculosis are cited as the most common causes of pericarditis among bacterial and mycobacterial aetiologies.

## display_text
Staphylococcus aureus and Mycobacterium tuberculosis are cited as the most common causes of pericarditis among bacterial and mycobacterial aetiologies.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-3F8F9D6E7F52AF

## concept_id
CON-INF-3F8F9D6E7F52AF

## subject
Penicillin-aminoglycoside synergy in endocarditis treatment

## predicate
is

## object
Infective endocarditis is treated with a combination of a cell-wall-active agent (penicillin or vancomycin) plus an aminoglycoside, exploiting their synergistic bactericidal action.

## display_text
Infective endocarditis is treated with a combination of a cell-wall-active agent (penicillin or vancomycin) plus an aminoglycoside, exploiting their synergistic bactericidal action.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-8C1DE35DE714DD

## concept_id
CON-INF-8C1DE35DE714DD

## subject
Generalised Coxsackievirus B disease in infants

## predicate
is

## object
Coxsackievirus B causes a severe, generalised disease with multi-organ involvement (heart, liver and brain) in infants, distinct from the milder illness typical of older patients.

## display_text
Coxsackievirus B causes a severe, generalised disease with multi-organ involvement (heart, liver and brain) in infants, distinct from the milder illness typical of older patients.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-A9C4520FA0EB77

## concept_id
CON-INF-A9C4520FA0EB77

## subject
Candida albicans myocarditis in the immunocompromised host

## predicate
is

## object
Candida albicans is a recognised opportunistic cause of myocarditis in immunocompromised patients, reaching the myocardium by haematogenous spread during candidaemia.

## display_text
Candida albicans is a recognised opportunistic cause of myocarditis in immunocompromised patients, reaching the myocardium by haematogenous spread during candidaemia.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-INF-6E83C5F7AAA95B

## concept_id
CON-INF-6E83C5F7AAA95B

## subject
Distinguishing acute endocarditis from rheumatic fever by joint findings

## predicate
is

## object
Migratory polyarthritis is not a typical symptom of acute infective endocarditis; it is instead a major feature of the distinct, immune-mediated disease rheumatic fever.

## display_text
Migratory polyarthritis is not a typical symptom of acute infective endocarditis; it is instead a major feature of the distinct, immune-mediated disease rheumatic fever.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students


---

# Item

## id
CLM-CVS-PERICARDIAL-EFFUSION-CLASSIFICATION-01

## concept_id
CON-CVS-197847608085B6

## subject
Pericardial effusion is classified by the character of the fluid, and lymphatic obstruction produces a transudative effusion

## predicate
is

## object
Pericardial effusion is classified by the character of the accumulated fluid: transudative (low-protein, from obstruction of lymphatic drainage, heart failure or hypoproteinaemia), exudative (protein-rich, from inflammation or infection), haemorrhagic (blood, from trauma, malignancy or dissection) or suppurative (frank pus, from pyogenic infection). Obstruction of pericardial lymphatic drainage specifically produces a transudative effusion rather than an inflammatory one.

## display_text
Pericardial effusion is classified by the character of the accumulated fluid: transudative (low-protein, from obstruction of lymphatic drainage, heart failure or hypoproteinaemia), exudative (protein-rich, from inflammation or infection), haemorrhagic (blood, from trauma, malignancy or dissection) or suppurative (frank pus, from pyogenic infection). Obstruction of pericardial lymphatic drainage specifically produces a transudative effusion rather than an inflammatory one.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-TB-PERICARDITIS-EXUDATE-01

## concept_id
CON-CVS-70D38691002F4A

## subject
Tuberculous pericarditis characteristically produces a fibrinous and haemorrhagic exudate

## predicate
is

## object
Tuberculous pericarditis characteristically produces a fibrinous and haemorrhagic exudate, reflecting caseating granulomatous inflammation and vascular involvement of the pericardium, distinct from the clear, serous fluid of viral pericarditis and the frank pus of pyogenic (suppurative) pericarditis.

## display_text
Tuberculous pericarditis characteristically produces a fibrinous and haemorrhagic exudate, reflecting caseating granulomatous inflammation and vascular involvement of the pericardium, distinct from the clear, serous fluid of viral pericarditis and the frank pus of pyogenic (suppurative) pericarditis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-SUPPURATIVE-PERICARDITIS-01

## concept_id
CON-CVS-BE0B74CF459B26

## subject
Suppurative pericarditis follows direct pyogenic contamination (e.g. a penetrating wound) and produces purulent exudate

## predicate
is

## object
Suppurative (purulent) pericarditis follows direct pyogenic bacterial contamination of the pericardial sac — classically a penetrating chest wound, but also contiguous spread from pneumonia or empyema, or haematogenous seeding — and produces frank purulent exudate. It carries a high risk of subsequent constrictive pericarditis as the purulent exudate organises into dense fibrous scar.

## display_text
Suppurative (purulent) pericarditis follows direct pyogenic bacterial contamination of the pericardial sac — classically a penetrating chest wound, but also contiguous spread from pneumonia or empyema, or haematogenous seeding — and produces frank purulent exudate. It carries a high risk of subsequent constrictive pericarditis as the purulent exudate organises into dense fibrous scar.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-ATHEROMA-PLAQUE-STRUCTURE-01

## concept_id
CON-CVS-217CCB33D34DB0

## subject
The fibrous atheromatous plaque, the basic lesion of atherosclerosis, sits in the tunica intima with a fibrous cap, lipid zone and basal zone

## predicate
is

## object
The fibrous atheromatous plaque is the basic lesion of atherosclerosis, developing from the fatty streak within the tunica intima. It has three zones: a fibrous cap (collagen and smooth muscle cells) covering a lipid zone (foam cells, extracellular lipid and cholesterol), overlying a basal zone (collagen, smooth muscle cells and macrophages). Atherosclerosis is by definition an intimal disease — the plaque does not primarily involve the tunica media or adventitia.

## display_text
The fibrous atheromatous plaque is the basic lesion of atherosclerosis, developing from the fatty streak within the tunica intima. It has three zones: a fibrous cap (collagen and smooth muscle cells) covering a lipid zone (foam cells, extracellular lipid and cholesterol), overlying a basal zone (collagen, smooth muscle cells and macrophages). Atherosclerosis is by definition an intimal disease — the plaque does not primarily involve the tunica media or adventitia.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-ATHEROSCLEROSIS-COMPLICATIONS-01

## concept_id
CON-CVS-E03F9800584C73

## subject
Atherosclerotic plaques complicate by thromboembolism, rupture/haemorrhage, calcification, aneurysm formation and organ-specific ischaemia

## predicate
is

## object
Complications of atherosclerosis include thromboembolism, plaque rupture and haemorrhage, dystrophic calcification (which hardens rather than softens the vessel wall), aneurysm formation, and downstream ischaemia specific to the artery involved — popliteal-artery atherosclerosis causes intermittent limb claudication, and atherosclerotic renal artery stenosis causes both hypertension (via reduced renal perfusion activating the renin-angiotensin system) and renal infarction. Hypertrophic cardiomyopathy is a primary sarcomere disease, not a complication of atherosclerosis.

## display_text
Complications of atherosclerosis include thromboembolism, plaque rupture and haemorrhage, dystrophic calcification (which hardens rather than softens the vessel wall), aneurysm formation, and downstream ischaemia specific to the artery involved — popliteal-artery atherosclerosis causes intermittent limb claudication, and atherosclerotic renal artery stenosis causes both hypertension (via reduced renal perfusion activating the renin-angiotensin system) and renal infarction. Hypertrophic cardiomyopathy is a primary sarcomere disease, not a complication of atherosclerosis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-BENIGN-HYPERTENSION-01

## concept_id
CON-CVS-F7B45A1E821B0A

## subject
Benign hypertension is sustained pressure below about 200/110 mmHg, producing hyaline arteriolosclerosis from plasma-protein deposition and death chiefly from congestive heart failure

## predicate
is

## object
Benign hypertension is chronic, sustained blood pressure characteristically below about 200/110 mmHg. Its characteristic renal arteriolar change is hyaline arteriolosclerosis — pink, structureless, homogeneous thickening of the arteriolar wall from the deposition of an abnormal (plasma) protein, seen also in diabetes and in otherwise-normotensive elderly patients as an ageing change. In benign hypertension, death occurs, in order of frequency, from congestive heart failure, coronary infarction, cerebral haemorrhage, and then renal failure.

## display_text
Benign hypertension is chronic, sustained blood pressure characteristically below about 200/110 mmHg. Its characteristic renal arteriolar change is hyaline arteriolosclerosis — pink, structureless, homogeneous thickening of the arteriolar wall from the deposition of an abnormal (plasma) protein, seen also in diabetes and in otherwise-normotensive elderly patients as an ageing change. In benign hypertension, death occurs, in order of frequency, from congestive heart failure, coronary infarction, cerebral haemorrhage, and then renal failure.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-PREECLAMPSIA-HYPERTENSION-RISK-01

## concept_id
CON-CVS-C400BC47E85D70

## subject
Pre-eclampsia risk is raised by first pregnancy, older maternal age and pre-existing diabetes

## predicate
is

## object
Pre-eclampsia — new-onset hypertension with proteinuria after 20 weeks of pregnancy — has its risk raised by nulliparity (first pregnancy), older maternal age, and pre-existing diabetes mellitus, among other factors (multiple gestation, obesity, chronic hypertension, autoimmune disease).

## display_text
Pre-eclampsia — new-onset hypertension with proteinuria after 20 weeks of pregnancy — has its risk raised by nulliparity (first pregnancy), older maternal age, and pre-existing diabetes mellitus, among other factors (multiple gestation, obesity, chronic hypertension, autoimmune disease).

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-SLEEP-APNEA-HYPERTENSION-RISK-01

## concept_id
CON-CVS-E0908501269680

## subject
Obstructive sleep apnoea is a risk factor for hypertension, stroke and heart attack

## predicate
is

## object
Obstructive sleep apnoea causes repeated nocturnal hypoxia and sympathetic surges that raise systemic blood pressure over time, and is an independent risk factor for hypertension, stroke and myocardial infarction (heart attack).

## display_text
Obstructive sleep apnoea causes repeated nocturnal hypoxia and sympathetic surges that raise systemic blood pressure over time, and is an independent risk factor for hypertension, stroke and myocardial infarction (heart attack).

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-ANEURYSM-CLASSIFICATION-01

## concept_id
CON-CVS-F3E95B92367825

## subject
A true aneurysm carries all vessel-wall layers and is either fusiform (entire circumference) or saccular (part of it); atherosclerotic aneurysms characteristically form in the abdominal aorta and can rupture, embolise or compress

## predicate
is

## object
A true aneurysm is an abnormal, localised dilation whose wall is formed of the complete, though often weakened, vessel wall layers — distinguishing it from a false (pseudo-) aneurysm, whose wall is adjacent connective tissue around a contained rupture. A fusiform aneurysm dilates the entire circumference of the vessel; a saccular aneurysm dilates only part of the circumference, as a sac-like outpouching. Atherosclerotic aneurysms characteristically form in the abdominal aorta. Aneurysm complications include thromboembolism, rupture and haemorrhage, and pressure effects on surrounding organs; an aneurysm does not improve blood flow.

## display_text
A true aneurysm is an abnormal, localised dilation whose wall is formed of the complete, though often weakened, vessel wall layers — distinguishing it from a false (pseudo-) aneurysm, whose wall is adjacent connective tissue around a contained rupture. A fusiform aneurysm dilates the entire circumference of the vessel; a saccular aneurysm dilates only part of the circumference, as a sac-like outpouching. Atherosclerotic aneurysms characteristically form in the abdominal aorta. Aneurysm complications include thromboembolism, rupture and haemorrhage, and pressure effects on surrounding organs; an aneurysm does not improve blood flow.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-DISSECTING-AORTIC-ANEURYSM-01

## concept_id
CON-CVS-4A3DD926C2BCE4

## subject
Dissecting aortic aneurysm is associated with Marfan syndrome, presents with tearing pain and hypotension, and can erode adjacent bone to cause pain

## predicate
is

## object
Dissecting aortic aneurysm — a tear in the intima allowing blood to track within the media, creating a false lumen — is most strongly associated with Marfan syndrome (cystic medial degeneration weakening the aortic wall), though hypertension and atherosclerosis also predispose. It classically presents with sudden, severe "tearing" chest or back pain and hypotension from blood loss into the false lumen or pericardium. A descending aortic aneurysm, dissecting or not, can erode adjacent vertebral bone and cause back pain from that direct pressure effect.

## display_text
Dissecting aortic aneurysm — a tear in the intima allowing blood to track within the media, creating a false lumen — is most strongly associated with Marfan syndrome (cystic medial degeneration weakening the aortic wall), though hypertension and atherosclerosis also predispose. It classically presents with sudden, severe "tearing" chest or back pain and hypotension from blood loss into the false lumen or pericardium. A descending aortic aneurysm, dissecting or not, can erode adjacent vertebral bone and cause back pain from that direct pressure effect.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-RASMUSSEN-ANEURYSM-01

## concept_id
CON-CVS-0FFB81F3C1CC4B

## subject
A Rasmussen aneurysm is a pulmonary artery aneurysm within a tuberculous lung cavity

## predicate
is

## object
A Rasmussen aneurysm is a pseudoaneurysm of a pulmonary artery branch, forming within the wall of a chronic tuberculous cavity as the vessel is progressively weakened by adjacent caseous necrosis and inflammation. Its rupture is a classic cause of life-threatening haemoptysis in pulmonary tuberculosis.

## display_text
A Rasmussen aneurysm is a pseudoaneurysm of a pulmonary artery branch, forming within the wall of a chronic tuberculous cavity as the vessel is progressively weakened by adjacent caseous necrosis and inflammation. Its rupture is a classic cause of life-threatening haemoptysis in pulmonary tuberculosis.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-MYCOTIC-ANEURYSM-01

## concept_id
CON-CVS-5916244D7A012F

## subject
A mycotic aneurysm arises from septic embolisation of infected material, classically from infective endocarditis

## predicate
is

## object
A mycotic aneurysm is a localised arterial dilation caused by infection of the vessel wall, most commonly from septic embolisation of infected material — classically vegetations shed from infective endocarditis, which seed and weaken a distal artery wall, or from direct bacteraemic seeding of an already-diseased wall. It presents as a pulsatile, tender mass at the affected site, often with systemic signs of the underlying infection (fever, positive blood cultures, a murmur).

## display_text
A mycotic aneurysm is a localised arterial dilation caused by infection of the vessel wall, most commonly from septic embolisation of infected material — classically vegetations shed from infective endocarditis, which seed and weaken a distal artery wall, or from direct bacteraemic seeding of an already-diseased wall. It presents as a pulsatile, tender mass at the affected site, often with systemic signs of the underlying infection (fever, positive blood cultures, a murmur).

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-TAKAYASU-ARTERITIS-01

## concept_id
CON-CVS-AA3A2E6B6084F2

## subject
Takayasu arteritis typically affects young women, involving the aorta and its major branches

## predicate
is

## object
Takayasu arteritis is a large-vessel vasculitis typically affecting women of childbearing age, involving the aorta and its major branches with granulomatous inflammation that leads to wall thickening, stenosis and, later, absent or diminished peripheral pulses ("pulseless disease").

## display_text
Takayasu arteritis is a large-vessel vasculitis typically affecting women of childbearing age, involving the aorta and its major branches with granulomatous inflammation that leads to wall thickening, stenosis and, later, absent or diminished peripheral pulses ("pulseless disease").

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-POLYARTERITIS-NODOSA-01

## concept_id
CON-CVS-00545B736FF8F7

## subject
Polyarteritis nodosa most commonly involves the medium-sized arteries of the gastrointestinal tract

## predicate
is

## object
Polyarteritis nodosa is a necrotising vasculitis of medium-sized muscular arteries that most commonly involves the arteries of the gastrointestinal tract, producing segmental, transmural inflammation with fibrinoid necrosis and a classic "beading" of small aneurysms on angiography; renal, musculoskeletal, skin and peripheral-nerve arteries are also frequently affected, but GIT involvement is the printed, most-common site tested here.

## display_text
Polyarteritis nodosa is a necrotising vasculitis of medium-sized muscular arteries that most commonly involves the arteries of the gastrointestinal tract, producing segmental, transmural inflammation with fibrinoid necrosis and a classic "beading" of small aneurysms on angiography; renal, musculoskeletal, skin and peripheral-nerve arteries are also frequently affected, but GIT involvement is the printed, most-common site tested here.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students


---

# Item

## id
CLM-CVS-VASOMOTOR-TONE-01

## concept_id
CON-CVS-511A10F77151BC

## subject
Continuous partial vasoconstriction of blood vessels (vasomotor tone) is maintained by ongoing sympathetic discharge

## predicate
is

## object
Blood vessels, particularly arterioles, are held in a state of continuous partial constriction called vasomotor tone, produced by ongoing, baseline tonic discharge from sympathetic vasoconstrictor (adrenergic) fibres to vascular smooth muscle, releasing norepinephrine onto alpha-adrenergic receptors even at rest.

## display_text
Blood vessels, particularly arterioles, are held in a state of continuous partial constriction called vasomotor tone, produced by ongoing, baseline tonic discharge from sympathetic vasoconstrictor (adrenergic) fibres to vascular smooth muscle, releasing norepinephrine onto alpha-adrenergic receptors even at rest.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-EPINEPHRINE-DUAL-ACTION-01

## concept_id
CON-CVS-8EE3B5EE80F342

## subject
Epinephrine from the adrenal medulla acts on both alpha- and beta-adrenergic receptors, constricting most vascular beds but dilating skeletal-muscle vessels

## predicate
is

## object
Epinephrine acts on alpha-adrenergic receptors (predominant in skin, splanchnic and renal vessels) to cause vasoconstriction, but on beta2-adrenergic receptors (predominant in skeletal muscle) to cause vasodilation, increasing blood flow to muscle during a "fight or flight" response.

## display_text
Epinephrine acts on alpha-adrenergic receptors (predominant in skin, splanchnic and renal vessels) to cause vasoconstriction, but on beta2-adrenergic receptors (predominant in skeletal muscle) to cause vasodilation, increasing blood flow to muscle during a "fight or flight" response.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-KININS-PAIN-PERMEABILITY-01

## concept_id
CON-CVS-AD0D8E768C2018

## subject
Kinins (bradykinin) are potent vasodilators that also increase capillary permeability and directly stimulate pain nerve endings

## predicate
is

## object
Kinins act directly on capillary and venular endothelium to increase capillary permeability, contributing to the fluid exudation seen in acute inflammation, and directly stimulate free nerve endings that carry pain sensation, producing pain in inflamed or injured tissue.

## display_text
Kinins act directly on capillary and venular endothelium to increase capillary permeability, contributing to the fluid exudation seen in acute inflammation, and directly stimulate free nerve endings that carry pain sensation, producing pain in inflamed or injured tissue.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.65

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-FLARE-REACTION-AXON-REFLEX-01

## concept_id
CON-CVS-7F05227FE0970F

## subject
The skin flare reaction is vasodilation via the axon reflex, in which antidromic impulses in sensory nerve branches release local vasodilator peptides

## predicate
is

## object
An impulse travelling centrally along one branch of a sensory afferent fibre from a stimulated skin site can also travel antidromically down neighbouring branches of that same afferent neuron that innervate adjacent skin, triggering local release of vasodilator neuropeptides from the nerve terminals and producing the flare's characteristic spread.

## display_text
An impulse travelling centrally along one branch of a sensory afferent fibre from a stimulated skin site can also travel antidromically down neighbouring branches of that same afferent neuron that innervate adjacent skin, triggering local release of vasodilator neuropeptides from the nerve terminals and producing the flare's characteristic spread.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-ENDOTHELIN-VASOCONSTRICTOR-01

## concept_id
CON-CVS-7D26B65DA355A1

## subject
Endothelin, released by endothelial cells in response to vessel stretch, is the most potent vasoconstrictor known

## predicate
is

## object
Endothelin is a peptide released by vascular endothelial cells in response to stretch of the vessel wall, tissue injury and hypoxia, acting on endothelin-A receptors on vascular smooth muscle to produce intense, prolonged vasoconstriction more potent on a molar basis than angiotensin II or norepinephrine.

## display_text
Endothelin is a peptide released by vascular endothelial cells in response to stretch of the vessel wall, tissue injury and hypoxia, acting on endothelin-A receptors on vascular smooth muscle to produce intense, prolonged vasoconstriction more potent on a molar basis than angiotensin II or norepinephrine.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-VASCULAR-FUNCTIONAL-CLASSIFICATION-01

## concept_id
CON-CVS-3E68C888B95668

## subject
Blood vessels are classified functionally as resistance vessels (arterioles), exchange vessels (capillaries) or capacitance vessels (veins)

## predicate
is

## object
Capillaries are the exchange vessels of the circulation: a single layer of endothelium with a large total surface area allows diffusion of gases, nutrients and waste between blood and interstitial fluid, distinct from the resistance role of arterioles and the capacitance role of veins.

## display_text
Capillaries are the exchange vessels of the circulation: a single layer of endothelium with a large total surface area allows diffusion of gases, nutrients and waste between blood and interstitial fluid, distinct from the resistance role of arterioles and the capacitance role of veins.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.75

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-PROSTACYCLIN-VASODILATOR-01

## concept_id
CON-CVS-487BC301A49AE2

## subject
Prostacyclin (PGI2), synthesised by endothelium from arachidonic acid via cyclooxygenase, is a local vasodilator as well as a platelet-aggregation inhibitor

## predicate
is

## object
Prostacyclin, synthesised by vascular endothelial cells from arachidonic acid via cyclooxygenase, acts on vascular smooth muscle to raise intracellular cAMP and produce local vasodilation, distinct from its better-known anti-platelet-aggregation role.

## display_text
Prostacyclin, synthesised by vascular endothelial cells from arachidonic acid via cyclooxygenase, acts on vascular smooth muscle to raise intracellular cAMP and produce local vasodilation, distinct from its better-known anti-platelet-aggregation role.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-BEZOLD-JARISCH-REFLEX-01

## concept_id
CON-CVS-E48F6E57CCBB2F

## subject
Left ventricular receptors act as both mechanoreceptors and chemoreceptors, and their stimulation (the Bezold-Jarisch reflex) causes bradycardia and hypotension

## predicate
is

## object
Vagal afferent stimulation of left ventricular mechano/chemoreceptors produces the Bezold-Jarisch reflex (coronary chemoreflex): bradycardia, hypotension and coronary vasodilation, mediated by a surge of parasympathetic outflow and withdrawal of sympathetic vasoconstrictor tone.

## display_text
Vagal afferent stimulation of left ventricular mechano/chemoreceptors produces the Bezold-Jarisch reflex (coronary chemoreflex): bradycardia, hypotension and coronary vasodilation, mediated by a surge of parasympathetic outflow and withdrawal of sympathetic vasoconstrictor tone.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students

---

# Item

## id
CLM-CVS-MEDULLARY-CARDIOVASCULAR-CENTERS-01

## concept_id
CON-CVS-595DFC43B96D7D

## subject
The medullary vasomotor centre has pressor (vasoconstrictor) and depressor (vasodilator) areas, and a separate cardiac inhibitory centre slows the heart via the vagus

## predicate
is

## object
The medullary vasomotor centre's pressor area contains the vasoconstrictor centre and generates tonic sympathetic discharge, while a separate cardiac inhibitory centre sends vagal efferents to the SA node to slow heart rate.

## display_text
The medullary vasomotor centre's pressor area contains the vasoconstrictor centre and generates tonic sympathetic discharge, while a separate cardiac inhibitory centre sends vagal efferents to the SA node to slow heart rate.

## risk_class
treatment_or_action

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## review_due


## qualifiers
population: MUST Year 2 medical students
