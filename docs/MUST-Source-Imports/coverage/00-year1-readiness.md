...
**`checkpointed-incomplete`.** The completed read-only sources yield 3297 questions, 3031 printed keys/answers, and 432 tested concepts (66 live-hit / 84 pending-hit / 282 new, all placement TBD). The newly closed four-page `MCQs - Epithelium MCQ.pdf` bank contributes 25 MCQs and a complete 25-item key. Its eight source-first handles replay 32 searches, and all eight are exact prior-FHB scope reuse, so it adds no concept. The remaining source set is 60 inventory rows and 58 SHA-256 values, checksum `e41069abd74f8c0ee06c06435cf875c803775eb66ddf7894b066a8a6d9410eb4`; debt is 17 substantive-text, 37 empty-text and 6 sparse-text rows.
