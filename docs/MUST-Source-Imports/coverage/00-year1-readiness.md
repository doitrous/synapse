...
**`checkpointed-incomplete`.** The completed read-only sources yield 6482 questions, 6118 prompt-matched recovered answers, and 485 tested concepts (66 live-hit / 90 pending-hit / 329 new, all placement TBD). The newly closed 21-page `QUESTION ON CYTOLOGY by DR.KANDEEL.pdf` is an empty-text iOS/Quartz scan branded `DR.KANDEEL`; it names its instructor and FHB101 cytology scope but prints no institution, department, sitting, marks or official-exam claim. Its six MCQ blocks contain 238 observed prompts and 236 printed key entries. The `CELL.4` block prints two different prompts labelled Q36 and two labelled Q37, so those two key labels cannot be assigned without inference; the exact recovered boundary is 234 prompt-matched answers. Eighteen source-first handles replay 72 searches and all collapse to exact prior-FHB reuse, so the cumulative concept delta is zero. The remaining source set is 32 inventory rows and 32 SHA-256 values, checksum `cace2183e8320d3cb35b09dd8d3e3d6d7852192731faa9e03e9434148a71f3e1`; debt is 6 substantive-text, 21 empty-text and 5 sparse-text rows.
