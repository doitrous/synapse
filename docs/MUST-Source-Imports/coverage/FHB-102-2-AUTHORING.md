...
| Claims | 117 | verified against local curriculum citations; independent review still owed |
| Citations | 122 | local curriculum |
| Article spans | 117 | linked to the clean-key claims |
| Concepts | 103 | under review / needs evidence |
| Articles | 33 | Draft |
| Questions | 117 | Draft |
| Question authoring holds | 124 | no student-facing record authored |
...
- Governed prompt observations: 5,444 total; 117 authored; **5,327 raw prompt observations remain**, including 124 explicit holds and one source-absent opening-family prompt.
- Governed answer observations: 5,211 total; 117 clean source-keyed prompts authored; **5,094 raw answer observations remain**, including the 124 held printed-key observations; Q32 contributes no answer observation.
...
- Record-level backlog is not asserted as 5,327 unique records: repeated and near-repeated prompts must still be deduplicated during authoring, per the one-question-one-record rule.
...
No MUST content has been uploaded or imported. All thirty-three student-facing articles and all one hundred seventeen questions remain Draft; all one hundred three concepts use the concept schema's non-published `under review` state and `publication_status: needs_evidence`. All 124 held prompts exist only as authoring-ledger dispositions with no student-facing record; Q32 exists only as a source-absent ledger disposition.
