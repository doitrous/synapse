...
| Kind | Created | Status |
|---|---:|---|
| Evidence resources | 25 | local-only source records; Q24–Q31 lecture records retain explicit bounded visual-read pages |
| Claims | 129 | verified against local curriculum citations; independent review still owed |
| Citations | 134 | local curriculum |
| Article spans | 129 | linked to the clean-key claims |
| Concepts | 113 | under review / needs evidence |
| Articles | 36 | Draft |
| Questions | 129 | Draft |
| Question authoring holds | 142 | no student-facing record authored |
...
- Governed prompt observations: 5,444 total; 129 authored; **5,315 raw prompt observations remain**, including 142 explicit holds and one source-absent opening-family prompt.
- Governed answer observations: 5,211 total; 129 clean source-keyed prompts authored; **5,082 raw answer observations remain**, including the 142 held printed-key observations; Q32 contributes no answer observation.
...
- Record-level backlog is not asserted as 5,315 unique records: repeated and near-repeated prompts must still be deduplicated during authoring, per the one-question-one-record rule.
...
No MUST content has been uploaded or imported. All thirty-six student-facing articles and all one hundred twenty-nine questions remain Draft; all one hundred thirteen concepts use the concept schema's non-published `under review` state and `publication_status: needs_evidence`. All 142 held prompts exist only as authoring-ledger dispositions with no student-facing record; Q32 exists only as a source-absent ledger disposition.
