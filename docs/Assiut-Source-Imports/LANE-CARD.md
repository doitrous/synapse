...
## 6. Commit rhythm
First commit landed within minutes of worktree setup; committed + pushed after each deliverable (S0, S1, S1b+S2+S3 as one commit). Report ≤ 20 lines to the chief-of-staff session, ends `HANDOFF: <branch>@<sha> · awaiting TRIAGE APPROVED`.
...
- **Scanned finals need real OCR budget, not the bulk compilations.** `MPT mcq questions (final 2022).pdf` OCR'd cleanly on pp.1-7 (223-501 words/page) but degraded on pp.8-11 — 59 of 90 questions are triaged but not keyed because the circled-answer glyph didn't survive OCR on those pages; a targeted `render` pass on just the ambiguous pages is the next step, not a full re-OCR.
