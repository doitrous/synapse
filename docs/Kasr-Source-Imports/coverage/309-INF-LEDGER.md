...
| bank Nematoda MCQ (Q1-50 of 67, answer table p.13) | 46 | 4 | 0 | Q51-67 | 67 |
| **bank Trematoda + Cestoda + Nematoda clusters total** | **89** | **8** | **0** | Q51-67 Nematoda | — |
| 309 INF module (Microbiology + Parasitology sub-corpora) | 89 | 8 | 0 | Nematoda Q51-67 p.12-13, Protozoology p.14-20, Entomology p.22 | — |
...
Author the remaining **Nematoda MCQ** (Q51-67, pp.12-13; same answer table p.13) — worth checking Q62 which is an
in-source exact duplicate of the authored Q39 (trichinellosis, same options, both key C) and should be held. Then
**Protozoology** (Q1-onward, pp.14-20; new answer table ~p.21, in a new `question/309-INF-protozoa-mcq.md` and a new
`ART-INF-309-PROTOZOA` article cross-linked to `ART-INF-309-PARA-GENERAL`) and **Entomology** (answers p.23). Recover
each section's keys from its own printed answer table before authoring.
