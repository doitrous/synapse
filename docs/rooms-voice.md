...
| `SFU_RTC_MAX_PORT` | `49999` | Top of it. Must match the published port mapping. |
...
transport, each binding UDP and TCP. The 10 000-port range is about 2 500
...
**The range lives in three places that must always agree:**

1. `SFU_RTC_MIN_PORT` / `SFU_RTC_MAX_PORT` (the app's env, in Coolify)
2. the app's **Ports Mappings** in Coolify —
   `40000-49999:40000-49999/udp,40000-49999:40000-49999/tcp`
...
and their audio silently fails. All three were set to `40000–49999` on
2026-09-11. A peer is capped at two transports; asking for a third replaces the
one facing the same direction, so a member cannot take the range by looping
`sfu:createTransport`.
