...
- 2026-08-28 (UI · clinical-case vitals verification): verified the `Vitals` strip renders on a live case in the demo build (branch `claude/dashboard-practical-ui-b03087`, now on `main`). Opened the seeded **Post-partum breathlessness** case (`cc-postpartum`, vitals lifted verbatim from its own stage-1 prose) in the case runner. The **Observations** card renders in the runner sidebar above **Case path** with note `room air` and HR 118 bpm · BP 108/68 mmHg · RR 30/min · SpO₂ 91% · Temp 37.4°C — correct values, units, and note. Abnormal flagging confirmed by computed `color`: HR / RR / SpO₂ = `rgb(168,18,30)` (`--color-danger`, red), BP / Temp = `rgb(22,25,32)` (`--color-ink`, neutral) — exactly the authored `abnormal: ['hr','rr','spo2']`. Verification method: the Browser pane would not composite frames in this environment (screenshots timed out), so confirmation was DOM-based (`get_page_text` + `getComputedStyle` on the `dt`/`dd` cells) rather than a screenshot — a more precise check of the flag colours than an image. Test-only caveat: the demo onboarding gate kept blocking, so the enrolment audience was seeded directly into `localStorage` (`nishany.account.audience.v1` = `{universityId:'kau', year:'1', group:''}`) to reach the app; no code change, demo-only. Full suite was already green at ship (1532 pass, tsc clean, build ✓).
- 2026-08-28 (ASU-IBM staging, finishing a salvaged pass): resumed `staging/asu-ibm-verify` (already had biochem-mcq bundle from `asu-ibm-author13` staged + article reviewer/publisher fixed) on branch `staging/asu-ibm-verify2`, merged current `main`. Confirmed all 9 canonical files (1 resource, 7 articles, 20 concepts, evidence, 3 question files totalling 168 records) were already staged verbatim and identical between `docs/Ain-Shams-Source-Imports/` and `docs/import-ready/`. Re-derived the lost uncommitted concept fix: all 20 concepts still carried `reviewer`/`final_publisher: Dr. Omar` — fixed to the standing ruling in both roots. Found and closed a real two-sided `relatedArticleIds` gap (2/7 articles, 18/20 concepts, same shape as the INF fix) with genuine content-verified links, not fabricated. Found and fixed a real eviction hazard in both pending-live overlay files (`module_subject`/`exam_signal` restated as ASU-IBM-only, dropping the pre-existing live Kasr/AU path and evidence lines on every one of 10 article + 28 concept rows) plus one literal duplicate row with a fabricated summary — all fixed and re-verified against the real Kasr/Alexandria dependency files via positional `medical:simulate` (`created:0, updated:10/28, errors:[]` both). Whole-tree `medical:simulate` (315 files): 0 regressions, only the known 29 AU-105 image-gap lines. `medical:audit`, `duplicate-keys`, `validate:authoring`/`:taxonomy` all clean. Wrote `docs/Ain-Shams-Source-Imports/pending-live/INDEX.md` (did not exist) and the ASU-IBM apply-order section in `docs/import-ready/INDEX.md`. Pushed gate-clean to `main`.
