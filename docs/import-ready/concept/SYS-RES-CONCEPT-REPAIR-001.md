# Item
## id
CON-RES-4D4B125FF50E60
## label
Acute laryngitis
## subject
resp
## status
under review
## definition
**Acute laryngitis** is an acute catarrhal inflammation of the mucosa lining the larynx.

Because the vocal folds are mucosal structures, swelling of that lining alters the voice — which is what __localises the inflammation to the larynx__.
## explicit_objective
Define acute laryngitis by the tissue involved, and explain why hoarseness localises inflammation to the larynx.
## original_wording
ACUTE LARYNGITIS: Definition: Acute catarrhal inflammation of laryngeal mucosa
## field_notes
repair: the original was a heading followed by the word "Definition:" and a fragment. It has been rewritten as a sentence. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-7B27CD2545F4A5
## label
Laryngeal papilloma and HPV 6/11
## subject
resp
## status
under review
## definition
**Laryngeal papillomas** are benign epithelial tumours caused by **human papillomavirus**, principally __types 6 and 11__.

The virus persists in adjacent, clinically normal mucosa, so the lesions **recur after removal** — the basis of recurrent respiratory papillomatosis in children.
## explicit_objective
Identify the virus responsible for laryngeal papillomatosis and explain why the lesions recur after excision.
## original_wording
A benign epithelial lesion, caused by human papilloma virus (HPV 6 &
## field_notes
repair: the original was truncated mid-parenthesis after "HPV 6 &". The completion to types 6 and 11 follows the standard association for laryngeal papillomatosis; the lesion has also been named, as the original gave only "a benign epithelial lesion" with no site. Both completions are inferences from the surviving text and standard pathology rather than from the source document.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-A7AE91E130076E
## label
Alveolar contents in acute bacterial pneumonia
## subject
resp
## status
under review
## definition
In consolidated lung the alveolar spaces are packed with **neutrophils** and **red blood cells** within a **fibrin exudate**.

That __replacement of alveolar air by cellular exudate__ is what makes the lung solid and radiodense.
## explicit_objective
Describe the alveolar contents in acute bacterial pneumonia and relate them to the radiographic appearance of consolidation.
## original_wording
Acute inflammatory cells & RBC Filling alveolar spaces
## field_notes
repair: the original was a figure caption, not a statement — a noun phrase with no verb and no disease named. It has been rewritten as a sentence and the setting named as acute bacterial pneumonia, which is the condition in which this appearance is described. The naming of the disease is an inference from the described appearance rather than from the source document.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-5BFE7F3DEA6061
## label
Nodes in acute cervical lymphadenitis
## subject
resp
## status
under review
## definition
Acutely inflamed cervical nodes enlarge but remain **discrete and mobile**, and are __soft, painful and tender__.

This contrasts with **malignant infiltration**, in which nodes are hard, matted, fixed and characteristically painless.
## explicit_objective
Describe the character of acutely inflamed cervical nodes and distinguish them from nodes infiltrated by malignancy.
## original_wording
Acute cervical lymphadenitis: Cervical lymph nodes on both sides are acute inflammatory cells. enlarged, separate, mobile, soft, painful and tender
## field_notes
repair: the original was internally corrupted — the clause "Cervical lymph nodes on both sides are acute inflammatory cells." is a histological caption fragment spliced into a sentence describing clinical signs, leaving the adjective list stranded after a full stop. The clinical description has been retained and the intruded fragment removed. The word "bilateral" from the original has not been carried into the label, since bilaterality is not a defining feature of the acute pattern; no other fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-A6F61F4C0A2D64
## label
Centriacinar vs panacinar emphysema: gross appearance
## subject
resp
## status
under review
## definition
**Centriacinar emphysema** destroys the region around the respiratory bronchioles rather than the whole acinus, so its gross appearance is __less marked than that of panacinar disease__.

The lungs are **less voluminous** and **less pale**, and overlap the heart less at post-mortem.
## explicit_objective
Distinguish centriacinar from panacinar emphysema by the naked-eye appearance of the lungs and by the part of the acinus destroyed.
## original_wording
- In centriacinar emphysema the lungs are less pale and less obscure the heart
## field_notes
repair: the original opened with a stray list dash and was ungrammatical — "less obscure the heart" rather than "obscure the heart less" — and stated a comparison without naming what was being compared with. The comparator has been supplied as panacinar emphysema, which is the pattern this appearance is conventionally contrasted with, and the underlying reason added to the definition. The comparator is an inference from the surviving text and standard pathology.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-A93F9D04787B4C
## label
Exudative pharyngitis: streptococci and adenovirus
## subject
resp
## status
under review
## definition
Marked **hyperaemia** of the pharynx with a visible **exudate** is characteristically associated with **beta-haemolytic streptococcal** and **adenoviral** infection.

Because two different organisms produce the same appearance, __the appearance alone cannot establish which is responsible__.
## explicit_objective
Recognise the exudative pattern of pharyngitis and explain why appearance alone does not identify the organism.
## original_wording
A more severe form is associated with marked hyperemia and exudate occurs with B hemolytic streptococcal and adenovirus infection
## field_notes
repair: the original had two main verbs spliced together — "is associated with… occurs with…" — and referred to "a more severe form" without naming the condition. It has been rewritten as one sentence and the condition named as pharyngitis, which the surrounding concepts establish. The abbreviation "B hemolytic" has been expanded to beta-haemolytic.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-EA9A6089817998
## label
Lung abscess: why right-sided
## subject
resp
## status
under review
## definition
Aspiration-related **lung abscesses** are usually solitary and more often __right-sided__.

The **right main bronchus** is wider, shorter and more vertical than the left, so aspirated material passes into it preferentially.
## explicit_objective
Predict the side of an aspiration lung abscess from the anatomy of the main bronchi.
## original_wording
A single abscess develops, more common on the right side
## field_notes
repair: the original named no organ and gave no reason. The abscess has been identified as a lung abscess from the article context, and the anatomical explanation added to the definition. The organ is an inference from the article the concept belongs to.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-BECD91B06EA39D
## label
Terminal and respiratory bronchioles: the boundary
## subject
resp
## status
under review
## definition
The **terminal bronchiole** is the last __purely conducting airway__.

The **respiratory bronchiole** immediately distal to it is the first airway to take part in gas exchange — where the conducting zone ends and the respiratory zone begins.
## explicit_objective
Place the terminal and respiratory bronchioles in sequence and identify where the conducting zone ends.
## original_wording
A terminal bronchiole (not shown) is immediately proximal to the respiratory bronchiole
## field_notes
repair: the original carried the parenthesis "(not shown)", a reference to a figure that does not accompany the concept. The parenthesis has been removed and the functional significance of the boundary added to the definition. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-RES-E0B3E37B828F65
## label
How respiratory pathogens injure the airway
## subject
resp
## status
under review
## definition
A common mechanism of respiratory infection is **adherence** of the organism to the respiratory epithelium, followed by __necrosis of those cells__ and an inflammatory reaction to the damage.

That sequence explains the loss of the **mucociliary escalator** during an infection, and the frequency with which **bacterial infection follows a viral one**.
## explicit_objective
Describe the sequence by which a respiratory pathogen injures the airway lining, and relate it to impaired clearance during infection.
## original_wording
A common mechanism is attachment of the organism to the respiratory epithelium, followed by necrosis of the cells and inflammatory response
## field_notes
repair: the statement was intact and correct; only the opening "A common mechanism is" lacked a subject. The subject has been named and the consequence added to the definition. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.
