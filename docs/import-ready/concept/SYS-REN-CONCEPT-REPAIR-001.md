# Item
## id
CON-REN-D823DE43FBEFAA
## label
Focal proliferative glomerulonephritis
## subject
renal
## status
under review
## definition
In **focal proliferative glomerulonephritis** only a proportion of glomeruli are involved, and within an affected glomerulus only part of the tuft shows proliferation.

__Focal__ describes how many glomeruli are affected; __segmental__ describes how much of each.
## explicit_objective
Distinguish focal from diffuse and segmental from global when describing a pattern of glomerular injury.
## original_wording
¡ FOCAL PROLIFERATIVE GN: Some glomeruli show segmental proliferations.
## field_notes
repair: the original label opened with a stray bullet glyph and a heading in capitals. The statement itself was intact and correct; it has been rewritten as a sentence and the two terms it depends on — focal and segmental — made explicit. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-0E170769834796
## label
Membranous nephropathy: diffuse GBM thickening
## subject
renal
## status
under review
## definition
In **membranous nephropathy** the glomerular basement membrane is thickened __diffusely and uniformly__, affecting all glomeruli and the whole of each capillary loop.

That uniformity is what distinguishes it from the **segmental** changes of other patterns.
## explicit_objective
Recognise the uniform, diffuse basement-membrane thickening of membranous nephropathy and contrast it with focal or segmental patterns.
## original_wording
(4.8) Membranous Nephropathy showing diffuse uniform thickening of the GBM.
## field_notes
repair: the original was a figure caption, carrying the figure number "(4.8)" and the participle "showing". It has been rewritten as a statement and the abbreviation GBM expanded. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-95F19D57F25A2B
## label
Papillary bladder carcinoma (about 80%)
## subject
renal
## status
under review
## definition
The great majority of bladder carcinomas — about **80%** — are **papillary tumours**, each frond consisting of a delicate fibrovascular core covered by neoplastic urothelium.

The __papillary architecture__ is what distinguishes them from the flat, non-papillary tumours that make up the remainder.
## explicit_objective
Describe the papillary architecture of the commonest bladder carcinomas and state the approximate proportion they represent.
## original_wording
80% of bladder carcinomas are papillary tumors formed of delicate 70.
## field_notes
repair: the original label was truncated mid-phrase, ending in "delicate 70" — the numeral is a page number that bled into the extracted text. The missing words have been supplied from the standard description of papillary urothelial carcinoma, whose fronds consist of a delicate fibrovascular core covered by urothelium. The completion is an inference from the surviving text and standard pathology, not from the original source document.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-A6AEEA7A18934B
## label
Solid non-papillary bladder carcinoma (about 20%)
## subject
renal
## status
under review
## definition
A minority of bladder carcinomas — about **20%** — are **flat and solid** rather than papillary.

These behave far more aggressively: they are __invasive at the time of diagnosis__ and characteristically infiltrate the **detrusor muscle**.
## explicit_objective
Contrast the flat, non-papillary bladder carcinoma with the papillary type in both architecture and behaviour at presentation.
## original_wording
20% of tumors are solid (flat, non-papillary) and invasive at presentation and infiltrating the muscle.
## field_notes
repair: the statement was intact but began with an unreferenced "tumors". The subject has been named explicitly as bladder carcinoma, which its companion concept establishes. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-64E32F60E90072
## label
Constituents of the glomerular basement membrane
## subject
renal
## status
under review
## definition
The glomerular basement membrane is built from **type IV collagen**, **laminin** and **polyanionic proteoglycans**, of which **heparan sulfate** is the main one.

The __fixed negative charge__ those proteoglycans carry is what makes the barrier selective for charge as well as size.
## explicit_objective
Name the constituents of the glomerular basement membrane and explain how its anionic charge contributes to filtration selectivity.
## original_wording
A glomerular basement membrane (GBM) consisting of collagen (mostly type IV), laminin, polyanionic proteoglycans mostly heparan sulfate.
## field_notes
repair: the original was a sentence fragment beginning "A glomerular basement membrane consisting of…", with no main verb. It has been completed as a statement. The functional consequence of the anionic charge has been added to the definition, not to the label. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-2926B6DCBB2134
## label
Acid urine favours uric acid and cystine stones
## subject
renal
## status
under review
## definition
**Uric acid** and **cystine** are both less soluble in acid urine, so a persistently __low urine pH__ favours their crystallisation.

This is why **alkalinising the urine** is used to prevent and dissolve stones of these two types — and why it does not help with calcium stones.
## explicit_objective
Relate urine pH to the solubility of stone-forming solutes, and identify which stone types are favoured by acid urine.
## original_wording
Acidity (decreased urine pH) leads to uric acid and cystine stones formation.
## field_notes
repair: the statement was intact but ungrammatical. It has been rewritten as a sentence, and the therapeutic consequence added to the definition. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-A33EEFD4C4A862
## label
Metabolic acidosis of renal failure
## subject
renal
## status
under review
## definition
As glomerular filtration falls, the acid anions produced by normal metabolism — **phosphates, sulphates and organic acids** — are __retained instead of being excreted__.

Their accumulation, together with **impaired renal generation of bicarbonate**, produces the metabolic acidosis of renal failure.
## explicit_objective
Explain the metabolic acidosis of renal failure from the retention of unexcreted acid anions.
## original_wording
Acidosis due to retention of phosphates, sulphates and organic.
## field_notes
repair: the original was truncated after "organic", omitting the noun. It has been completed as "organic acids", which is the only reading consistent with the preceding list of retained acid anions. The renal setting has been named explicitly, as the original gave no context. Completion inferred from the surviving text and standard physiology rather than from the source document.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-374A36F2DAF367
## label
Acquired bladder diverticula and outflow obstruction
## subject
renal
## status
under review
## definition
**Acquired bladder diverticula** form where mucosa herniates between hypertrophied detrusor bundles, most often as a consequence of __chronic outflow obstruction__ — in men, usually **prostatic enlargement**.

They differ from **congenital diverticula**, which are present from birth and are not obstruction-related.
## explicit_objective
Distinguish acquired from congenital bladder diverticula and name the obstruction that usually causes the acquired form.
## original_wording
Acquired diverticula: Mostly associated with prostatic enlargement.
## field_notes
repair: the original was a heading with a fragment after a colon. It has been rewritten as a statement, and the mechanism and the contrast with congenital diverticula added to the definition. No fact was changed.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.

---

# Item
## id
CON-REN-8A11F1D9204098
## label
Age distribution of bladder carcinoma
## subject
renal
## status
under review
## definition
Bladder carcinoma is predominantly a disease of __later life__, with roughly **four-fifths of patients** presenting between the ages of **50 and 80**.
## explicit_objective
State the age distribution of bladder carcinoma and use it in weighting a differential for visible haematuria.
## original_wording
About 80% of patients are between the ages of 50 and 80 years.
## uncertainty
The original label named no disease — it began "About 80% of patients" with no referent anywhere in the record. Bladder carcinoma has been supplied as the subject because the concept sits among the bladder-carcinoma concepts of the same article and because the stated 50–80 range matches the known age distribution of that tumour. This is an inference about which disease was meant, not merely about wording, and it should be confirmed against the source document before this concept is published.
## field_notes
repair: subject supplied by inference — see the uncertainty field. The percentage and the age range are unchanged from the original.
arabicLabel: no reviewed Arabic terminology has been supplied; the label is not guessed.
