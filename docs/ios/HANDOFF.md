...
3. **Notebook rich-text editor — Phase 2** (Phase 1 shipped, see above; this is the "rich-text editor" Omar asked for). Native rich editing on top of the now-lossless model: a `UITextView`-backed editor + toolbar for **bold/italic/underline/strikethrough** (map to Lexical text-node `format` bitfields — bold=1, italic=2, strikethrough=4, underline=8) serialising paragraph + text-run `editorJson`, then block types (heading h1–h4, quote, bullet/numbered/check lists). **Never flatten node types it does not model** (tables/images/HR) — preserve them opaquely; Phase-1's `isSimple` banner is the interim guard. Web reference: `src/components/notebook/NoteEditor.tsx` (Lexical). Also still open here: the Resources `ResourceFileStore.swift:61` video→`.pdf` bug (route `.video` to an external open, not the PDF reader — blind build, empty catalogue on this cohort).
