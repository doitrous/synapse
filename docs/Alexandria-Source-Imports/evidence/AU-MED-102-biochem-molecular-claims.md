<!--
  AU-MED-102 Biochemistry sub-lane D — one claim per newly minted concept, supporting the
  concept and the article's callout/annotation content. concept_id values must already
  exist (they are minted in concept/AU-MED-102-biochem-molecular-concepts.md, same lane).
-->

# Item

## id
CLM-FND-92DD65D96E3FA1-01

## concept_id
CON-FND-92DD65D96E3FA1

## subject
Purine catabolism

## predicate
ends at

## object
uric acid in humans, since humans lack uricase

## display_text
In humans, purine catabolism ends at uric acid because humans lack uricase.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-7302601EA492D2-01

## concept_id
CON-FND-7302601EA492D2

## subject
DNA replication fidelity

## predicate
rests on

## object
complementary base pairing and DNA polymerase's proofreading

## display_text
DNA replication fidelity is ensured by complementary base pairing together with the specificity and proofreading of DNA polymerase.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-73C77966B56FED-01

## concept_id
CON-FND-73C77966B56FED

## subject
Telomerase

## predicate
protects

## object
DNA from the shortening that would otherwise occur with ageing

## display_text
Telomerase is the enzyme that protects DNA from the shortening associated with ageing.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-F1E54D68C8FAB0-01

## concept_id
CON-FND-F1E54D68C8FAB0

## subject
RNA

## predicate
is distinguished from DNA by

## object
uracil instead of thymine and single-strandedness

## display_text
RNA is distinguished from DNA by carrying uracil in place of thymine and by being single-stranded.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-5FF8EB2DB4D662-01

## concept_id
CON-FND-5FF8EB2DB4D662

## subject
mRNA processing

## predicate
consists of

## object
5' capping, poly-A tailing and splicing, distinct from nuclear export

## display_text
mRNA processing consists of 5' capping, poly-A tail attachment and removal of introns, and does not include transfer of mRNA into the nucleus.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-A1B0BFB9626438-01

## concept_id
CON-FND-A1B0BFB9626438

## subject
The genetic code

## predicate
is

## object
degenerate, unambiguous, non-overlapping and universal

## display_text
The genetic code is degenerate, non-overlapping and universal, and is not ambiguous.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-E8CD7F7F690B14-01

## concept_id
CON-FND-E8CD7F7F690B14

## subject
Xeroderma pigmentosum

## predicate
results from a defect in

## object
nucleotide excision (DNA) repair

## display_text
Xeroderma pigmentosum results from a defect in DNA repair.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-8E4A3DB9BC03AC-01

## concept_id
CON-FND-8E4A3DB9BC03AC

## subject
The order of nucleotides in a DNA strand

## predicate
is written

## object
from the 5' to the 3' direction

## display_text
The order of nucleotides in a DNA strand is always written from the 5' to the 3' direction.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-5B8E3AAFEB6C35-01

## concept_id
CON-FND-5B8E3AAFEB6C35

## subject
DNA replication

## predicate
is characterised as

## object
semiconservative, requiring DNA polymerases, proceeding 5' to 3'

## display_text
Replication is semiconservative and requires DNA polymerases; it does not occur in the 3' to 5' direction.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-412F3EDF118F44-01

## concept_id
CON-FND-412F3EDF118F44

## subject
RNA polymerase I

## predicate
transcribes

## object
the genes of the large ribosomal RNAs (5.8S, 18S, 28S rRNA)

## display_text
RNA polymerase I transcribes the genes of 5.8S, 18S and 28S ribosomal RNA.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-DF5E3014A149FC-01

## concept_id
CON-FND-DF5E3014A149FC

## subject
A transition point mutation

## predicate
is

## object
a purine-to-purine or pyrimidine-to-pyrimidine base substitution

## display_text
A transition point mutation is a purine-to-purine or pyrimidine-to-pyrimidine base substitution, for example guanine replaced by adenine.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-914D9DDFB56AD1-01

## concept_id
CON-FND-914D9DDFB56AD1

## subject
A nucleotide

## predicate
is converted to a nucleoside by

## object
removal of its phosphate group

## display_text
A nucleotide is converted into a nucleoside by removal of the phosphate group.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-CA2D65E688434A-01

## concept_id
CON-FND-CA2D65E688434A

## subject
The acceptor arm of tRNA

## predicate
terminates at its 3' OH end with

## object
the sequence 5'-CCA-3'

## display_text
The acceptor arm of tRNA terminates at its 3' OH end with the sequence CCA.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-09FACBDCBBF8FD-01

## concept_id
CON-FND-09FACBDCBBF8FD

## subject
AUG

## predicate
is

## object
the mRNA start codon in most cases, recognised by a methionyl-tRNA complex

## display_text
AUG is the mRNA start codon in most cases, and translation initiation in eukaryotes forms a tRNA complex charged with methionine.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-252B3C77D181DA-01

## concept_id
CON-FND-252B3C77D181DA

## subject
DNA ligase

## predicate
joins

## object
the Okazaki fragments of the lagging strand

## display_text
The function of DNA ligase is to join the Okazaki fragments.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-D717E6E7EEA466-01

## concept_id
CON-FND-D717E6E7EEA466

## subject
The coding strand of DNA

## predicate
differs from its mRNA only in that

## object
mRNA uses uracil instead of thymine

## display_text
The coding strand of DNA is the same as its associated mRNA except that mRNA uses uracil instead of thymine.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-906B844C9AEE7D-01

## concept_id
CON-FND-906B844C9AEE7D

## subject
The anticodon

## predicate
is located on

## object
tRNA and pairs with the codon on mRNA

## display_text
The anticodon is a set of three nucleotides on tRNA that corresponds to the codon on mRNA.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-FFEE58EC9C0784-01

## concept_id
CON-FND-FFEE58EC9C0784

## subject
Topoisomerase

## predicate
relieves

## object
the supercoil created by helicase's unwinding of the parental duplex

## display_text
Topoisomerase relieves the supercoil on the parental duplex of DNA caused by unwinding during synthesis.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-31C41EFEF31740-01

## concept_id
CON-FND-31C41EFEF31740

## subject
Chargaff's rule

## predicate
states that

## object
adenine equals thymine and guanine equals cytosine in double-stranded DNA

## display_text
By Chargaff's rule, if cytosine is 20% of total DNA bases, adenine is 30%.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-DB1988D55A69E3-01

## concept_id
CON-FND-DB1988D55A69E3

## subject
PCR

## predicate
requires

## object
two DNA primers and a thermostable DNA polymerase, run in vitro

## display_text
PCR is an in vitro technique for DNA amplification requiring two types of DNA primers and a thermostable DNA polymerase.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-A1FC2FAF9F0211-01

## concept_id
CON-FND-A1FC2FAF9F0211

## subject
Cystine

## predicate
has no direct codon because

## object
it is formed after translation from two cysteine residues

## display_text
Cystine is one of the amino acids that does not have a specific code on DNA, because it is formed post-translationally.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-8C5B1666F4F7C1-01

## concept_id
CON-FND-8C5B1666F4F7C1

## subject
Phosphorylation of a protein

## predicate
can

## object
either increase or decrease its activity

## display_text
Phosphorylation of a protein can either increase or decrease its activity, and can occur on serine, threonine or tyrosine residues.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-13FCDB652CB258-01

## concept_id
CON-FND-13FCDB652CB258

## subject
A target cell for a hormone

## predicate
is defined by

## object
expressing a receptor specific to that hormone

## display_text
A cell is defined as a target for a hormone by having a receptor specific to that hormone.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-22F8C729D1E8B0-01

## concept_id
CON-FND-22F8C729D1E8B0

## subject
Synaptic transmission across the synaptic cleft

## predicate
is classified as

## object
paracrine signalling

## display_text
Transmission of a nerve impulse through the synaptic cleft is classified as paracrine signalling.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-1D57FC5C8BFF90-01

## concept_id
CON-FND-1D57FC5C8BFF90

## subject
A typical intracellular receptor

## predicate
has about

## object
two domains, fewer than a typical cell-membrane receptor

## display_text
A typical intracellular receptor is described as having two domains; a typical cell-membrane receptor is described with three.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-D10E79C01B3345-01

## concept_id
CON-FND-D10E79C01B3345

## subject
Phosphodiesterase

## predicate
reverses the action of

## object
adenylate cyclase, degrading the cAMP it makes

## display_text
Phosphodiesterase reverses the action of adenylate cyclase, which is how the cAMP second messenger is degraded once it has signalled.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-22D1D9B77A768E-01

## concept_id
CON-FND-22D1D9B77A768E

## subject
Cytosine

## predicate
is a

## object
pyrimidine base, 2-oxy-4-amino, present in both DNA and RNA

## display_text
Cytosine is a pyrimidine base (2-oxy-4-amino) found in both DNA and RNA, unlike thymine or uracil which each belong to only one.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-38857DFD506559-01

## concept_id
CON-FND-38857DFD506559

## subject
Peptidyl transferase

## predicate
is triggered by

## object
release factors occupying the A site at a stop codon

## display_text
Peptidyl transferase, an activity of the 60S ribosomal subunit, is triggered to hydrolyse the peptide-tRNA bond by release factors occupying the A site.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-FE41A702648A00-01

## concept_id
CON-FND-FE41A702648A00

## subject
An enhancer

## predicate
is

## object
a regulatory DNA sequence that need not sit at a fixed location relative to the gene it regulates

## display_text
An enhancer does not need to be in a fixed location and is most responsible for high transcription rates of particular genes.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-874F418DFB12AF-01

## concept_id
CON-FND-874F418DFB12AF

## subject
Reverse transcriptase

## predicate
is required for

## object
synthesising single-stranded DNA from an RNA template

## display_text
A reverse transcriptase must be present for the formation of single-stranded DNA from RNA.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-21B283A2FE53C4-01

## concept_id
CON-FND-21B283A2FE53C4

## subject
Deamination of cytosine

## predicate
produces

## object
a point mutation (uracil) if not repaired

## display_text
Deamination of cytosine, if not repaired, produces a point mutation.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-55348CAA8F9C16-01

## concept_id
CON-FND-55348CAA8F9C16

## subject
The primosome

## predicate
introduces

## object
positive supercoils into DNA

## display_text
The primosome is credited with introducing positive supercoils into DNA.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-FE0A89860189A5-01

## concept_id
CON-FND-FE0A89860189A5

## subject
RNA polymerase

## predicate
adds nucleotides

## object
only at the 3' end of the growing chain, without a primer

## display_text
RNA polymerase adds nucleotides at the 3' end of the growing polynucleotide chain.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-6EF5C044E64C46-01

## concept_id
CON-FND-6EF5C044E64C46

## subject
Single-strand binding protein

## predicate
keeps

## object
the separated DNA strands from re-annealing

## display_text
Single-strand binding protein binds single-stranded DNA to keep the two strands from binding together again.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-7CEF0F2F8C9EE1-01

## concept_id
CON-FND-7CEF0F2F8C9EE1

## subject
Insertion or deletion of bases

## predicate
causes

## object
a frameshift mutation

## display_text
Deletion or insertion of bases causes a frameshift mutation.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-9D5D6275474035-01

## concept_id
CON-FND-9D5D6275474035

## subject
Decapping

## predicate
shuts down

## object
translation of a processed mRNA

## display_text
Decapping may shut down the translation of a processed mRNA.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-60F505DFC88026-01

## concept_id
CON-FND-60F505DFC88026

## subject
Translocation

## predicate
moves

## object
the peptide-bearing tRNA from the ribosomal A site to the P site

## display_text
Translocation moves the ribosome so the tRNA carrying the growing polypeptide chain shifts from the A site to the P site, freeing the A site.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-B92EB41248D631-01

## concept_id
CON-FND-B92EB41248D631

## subject
eIF-1 and eIF-3

## predicate
are required for

## object
binding of mRNA to the 40S ribosomal subunit

## display_text
eIF-1 and eIF-3 in protein synthesis are required for binding of mRNA to the 40S ribosomal subunit.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-D6A7B168134B07-01

## concept_id
CON-FND-D6A7B168134B07

## subject
Tetracycline

## predicate
prevents

## object
tRNA attachment to the ribosomal A site

## display_text
Tetracycline prevents tRNA from attaching to the A site of the ribosome.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-36FA9827A6B99C-01

## concept_id
CON-FND-36FA9827A6B99C

## subject
Base excision repair

## predicate
removes

## object
uracil and incorporates the correct base

## display_text
Base excision repair is the mechanism that removes uracil and incorporates the correct base.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-014D200ED96498-01

## concept_id
CON-FND-014D200ED96498

## subject
Ciprofloxacin

## predicate
inhibits

## object
bacterial DNA gyrase, a topoisomerase

## display_text
Ciprofloxacin cures a bacterial infection by inhibiting bacterial DNA gyrase, a topoisomerase.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-32433CE637CA9A-01

## concept_id
CON-FND-32433CE637CA9A

## subject
RNA editing

## predicate
produces

## object
a different protein from the same gene, unlike alternative splicing

## display_text
The main function of RNA editing is to synthesise a different protein from the same gene.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)

---

# Item

## id
CLM-FND-A427EA66958B81-01

## concept_id
CON-FND-A427EA66958B81

## subject
A mature red blood cell

## predicate
cannot synthesise new mRNA because

## object
gene loss (losing its nucleus) during erythropoiesis

## display_text
According to the regulation of gene expression, mature red blood cells cannot form mRNA because of gene loss.

## risk_class
foundational_stable

## verification_status
needs_evidence

## conflict_status
none

## confidence
0.7

## freshness
stable_local_curriculum_fact

## time_sensitive
no

## qualifiers
population: AU-MED-102 Year 1 (Biochemistry)
