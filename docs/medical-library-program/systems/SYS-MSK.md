# SYS-MSK — Musculoskeletal System

**System 9 of 19** in the locked execution order (LD-09). Previous: [SYS-NEU](SYS-NEU.md). Next: [SYS-HEM](SYS-HEM.md).
...
| **Active task** | `SYS-MSK-INVENTORY-001` (Not started) |
...
**In scope.** Regional anatomy and biomechanics, musculoskeletal presentations, trauma and orthopaedics, arthritis and rheumatology, bone and metabolic disease, and musculoskeletal assessment/rehabilitation/procedures.

**Out of scope.** MSK drug classes including gout and osteoporosis drugs (DIS-PHA, per the 2026-08-12 de-duplication).

**Learner years.** Years 1–2 regional anatomy and biomechanics; Years 3–4 trauma, rheumatology and assessment

**Prerequisites.** SYS-FND, DIS-ANA (extensive), DIS-HIS, DIS-PHY.

**Cross-system dependencies.** SYS-END (bone metabolism — see the SYS-END overlap note), SYS-IMM (autoimmune arthritis), SYS-NEU (peripheral nerve, neuromuscular junction), SYS-MUL (trauma, fractures in the polytrauma patient), SYS-DER (connective-tissue disease with skin signs).

**Runtime tree.** Runtime subject `msk` — 10 topics, fully crosswalked. `Musculoskeletal skills` was renamed in the 2026-08-12 de-duplication.
...
| Subtopics | 13 |
| Microtopics | 47 |
| **Total descendants** | **66** |

| Topic ID | Title | Template | Subtopics | Microtopics | Initial classification |
|---|---|---|---:|---:|---|
| `SYS-MSK-T01` | Regional anatomy and biomechanics | `TPL-CONCEPT` | 3 | 14 | Hub (confirm in inventory) |
| `SYS-MSK-T02` | Musculoskeletal presentations | `TPL-PRESENTATION` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-MSK-T03` | Trauma and orthopedics | `TPL-EMERGENCY` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-MSK-T04` | Arthritis and rheumatology | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-MSK-T05` | Bone and metabolic disease | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-MSK-T06` | Musculoskeletal assessment, rehabilitation and procedures | `TPL-SKILL` | 2 | 7 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (66 nodes)</summary>

- `SYS-MSK-T01` **Regional anatomy and biomechanics** *(TPL-CONCEPT)*
  - `SYS-MSK-T01-S01` **Upper limb** — `Shoulder` · `Arm` · `Elbow` · `Forearm` · `Hand`
  - `SYS-MSK-T01-S02` **Lower limb** — `Hip` · `Thigh` · `Knee` · `Leg` · `Ankle` · `Foot`
  - `SYS-MSK-T01-S03` **Spine and axial skeleton** — `Vertebral column` · `Back` · `Pelvis`
- `SYS-MSK-T02` **Musculoskeletal presentations** *(TPL-PRESENTATION)*
  - `SYS-MSK-T02-S01` **Pain and swelling** — `Monoarthritis` · `Polyarthritis` · `Back pain` · `Limb pain`
  - `SYS-MSK-T02-S02` **Functional problems** — `Weakness` · `Gait disorder` · `Reduced range of motion`
- `SYS-MSK-T03` **Trauma and orthopedics** *(TPL-EMERGENCY)*
  - `SYS-MSK-T03-S01` **Fractures and dislocations** — `Upper limb` · `Lower limb` · `Spine`
  - `SYS-MSK-T03-S02` **Soft-tissue injury** — `Tendon` · `Ligament` · `Muscle` · `Compartment syndrome`
- `SYS-MSK-T04` **Arthritis and rheumatology** *(TPL-CONDITION)*
  - `SYS-MSK-T04-S01` **Inflammatory arthritis** — `Rheumatoid` · `Spondyloarthritis` · `Juvenile arthritis`
  - `SYS-MSK-T04-S02` **Crystal and degenerative disease** — `Gout` · `Pseudogout` · `Osteoarthritis`
- `SYS-MSK-T05` **Bone and metabolic disease** *(TPL-CONDITION)*
  - `SYS-MSK-T05-S01` **Metabolic bone disease** — `Osteoporosis` · `Osteomalacia` · `Paget disease`
  - `SYS-MSK-T05-S02` **Infection and tumors** — `Osteomyelitis` · `Septic arthritis` · `Bone tumors`
- `SYS-MSK-T06` **Musculoskeletal assessment, rehabilitation and procedures** *(TPL-SKILL)*
  - `SYS-MSK-T06-S01` **Examination and imaging** — `Joint examination` · `X-ray` · `MRI`
  - `SYS-MSK-T06-S02` **Management skills** — `Casting` · `Splinting` · `Physiotherapy` · `Joint aspiration`

</details>

### Decided classification (`SYS-MSK-INVENTORY-001`, 2026-08-12)

All 66 nodes are classified. Evidence:
[`SYS-MSK-node-classification.json`](../evidence/SYS-MSK-node-classification.json).

| Classification | Nodes |
|---|---:|
| navigation-only hub | 18 |
| overview article | 1 |
| atomic article home | 47 |
...
*Initial classification is a placeholder. `SYS-MSK-INVENTORY-001` replaces every
...
|---|---|---|
| `Musculoskeletal system` (By system) | `already covered under a different Synapse label` | Direct match. |
| `Surgery → Orthopedics` | `secondary placement or cross-reference` |  |
| `Internal medicine → Rheumatology` | `secondary placement or cross-reference` |  |
...
SYS-MSK-T01 `Regional anatomy and biomechanics` has 3 subtopics and 17 descendants — the largest anatomy cluster in the system route. Confirm in Phase 1 whether upper limb / lower limb / back deserve promotion to sibling topics, given the runtime tree already treats them as three separate topics. This is a `useful rename or split` candidate sourced from the runtime tree, not from AMBOSS.
...
No canonical article or concept. 9 articles and 123 concepts secondary, dominated by DIS-ANA. 1 article `Published` (`ART-MSK-TOP-C25BAA2467`), affected by BLK-06/BLK-07.

**Measured baseline for `SYS-MSK`:**
...
|---|---|---|---|---|---|
| — | *No article has `SYS-MSK` as its canonical home* | — | — | — | Every article in this system is new, or is a `move of canonical home` decision on a discipline-rooted record (DEC-005) |

### Decided dispositions (`SYS-MSK-INVENTORY-001`, 2026-08-12)

Evidence: [`SYS-MSK-disposition-ledger.json`](../evidence/SYS-MSK-disposition-ledger.json).

| | Articles | Concepts |
|---|---:|---:|
| Touching this system | 9 | 123 |
...
`SYS-MSK-INVENTORY-001` must count how many of them belong to this system and
...
| Atomic articles | 33–61 | ≈0.7–1.3 per microtopic: some microtopics merge into one studiable unit, others split |
| **Total articles** | **35–67** | |

These are ranges derived from node counts, not commitments. `SYS-MSK-INVENTORY-001`
...
**Baseline to improve on honestly:** 123 concepts currently touch this
...
having):** SYS-END (bone metabolism — see the SYS-END overlap note), SYS-IMM (autoimmune arthritis), SYS-NEU (peripheral nerve, neuromuscular junction), SYS-MUL (trauma, fractures in the polytrauma patient), SYS-DER (connective-tissue disease with skin signs).
...
- Fracture management and reduction — procedure content, faculty review.
- DMARD and biologic therapy — dosing and monitoring; faculty review.
- Septic arthritis — emergency, time-critical diagnosis.
- Local anaesthetic and joint injection technique — procedure.
...
- Regional anatomy plates for each compartment (upper limb, lower limb, back) — the bulk of the requirement.
- Joint cross-section and ligament diagrams.
- Brachial and lumbosacral plexus diagrams — pure spatial content.
- Sarcomere and excitation–contraction coupling sequence diagram.
- Fracture pattern classification comparison plate.
- Radiograph examples per common fracture — rights-sensitive.
- Gait cycle diagram.
...
Moderate Qbank density. **Very high OSCE relevance** — MSK examination stations (shoulder, knee, hip, spine, GALS) are standard, and SYS-MSK-T06 is explicitly a `TPL-SKILL` topic. Record substantial practical targets.
...
|---:|---|---|---|---|
| 1 | `SYS-MSK-INVENTORY-001` | Classify every one of the 66 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-MSK-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-MSK-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-MSK-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-MSK-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-MSK-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-MSK-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-MSK-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-MSK` | System completion | — | tasks 1–8 |
...
- Anatomy plate rights are the binding constraint. Plan for openly licensed sources or commissioned diagrams; do not plan articles that assume unlicensed atlas images.

**Completion criteria (`GATE-SYS-MSK`).** All 66
...
> **`SYS-MSK-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 66 `SYS-MSK` nodes
  ...
  every one of the 9 articles and 123 concepts that
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-MSK-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-MSK-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-MSK-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-MSK-SOURCE-001`.
