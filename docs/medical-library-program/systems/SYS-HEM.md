# SYS-HEM — Blood & Lymphoreticular System

**System 10 of 19** in the locked execution order (LD-09). Previous: [SYS-MSK](SYS-MSK.md). Next: [SYS-IMM](SYS-IMM.md).
...
| **Active task** | `SYS-HEM-INVENTORY-001` (Not started) |
...
**In scope.** Haematopoiesis and blood science, anaemia and red-cell disorders, haemostasis and thrombosis, white-cell disorders, lymphoid and plasma-cell disease, and transfusion medicine.

**Out of scope.** Anticoagulant and antiplatelet drug classes (DIS-PHA). Anaemia of chronic kidney disease is canonically SYS-HEM with SYS-REN secondary.

**Learner years.** Years 1–2 haematopoiesis and blood science; Years 3–4 disease and transfusion

**Prerequisites.** SYS-FND (cell biology, neoplasia), DIS-HIS, DIS-PHY.

**Cross-system dependencies.** SYS-IMM (lymphoid tissue, immunodeficiency overlap), SYS-CVS (thrombosis, anticoagulation), SYS-INF (malaria, parasitic anaemia — high Egyptian relevance), SYS-OBS (anaemia in pregnancy, rhesus disease), SYS-MUL (massive transfusion, DIC).

**Runtime tree.** No runtime subject maps to SYS-HEM.
...
|---|---|---|---:|---:|---|
| `SYS-HEM-T01` | Hematopoiesis and blood science | `TPL-CONCEPT` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-HEM-T02` | Anemia and red-cell disorders | `TPL-CONCEPT` | 3 | 8 | Hub (confirm in inventory) |
| `SYS-HEM-T03` | Hemostasis and thrombosis | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-HEM-T04` | White-cell disorders | `TPL-CONCEPT` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-HEM-T05` | Lymphoid and plasma-cell disease | `TPL-CONCEPT` | 2 | 4 | Hub (confirm in inventory) |
| `SYS-HEM-T06` | Transfusion medicine | `TPL-CONDITION` | 2 | 7 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (56 nodes)</summary>

- `SYS-HEM-T01` **Hematopoiesis and blood science** *(TPL-CONCEPT)*
  - `SYS-HEM-T01-S01` **Blood cell development** — `Erythropoiesis` · `Leukopoiesis` · `Platelets`
  - `SYS-HEM-T01-S02` **Laboratory foundations** — `CBC` · `Blood film` · `Bone marrow`
- `SYS-HEM-T02` **Anemia and red-cell disorders** *(TPL-CONCEPT)*
  - `SYS-HEM-T02-S01` **Microcytic anemia** — `Iron deficiency` · `Thalassemia` · `Chronic disease`
  - `SYS-HEM-T02-S02` **Normocytic and macrocytic anemia** — `Hemolysis` · `B12 and folate` · `Marrow failure`
  - `SYS-HEM-T02-S03` **Hemoglobin disorders** — `Sickle cell disease` · `Thalassemia`
- `SYS-HEM-T03` **Hemostasis and thrombosis** *(TPL-CONDITION)*
  - `SYS-HEM-T03-S01` **Bleeding disorders** — `Platelet disorders` · `Hemophilia` · `von Willebrand disease`
  - `SYS-HEM-T03-S02` **Thrombotic disorders** — `VTE` · `Thrombophilia` · `DIC`
- `SYS-HEM-T04` **White-cell disorders** *(TPL-CONCEPT)*
  - `SYS-HEM-T04-S01` **Benign leukocyte disorders** — `Neutropenia` · `Leukocytosis` · `Reactive changes`
  - `SYS-HEM-T04-S02` **Leukemia and myeloproliferative disease** — `Acute leukemia` · `Chronic leukemia` · `MPN`
- `SYS-HEM-T05` **Lymphoid and plasma-cell disease** *(TPL-CONCEPT)*
  - `SYS-HEM-T05-S01` **Lymphoma** — `Hodgkin` · `Non-Hodgkin`
  - `SYS-HEM-T05-S02` **Plasma-cell disorders** — `Multiple myeloma` · `Monoclonal gammopathy`
- `SYS-HEM-T06` **Transfusion medicine** *(TPL-CONDITION)*
  - `SYS-HEM-T06-S01` **Blood components** — `Red cells` · `Platelets` · `Plasma` · `Cryoprecipitate`
  - `SYS-HEM-T06-S02` **Transfusion safety** — `Compatibility` · `Reactions` · `Massive transfusion`
...
|---|---|---|
| `Blood and lymphoreticular system` (By system) | `already covered under a different Synapse label` | Direct match. |
| `Internal medicine → Hematology/Oncology` | `useful rename or split` | AMBOSS bundles haematology with oncology. Synapse splits them: blood disease in SYS-HEM, oncology principles in SYS-MUL-T05. Keep the split; record the mapping. |
...
None proposed. Confirm the SYS-HEM / SYS-IMM lymphoid boundary explicitly — lymph node and spleen structure has a legitimate claim in both.
...
**Second-strongest canonical inventory: 7 articles and 94 concepts homed here**, plus 2 articles and 28 concepts secondary. Disposition: `enrich` — these are histology-derived records that need clinical extension, annotations and image recommendations.

**Measured baseline for `SYS-HEM`:**
...
|---|---|---|---|---|---|
| `ART-HEM-TOP-B697DE3AAD` | Anemia Classification | In review | `needs_evidence` | `TPL-CONCEPT` | **enrich** — annotations (0), related-reading projection, image recommendations, archetype-contract completion |
| `ART-HEM-TOP-CC03030BEE` | Coagulation Physiology | In review | `needs_evidence` | `TPL-CONCEPT` | **enrich** — annotations (0), related-reading projection, image recommendations, archetype-contract completion |
| `ART-HEM-TOP-37142FFF1F` | Plasma Proteins | In review | `needs_evidence` | `TPL-CONCEPT` | **enrich** — annotations (0), related-reading projection, image recommendations, archetype-contract completion |
| `ART-HEM-TOP-D3C13B01E3` | Erythrocyte Physiology | In review | `needs_evidence` | `TPL-CONCEPT` | **enrich** — annotations (0), related-reading projection, image recommendations, archetype-contract completion |
...
`SYS-HEM-INVENTORY-001` must count how many of them belong to this system and
...
|---|---|---|
| Navigation-only hubs | 6–13 | topics are hubs by default; roughly half the subtopics stay hubs under LD-04 |
| Overview articles | 2–6 | only where an overview adds real teaching value beyond its children |
| Atomic articles | 26–48 | ≈0.7–1.3 per microtopic: some microtopics merge into one studiable unit, others split |
| **Total articles** | **28–54** | |

These are ranges derived from node counts, not commitments. `SYS-HEM-INVENTORY-001`
...
**Baseline to improve on honestly:** 122 concepts currently touch this
system; 94 are canonically homed here; 0 relations touch
...
having):** SYS-IMM (lymphoid tissue, immunodeficiency overlap), SYS-CVS (thrombosis, anticoagulation), SYS-INF (malaria, parasitic anaemia — high Egyptian relevance), SYS-OBS (anaemia in pregnancy, rhesus disease), SYS-MUL (massive transfusion, DIC).
...
- Anticoagulation initiation, reversal and bridging — `treatment_or_action`, faculty review.
- Transfusion thresholds and reaction management — patient-safety critical.
- Chemotherapy regimens — out of Years 1–4 depth for dosing; record principles only and mark the boundary explicitly.
...
- Haematopoiesis lineage tree — the organising diagram for the whole system.
- Blood film morphology plates for each named anaemia and leukaemia — this system is almost entirely morphological.
- Coagulation cascade diagram with drug and test intervention points.
- Lymph node architecture histology plate.
- Haemoglobin electrophoresis pattern comparison.
...
High Qbank density — blood film and coagulation interpretation are standard item formats. Low OSCE relevance; **very high laboratory-interpretation practical relevance**, making this a priority system for `Lab interpretation` practicals after GATE-LIBRARY-001.
...
|---:|---|---|---|---|
| 1 | `SYS-HEM-INVENTORY-001` | Classify every one of the 56 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-HEM-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-HEM-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-HEM-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-HEM-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-HEM-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-HEM-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-HEM-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-HEM` | System completion | — | tasks 1–8 |
...
- Morphology plate rights are the binding constraint, as with MSK anatomy.
- The 94 existing concepts are histology-shaped; converting them into clinically useful assessable objectives is `enrich`/`split` work, not a simple keep.

**Completion criteria (`GATE-SYS-HEM`).** All 56
...
> **`SYS-HEM-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 56 `SYS-HEM` nodes
  ...
  every one of the 9 articles and 122 concepts that
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-HEM-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-HEM-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-HEM-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-HEM-SOURCE-001`.
