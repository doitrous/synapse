# SYS-REN — Renal & Urinary System

**System 5 of 19** in the locked execution order (LD-09). Previous: [SYS-RES](SYS-RES.md). Next: [SYS-GIT](SYS-GIT.md).
...
| **Active task** | `SYS-REN-INVENTORY-001` (Not started) |
...
**In scope.** Renal and urinary structure and function, fluid/electrolytes/acid–base, acute and chronic kidney disease, glomerular and vascular disease, tubular and interstitial disease, urinary tract and urology, and renal replacement and procedures.

**Out of scope.** Nephrotoxic and diuretic drug classes (DIS-PHA). Male genitourinary disease (SYS-AND) — SYS-REN keeps the urinary tract, SYS-AND keeps the male reproductive organs.

**Learner years.** Years 1–2 structure/function and fluid balance; Years 3–4 disease and replacement therapy

**Prerequisites.** SYS-FND, DIS-ANA, DIS-PHY, SYS-CVS (perfusion, RAAS).

**Cross-system dependencies.** SYS-CVS (RAAS, hypertension, oedema), SYS-END (calcium/phosphate, erythropoietin, ADH), SYS-AND (shared lower urinary tract), SYS-MUL (AKI in critical illness), SYS-HEM (anaemia of CKD).

**Runtime tree.** Runtime subject `renal` — 9 topics, fully crosswalked. Note the topic was renamed to `Renal replacement therapy` in the 2026-08-12 de-duplication.
...
|---|---|---|---:|---:|---|
| `SYS-REN-T01` | Structure and function | `TPL-CONCEPT` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-REN-T02` | Fluid, electrolytes and acid-base | `TPL-CONDITION` | 3 | 12 | Hub (confirm in inventory) |
| `SYS-REN-T03` | Acute and chronic kidney disease | `TPL-CONDITION` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-REN-T04` | Glomerular and vascular disease | `TPL-CONDITION` | 3 | 9 | Hub (confirm in inventory) |
| `SYS-REN-T05` | Tubular and interstitial disease | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-REN-T06` | Urinary tract and urology | `TPL-CONDITION` | 3 | 9 | Hub (confirm in inventory) |
| `SYS-REN-T07` | Renal replacement and procedures | `TPL-SKILL` | 2 | 7 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (81 nodes)</summary>

- `SYS-REN-T01` **Structure and function** *(TPL-CONCEPT)*
  - `SYS-REN-T01-S01` **Renal anatomy and histology** — `Kidney` · `Nephron` · `Urinary tract`
  - `SYS-REN-T01-S02` **Renal physiology** — `Filtration` · `Tubular handling` · `Concentration` · `RAAS`
- `SYS-REN-T02` **Fluid, electrolytes and acid-base** *(TPL-CONDITION)*
  - `SYS-REN-T02-S01` **Volume and sodium** — `Hypovolemia` · `Hypervolemia` · `Hyponatremia` · `Hypernatremia`
  - `SYS-REN-T02-S02` **Potassium and other electrolytes** — `Hypokalemia` · `Hyperkalemia` · `Calcium` · `Magnesium`
  - `SYS-REN-T02-S03` **Acid-base disorders** — `Metabolic acidosis` · `Metabolic alkalosis` · `Respiratory disorders` · `Mixed disorders`
- `SYS-REN-T03` **Acute and chronic kidney disease** *(TPL-CONDITION)*
  - `SYS-REN-T03-S01` **Acute kidney injury** — `Prerenal` · `Intrinsic` · `Postrenal`
  - `SYS-REN-T03-S02` **Chronic kidney disease** — `Staging` · `Complications` · `Progression` · `Conservative care`
- `SYS-REN-T04` **Glomerular and vascular disease** *(TPL-CONDITION)*
  - `SYS-REN-T04-S01` **Nephritic disease** — `IgA nephropathy` · `Postinfectious GN` · `RPGN`
  - `SYS-REN-T04-S02` **Nephrotic disease** — `Minimal change` · `FSGS` · `Membranous disease`
  - `SYS-REN-T04-S03` **Renal vascular disease** — `Renal artery stenosis` · `TMA` · `Hypertensive nephropathy`
- `SYS-REN-T05` **Tubular and interstitial disease** *(TPL-CONDITION)*
  - `SYS-REN-T05-S01` **Tubular disorders** — `RTA` · `Fanconi syndrome` · `Diabetes insipidus`
  - `SYS-REN-T05-S02` **Interstitial disease** — `Acute interstitial nephritis` · `Chronic tubulointerstitial disease` · `Pyelonephritis`
- `SYS-REN-T06` **Urinary tract and urology** *(TPL-CONDITION)*
  - `SYS-REN-T06-S01` **Infection** — `Cystitis` · `Pyelonephritis` · `Urosepsis`
  - `SYS-REN-T06-S02` **Stones and obstruction** — `Nephrolithiasis` · `Hydronephrosis` · `Urinary retention`
  - `SYS-REN-T06-S03` **Urologic tumors and voiding disorders** — `Bladder cancer` · `Incontinence` · `Neurogenic bladder`
- `SYS-REN-T07` **Renal replacement and procedures** *(TPL-SKILL)*
  - `SYS-REN-T07-S01` **Dialysis and transplantation** — `Hemodialysis` · `Peritoneal dialysis` · `Transplantation`
  - `SYS-REN-T07-S02` **Tests and procedures** — `Urinalysis` · `Renal imaging` · `Catheterization` · `Fluid prescription`

</details>

### Decided classification (`SYS-REN-INVENTORY-001`, 2026-08-12)

All 81 nodes are classified. Evidence:
[`SYS-REN-node-classification.json`](../evidence/SYS-REN-node-classification.json).
...
16 of these nodes carry a local teaching signal — the corpus teaches
them in two or more processed sources — and are the ones to author first (`LD-14`).

*Initial classification is a placeholder. `SYS-REN-INVENTORY-001` replaces every
...
|---|---|---|
| `Renal and urinary system` (By system) | `already covered under a different Synapse label` | Direct match. |
| `Urology` (Clinical knowledge, 8 child groups) | `secondary placement or cross-reference` | Synapse owns DIS-URO-equivalent material under SYS-REN-T06 and SYS-AND. Do not adopt a parallel urology root. |
...
Confirm the SYS-REN-T06 `Urinary tract and urology` / SYS-AND boundary explicitly. Proposal: bladder, ureter, urethra and stones canonically in SYS-REN; prostate, testis and penis canonically in SYS-AND; each cross-referenced.
...
No canonical article or concept. 10 articles and 119 concepts reach in secondarily. 3 articles are `Published` (`ART-REN-TOP-*`), affected by BLK-06/BLK-07.

**Measured baseline for `SYS-REN`:**
...
|---|---|---|---|---|---|
| — | *No article has `SYS-REN` as its canonical home* | — | — | — | Every article in this system is new, or is a `move of canonical home` decision on a discipline-rooted record (DEC-005) |

### Decided dispositions (`SYS-REN-INVENTORY-001`, 2026-08-12)

Evidence: [`SYS-REN-disposition-ledger.json`](../evidence/SYS-REN-disposition-ledger.json).

| | Articles | Concepts |
|---|---:|---:|
| Touching this system | 10 | 119 |
...
`SYS-REN-INVENTORY-001` must count how many of them belong to this system and
...
| **Total articles** | **42–81** | |

These are ranges derived from node counts, not commitments. `SYS-REN-INVENTORY-001`
...
**Baseline to improve on honestly:** 119 concepts currently touch this
...
having):** SYS-CVS (RAAS, hypertension, oedema), SYS-END (calcium/phosphate, erythropoietin, ADH), SYS-AND (shared lower urinary tract), SYS-MUL (AKI in critical illness), SYS-HEM (anaemia of CKD).
...
- Electrolyte correction rates — hyponatraemia correction is a genuine iatrogenic-harm risk; `treatment_or_action`, faculty review, non-negotiable.
- Dialysis initiation criteria and AKI management — time-sensitive guideline material.
- Drug dosing in renal impairment — dosing content; faculty review.
...
- Nephron diagram with transporter sites by segment — the single highest-value visual in this system; diuretic and tubular-disorder teaching depends on it.
- Glomerular filtration barrier ultrastructure (histology plate).
- Acid–base Davenport / nomogram interpretation graph.
- Glomerular disease light/immunofluorescence/EM comparison table.
- Urinary tract anatomy plate with stone impaction points.
...
High Qbank density, especially electrolyte and acid–base interpretation items. Moderate OSCE relevance; strong laboratory-interpretation practical relevance.
...
|---:|---|---|---|---|
| 1 | `SYS-REN-INVENTORY-001` | Classify every one of the 81 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-REN-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-REN-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-REN-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-REN-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-REN-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-REN-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-REN-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-REN` | System completion | — | tasks 1–8 |
...
- Acid–base is taught differently across Egyptian faculties; the university-notes mechanism (`universityNotes`) exists and should be used rather than picking one convention silently.

**Completion criteria (`GATE-SYS-REN`).** All 81
...
> **`SYS-REN-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 81 `SYS-REN` nodes
  ...
  every one of the 10 articles and 119 concepts that
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-REN-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-REN-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-REN-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-REN-SOURCE-001`.
