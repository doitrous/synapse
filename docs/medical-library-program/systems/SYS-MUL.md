# SYS-MUL — Multisystem Processes, Emergencies & Critical Care

**System 18 of 19** in the locked execution order (LD-09). Previous: [SYS-DER](SYS-DER.md). Next: [SYS-POP](SYS-POP.md).
...
| **Active task** | `SYS-MUL-INVENTORY-001` (Not started) |
...
**In scope.** Acute deterioration, sepsis and organ failure, trauma, toxicology and environmental medicine, oncology principles, perioperative and critical care, and pain/palliative/end-of-life care.

**Out of scope.** Organ-specific emergencies keep their canonical home in the organ system (STEMI in SYS-CVS, status epilepticus in SYS-NEU); SYS-MUL owns the cross-system syndrome and the generic approach.

**Learner years.** Years 3–4

**Prerequisites.** All organ systems. **This is the terminal system in the learning order** — it integrates everything.

**Cross-system dependencies.** Every system. Also SKL-ERS (emergency and resuscitation skills), DIS-ANE, DIS-EMC, KNW-EMG.

**Runtime tree.** No runtime subject maps to SYS-MUL. Several runtime `*-emergencies-and-skills` topics crosswalk to `KNW-EMG` as their primary placement.
...
|---|---|---|---:|---:|---|
| `SYS-MUL-T01` | Acute deterioration | `TPL-EMERGENCY` | 2 | 9 | Hub (confirm in inventory) |
| `SYS-MUL-T02` | Sepsis and organ failure | `TPL-EMERGENCY` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-MUL-T03` | Trauma | `TPL-EMERGENCY` | 2 | 9 | Hub (confirm in inventory) |
| `SYS-MUL-T04` | Toxicology and environmental medicine | `TPL-EMERGENCY` | 2 | 8 | Hub (confirm in inventory) |
| `SYS-MUL-T05` | Oncology principles | `TPL-EMERGENCY` | 2 | 9 | Hub (confirm in inventory) |
| `SYS-MUL-T06` | Perioperative and critical care | `TPL-EMERGENCY` | 3 | 12 | Hub (confirm in inventory) |
| `SYS-MUL-T07` | Pain, palliative and end-of-life care | `TPL-EMERGENCY` | 2 | 7 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (83 nodes)</summary>

- `SYS-MUL-T01` **Acute deterioration** *(TPL-EMERGENCY)*
  - `SYS-MUL-T01-S01` **ABCDE assessment** — `Airway` · `Breathing` · `Circulation` · `Disability` · `Exposure`
  - `SYS-MUL-T01-S02` **Shock and resuscitation** — `Hypovolemic` · `Cardiogenic` · `Distributive` · `Obstructive`
- `SYS-MUL-T02` **Sepsis and organ failure** *(TPL-EMERGENCY)*
  - `SYS-MUL-T02-S01` **Sepsis syndromes** — `Recognition` · `Initial management` · `Source control`
  - `SYS-MUL-T02-S02` **Multiple organ failure** — `Respiratory` · `Renal` · `Cardiovascular` · `Neurologic`
- `SYS-MUL-T03` **Trauma** *(TPL-EMERGENCY)*
  - `SYS-MUL-T03-S01` **Primary and secondary survey** — `Airway` · `Chest` · `Abdomen` · `Pelvis` · `Neurologic`
  - `SYS-MUL-T03-S02` **Specific trauma** — `Head injury` · `Spinal injury` · `Burns` · `Hemorrhage`
- `SYS-MUL-T04` **Toxicology and environmental medicine** *(TPL-EMERGENCY)*
  - `SYS-MUL-T04-S01` **Poisoning** — `Paracetamol` · `Organophosphate` · `Opioid` · `Corrosives`
  - `SYS-MUL-T04-S02` **Environmental emergencies** — `Heat illness` · `Drowning` · `Envenomation` · `Electrical injury`
- `SYS-MUL-T05` **Oncology principles** *(TPL-EMERGENCY)*
  - `SYS-MUL-T05-S01` **Cancer diagnosis and staging** — `Screening` · `Biopsy` · `TNM` · `Performance status`
  - `SYS-MUL-T05-S02` **Cancer treatment and emergencies** — `Chemotherapy` · `Radiotherapy` · `Neutropenic sepsis` · `Cord compression` · `TLS`
- `SYS-MUL-T06` **Perioperative and critical care** *(TPL-EMERGENCY)*
  - `SYS-MUL-T06-S01` **Perioperative medicine** — `Risk assessment` · `Fluids` · `Analgesia` · `Complications`
  - `SYS-MUL-T06-S02` **Critical-care support** — `Oxygen` · `Ventilation` · `Vasopressors` · `Nutrition`
  - `SYS-MUL-T06-S03` **Anaesthesia** — `Preoperative preparation` · `Airway and ventilation` · `General and regional techniques` · `Monitoring and recovery`
- `SYS-MUL-T07` **Pain, palliative and end-of-life care** *(TPL-EMERGENCY)*
  - `SYS-MUL-T07-S01` **Pain management** — `Assessment` · `Non-opioids` · `Opioids` · `Neuropathic pain`
  - `SYS-MUL-T07-S02` **Palliative care** — `Symptom control` · `Goals of care` · `End-of-life communication`

</details>

### Decided classification (`SYS-MUL-INVENTORY-001`, 2026-08-12)

All 83 nodes are classified. Evidence:
[`SYS-MUL-node-classification.json`](../evidence/SYS-MUL-node-classification.json).
...
| empty but legitimate planned destination | 1 |
| **Planned articles** | **61** |
...
*Initial classification is a placeholder. `SYS-MUL-INVENTORY-001` replaces every
...
|---|---|---|
| `Multisystem processes and disorders` (By system) | `already covered under a different Synapse label` | Direct match in name; Synapse adds perioperative, palliative and critical care, which the taxonomy review justified against the GMC MLA content map. |
| `Emergency medicine` (Clinical knowledge, 20 groups) | `secondary placement or cross-reference` | DIS-EMC. |
| `Anesthesiology` (25 direct articles) | `secondary placement or cross-reference` | SYS-MUL-T06 and DIS-ANE. Anaesthesiology was an explicit taxonomy-review addition. |
| `On-call survival guide` (40 nodes), `Clerkship survival guide` (11 nodes) | `outside Years 1–4 scope` | Practice-orientation material for US residents. Rejected wholesale. |
...
Resolve SYS-MUL-T01 `Acute deterioration` against SYS-FND-T06 `Core mechanisms` (see the SYS-FND note) in one Phase 1 task.
...
|---|---|---|---|---|---|
| — | *No article has `SYS-MUL` as its canonical home* | — | — | — | Every article in this system is new, or is a `move of canonical home` decision on a discipline-rooted record (DEC-005) |

### Decided dispositions (`SYS-MUL-INVENTORY-001`, 2026-08-12)

Evidence: [`SYS-MUL-disposition-ledger.json`](../evidence/SYS-MUL-disposition-ledger.json).
...
`SYS-MUL-INVENTORY-001` must count how many of them belong to this system and
...
| Atomic articles | 43–79 | ≈0.7–1.3 per microtopic: some microtopics merge into one studiable unit, others split |
| **Total articles** | **45–86** | |

These are ranges derived from node counts, not commitments. `SYS-MUL-INVENTORY-001`
...
having):** Every system. Also SKL-ERS (emergency and resuscitation skills), DIS-ANE, DIS-EMC, KNW-EMG.
...
- **This system is high-risk almost in its entirety.** Resuscitation algorithms, sepsis bundles, trauma primary survey, toxicological antidotes and doses, analgesic ladders and end-of-life prescribing are all `treatment_or_action` or `emergency`, and most are time-sensitive.
- Resuscitation guidelines are versioned and dated; every algorithm article must carry the guideline version, its date, and a review date.
- Opioid dosing in palliative care — dosing content with a genuine safety profile.
...
- ABCDE assessment sequence diagram.
- Resuscitation algorithm flowcharts (adult and paediatric) — versioned and dated; the algorithm *is* the content.
- Trauma primary survey flowchart.
- Shock type comparison table (haemodynamic profiles) — a table that carries the whole differential.
- Analgesic ladder diagram.
...
Moderate Qbank density but very high clinical-reasoning weighting. **The highest OSCE and simulation relevance of any system** — resuscitation, deteriorating-patient, and breaking-bad-news stations. Record the largest practical target here.
...
|---:|---|---|---|---|
| 1 | `SYS-MUL-INVENTORY-001` | Classify every one of the 83 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-MUL-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-MUL-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-MUL-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-MUL-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-MUL-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-MUL-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-MUL-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-MUL` | System completion | — | tasks 1–8 |
...
- Greenfield, entirely high-risk, and dependent on the faculty-review route (OQ-02). Without an answer to OQ-02, most of this system cannot be published at all.
- Guideline versioning discipline is essential: an out-of-date resuscitation algorithm is worse than none.

**Completion criteria (`GATE-SYS-MUL`).** All 83
...
> **`SYS-MUL-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 83 `SYS-MUL` nodes
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-MUL-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-MUL-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-MUL-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-MUL-SOURCE-001`.
