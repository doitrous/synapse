# SYS-IMM — Immune System

**System 11 of 19** in the locked execution order (LD-09). Previous: [SYS-HEM](SYS-HEM.md). Next: [SYS-INF](SYS-INF.md).
...
| **Active task** | `SYS-IMM-INVENTORY-001` (Not started) |
...
**In scope.** Normal immune function, hypersensitivity and allergy, autoimmune disease, immunodeficiency, and transplantation and immunotherapy.

**Out of scope.** Immunosuppressant and anti-inflammatory drug classes (DIS-PHA). Vaccine schedules (SYS-DEV / SYS-POP) — SYS-IMM owns the immunological mechanism of vaccination.

**Learner years.** Years 1–2 normal immune function; Years 3–4 immune disease

**Prerequisites.** SYS-FND (cell biology, inflammation), DIS-HIS.

**Cross-system dependencies.** SYS-HEM (lymphoid tissue), SYS-INF (host defence, HIV), SYS-MSK (autoimmune arthritis), SYS-DER (autoimmune blistering disease), SYS-RES (asthma, hypersensitivity pneumonitis), SYS-GIT (coeliac, IBD).

**Runtime tree.** No runtime subject maps to SYS-IMM.
...
|---|---|---|---:|---:|---|
| `SYS-IMM-T01` | Normal immune function | `TPL-CONCEPT` | 2 | 8 | Hub (confirm in inventory) |
| `SYS-IMM-T02` | Hypersensitivity and allergy | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-IMM-T03` | Autoimmune disease | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-IMM-T04` | Immunodeficiency | `TPL-CONDITION` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-IMM-T05` | Transplantation and immunotherapy | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (48 nodes)</summary>

- `SYS-IMM-T01` **Normal immune function** *(TPL-CONCEPT)*
  - `SYS-IMM-T01-S01` **Innate immunity** — `Barriers` · `Complement` · `Phagocytes` · `Inflammation`
  - `SYS-IMM-T01-S02` **Adaptive immunity** — `B cells` · `T cells` · `Antibodies` · `Tolerance`
- `SYS-IMM-T02` **Hypersensitivity and allergy** *(TPL-CONDITION)*
  - `SYS-IMM-T02-S01` **Immediate hypersensitivity** — `Anaphylaxis` · `Atopy` · `Urticaria`
  - `SYS-IMM-T02-S02` **Delayed and immune-complex disease** — `Type II` · `Type III` · `Type IV`
- `SYS-IMM-T03` **Autoimmune disease** *(TPL-CONDITION)*
  - `SYS-IMM-T03-S01` **Systemic autoimmune disease** — `SLE` · `Systemic sclerosis` · `Vasculitis`
  - `SYS-IMM-T03-S02` **Organ-specific autoimmunity** — `Thyroid` · `Neuromuscular` · `Gastrointestinal`
- `SYS-IMM-T04` **Immunodeficiency** *(TPL-CONDITION)*
  - `SYS-IMM-T04-S01` **Primary immunodeficiency** — `Humoral` · `Cellular` · `Combined` · `Phagocyte`
  - `SYS-IMM-T04-S02` **Secondary immunodeficiency** — `HIV` · `Malnutrition` · `Iatrogenic`
- `SYS-IMM-T05` **Transplantation and immunotherapy** *(TPL-CONDITION)*
  - `SYS-IMM-T05-S01` **Transplant immunology** — `Rejection` · `GVHD` · `Immunosuppression`
  - `SYS-IMM-T05-S02` **Biologics and immunomodulation** — `Monoclonal antibodies` · `Cytokine therapy` · `Vaccines`

</details>

### Decided classification (`SYS-IMM-INVENTORY-001`, 2026-08-12)

All 48 nodes are classified. Evidence:
[`SYS-IMM-node-classification.json`](../evidence/SYS-IMM-node-classification.json).

| Classification | Nodes |
|---|---:|
| navigation-only hub | 15 |
| overview article | 0 |
| atomic article home | 33 |
...
*Initial classification is a placeholder. `SYS-IMM-INVENTORY-001` replaces every
...
|---|---|---|
| `Immune system` (By system) | `already covered under a different Synapse label` | Direct match. |
| `Immunology` (By discipline) | `secondary placement or cross-reference` | DIS-IMU is the discipline route for the same canonical items. |
...
None proposed. Confirm the SYS-IMM / SYS-HEM lymphoid-tissue boundary alongside the SYS-HEM decision, in one Phase 1 task rather than two.
...
No canonical article or concept under SYS-IMM. 10 articles and 133 concepts reach in secondarily — **the largest secondary-placement count of any system**, reflecting 121 DIS-IMU concepts. Phase 1 must decide whether DIS-IMU or SYS-IMM is the canonical home for immunological mechanism; the taxonomy review says both routes are equal, so this is a per-record decision under DEC-005.

**Measured baseline for `SYS-IMM`:**
...
|---|---|---|---|---|---|
| — | *No article has `SYS-IMM` as its canonical home* | — | — | — | Every article in this system is new, or is a `move of canonical home` decision on a discipline-rooted record (DEC-005) |

### Decided dispositions (`SYS-IMM-INVENTORY-001`, 2026-08-12)

Evidence: [`SYS-IMM-disposition-ledger.json`](../evidence/SYS-IMM-disposition-ledger.json).

| | Articles | Concepts |
|---|---:|---:|
| Touching this system | 10 | 133 |
| Canonically homed here | 0 | 0 |
| `enrich` | 0 | 0 |
| `correct` (BLK-09) | 10 | 133 |
| Outside the subject-ID contract | 10 | 133 |
...
`SYS-IMM-INVENTORY-001` must count how many of them belong to this system and
...
|---|---|---|
| Navigation-only hubs | 5–10 | topics are hubs by default; roughly half the subtopics stay hubs under LD-04 |
| Overview articles | 2–5 | only where an overview adds real teaching value beyond its children |
| Atomic articles | 23–43 | ≈0.7–1.3 per microtopic: some microtopics merge into one studiable unit, others split |
| **Total articles** | **25–48** | |

These are ranges derived from node counts, not commitments. `SYS-IMM-INVENTORY-001`
...
**Baseline to improve on honestly:** 133 concepts currently touch this
...
having):** SYS-HEM (lymphoid tissue), SYS-INF (host defence, HIV), SYS-MSK (autoimmune arthritis), SYS-DER (autoimmune blistering disease), SYS-RES (asthma, hypersensitivity pneumonitis), SYS-GIT (coeliac, IBD).
...
- Anaphylaxis management — adrenaline dose and route; emergency, absolute faculty review.
- Immunosuppression and transplant regimens — dosing, out of depth for Years 1–4 beyond principles.
- Biologic therapy indications — time-sensitive.
...
- Innate/adaptive immunity overview flowchart.
- T-cell and B-cell activation sequence diagrams — mechanism sequences.
- Antibody structure and isotype comparison table.
- Complement pathway diagram with deficiency points.
- Hypersensitivity type I–IV comparison table — the classic teaching object.
- MHC/antigen presentation diagram.
...
High Qbank density — mechanism items and hypersensitivity classification are staples. Low OSCE relevance except allergy history and adrenaline auto-injector counselling.
...
|---:|---|---|---|---|
| 1 | `SYS-IMM-INVENTORY-001` | Classify every one of the 48 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-IMM-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-IMM-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-IMM-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-IMM-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-IMM-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-IMM-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-IMM-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-IMM` | System completion | — | tasks 1–8 |
...
- The DIS-IMU/SYS-IMM canonical-home question affects 121 concepts and must be settled before any enrichment batch, or the same records will be edited twice.

**Completion criteria (`GATE-SYS-IMM`).** All 48
...
> **`SYS-IMM-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 48 `SYS-IMM` nodes
  ...
  every one of the 10 articles and 133 concepts that
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-IMM-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-IMM-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-IMM-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-IMM-SOURCE-001`.
