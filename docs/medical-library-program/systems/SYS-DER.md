# SYS-DER — Skin & Subcutaneous Tissue

**System 17 of 19** in the locked execution order (LD-09). Previous: [SYS-PSY](SYS-PSY.md). Next: [SYS-MUL](SYS-MUL.md).
...
| **Active task** | `SYS-DER-INVENTORY-001` (Not started) |
...
**In scope.** Dermatological assessment, inflammatory skin disease, skin infection and infestation, autoimmune and blistering disease, skin tumours, and wounds/burns/hair/nails.

**Out of scope.** Topical and systemic dermatological drug classes (DIS-PHA). Burns as major trauma (SYS-MUL).

**Learner years.** Years 3–4

**Prerequisites.** SYS-FND (inflammation), DIS-HIS (skin histology), SYS-IMM.

**Cross-system dependencies.** SYS-IMM (autoimmune blistering, eczema), SYS-INF (skin infection and infestation — high Egyptian relevance for leishmaniasis), SYS-MSK (connective-tissue disease), SYS-MUL (burns, toxic epidermal necrolysis).

**Runtime tree.** No runtime subject maps to SYS-DER.
...
|---|---|---|---:|---:|---|
| `SYS-DER-T01` | Dermatologic assessment | `TPL-PRESENTATION` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-DER-T02` | Inflammatory skin disease | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-DER-T03` | Skin infection and infestation | `TPL-CONDITION` | 2 | 8 | Hub (confirm in inventory) |
| `SYS-DER-T04` | Autoimmune and blistering disease | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-DER-T05` | Skin tumors | `TPL-CONDITION` | 2 | 5 | Hub (confirm in inventory) |
| `SYS-DER-T06` | Wounds, burns, hair and nails | `TPL-CONDITION` | 2 | 5 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (55 nodes)</summary>

- `SYS-DER-T01` **Dermatologic assessment** *(TPL-PRESENTATION)*
  - `SYS-DER-T01-S01` **Lesion morphology** — `Primary lesions` · `Secondary changes` · `Distribution`
  - `SYS-DER-T01-S02` **Common presentations** — `Rash` · `Pruritus` · `Ulcer` · `Pigment change`
- `SYS-DER-T02` **Inflammatory skin disease** *(TPL-CONDITION)*
  - `SYS-DER-T02-S01` **Eczematous and papulosquamous disease** — `Atopic dermatitis` · `Contact dermatitis` · `Psoriasis`
  - `SYS-DER-T02-S02` **Acneiform and follicular disease** — `Acne` · `Rosacea` · `Hidradenitis`
- `SYS-DER-T03` **Skin infection and infestation** *(TPL-CONDITION)*
  - `SYS-DER-T03-S01` **Bacterial and viral disease** — `Cellulitis` · `Impetigo` · `Herpes` · `Warts`
  - `SYS-DER-T03-S02` **Fungal and parasitic disease** — `Dermatophytes` · `Candida` · `Scabies` · `Lice`
- `SYS-DER-T04` **Autoimmune and blistering disease** *(TPL-CONDITION)*
  - `SYS-DER-T04-S01` **Blistering disorders** — `Pemphigus` · `Pemphigoid` · `Dermatitis herpetiformis`
  - `SYS-DER-T04-S02` **Connective-tissue manifestations** — `Lupus` · `Dermatomyositis` · `Scleroderma`
- `SYS-DER-T05` **Skin tumors** *(TPL-CONDITION)*
  - `SYS-DER-T05-S01` **Benign and premalignant lesions** — `Nevi` · `Keratoses`
  - `SYS-DER-T05-S02` **Skin cancer** — `Melanoma` · `Basal cell carcinoma` · `Squamous cell carcinoma`
- `SYS-DER-T06` **Wounds, burns, hair and nails** *(TPL-CONDITION)*
  - `SYS-DER-T06-S01` **Wounds and burns** — `Pressure injury` · `Burn assessment` · `Wound healing`
  - `SYS-DER-T06-S02` **Appendage disorders** — `Alopecia` · `Nail disorders`

</details>

### Decided classification (`SYS-DER-INVENTORY-001`, 2026-08-12)

All 55 nodes are classified. Evidence:
[`SYS-DER-node-classification.json`](../evidence/SYS-DER-node-classification.json).
...
*Initial classification is a placeholder. `SYS-DER-INVENTORY-001` replaces every
...
|---|---|---|
| `Skin and subcutaneous tissue` (By system) | `already covered under a different Synapse label` | Direct match. |
| `Dermatology` (Clinical knowledge, 10 groups) | `secondary placement or cross-reference` | DIS-DRM is the discipline route. |
...
No processed local source. 6 articles and 57 concepts reach in secondarily — **the lowest secondary count of any non-zero system**, reflecting how little skin histology is in the processed corpus.

### Decided source plan (`SYS-DER-SOURCE-001`, 2026-08-12)

Evidence: [`SYS-DER-source-plan.md`](../evidence/SYS-DER-source-plan.md) ·
[`SYS-DER-source-plan.json`](../evidence/SYS-DER-source-plan.json).

| | |
|---|---:|
| Planned articles | 37 |
| — with a processed local source | 21 |
| — authored from authoritative sources only | 16 |
| Distinct corpus files touching this system | 63 |
...
No canonical article or concept. 6 articles and 57 concepts secondary.

**Measured baseline for `SYS-DER`:**
...
|---|---|---|---|---|---|
| — | *No article has `SYS-DER` as its canonical home* | — | — | — | Every article in this system is new, or is a `move of canonical home` decision on a discipline-rooted record (DEC-005) |

### Decided dispositions (`SYS-DER-INVENTORY-001`, 2026-08-12)

Evidence: [`SYS-DER-disposition-ledger.json`](../evidence/SYS-DER-disposition-ledger.json).

| | Articles | Concepts |
|---|---:|---:|
| Touching this system | 6 | 57 |
| Canonically homed here | 0 | 0 |
| `enrich` | 0 | 0 |
| `correct` (BLK-09) | 6 | 57 |
| Outside the subject-ID contract | 6 | 57 |
...
`SYS-DER-INVENTORY-001` must count how many of them belong to this system and
...
These are ranges derived from node counts, not commitments. `SYS-DER-INVENTORY-001`
...
**Baseline to improve on honestly:** 57 concepts currently touch this
...
having):** SYS-IMM (autoimmune blistering, eczema), SYS-INF (skin infection and infestation — high Egyptian relevance for leishmaniasis), SYS-MSK (connective-tissue disease), SYS-MUL (burns, toxic epidermal necrolysis).
...
- Systemic therapy for psoriasis and eczema (methotrexate, ciclosporin, biologics) — dosing and monitoring.
- Melanoma recognition and referral thresholds — a diagnostic-safety topic; getting the referral criteria wrong is a real harm.
- Burns fluid resuscitation — formula-based dosing; emergency; faculty review.
- Toxic epidermal necrolysis / Stevens–Johnson — drug-induced emergency.
...
- **This system is visually dependent to an unusual degree: dermatology is pattern recognition.** Primary and secondary lesion morphology plate — the vocabulary of the whole system.
- Skin layer and appendage histology plate.
- Clinical photographs for each named condition — the binding constraint. Rights and, critically, **skin-tone representation**: an Egyptian student needs conditions shown on the skin tones they will actually see. Record this explicitly in every recommendation's source direction.
- Dermoscopy examples for pigmented lesions.
- Burn depth and body-surface-area (rule of nines) diagram.
...
Moderate Qbank density, image-dependent. Moderate OSCE relevance: skin lesion description and examination, sun-protection counselling. **Image-based interpretation practicals are the natural format** — record targets.
...
|---:|---|---|---|---|
| 1 | `SYS-DER-INVENTORY-001` | Classify every one of the 55 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-DER-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-DER-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-DER-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-DER-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-DER-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-DER-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-DER-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-DER` | System completion | — | tasks 1–8 |
...
- Without licensed, skin-tone-appropriate clinical images this system cannot reach a usable standard. The image-recommendation backlog is the gating constraint, not the prose.
- Do not author condition articles that assume an image will exist; state the dependency in the acceptance criteria.

**Completion criteria (`GATE-SYS-DER`).** All 55
...
> **`SYS-DER-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 55 `SYS-DER` nodes
  ...
  every one of the 6 articles and 57 concepts that
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-DER-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-DER-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-DER-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-DER-SOURCE-001`.
