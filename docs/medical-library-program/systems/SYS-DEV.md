# SYS-DEV — Human Development & Life Stages

**System 2 of 19** in the locked execution order (LD-09). Previous: [SYS-FND](SYS-FND.md). Next: [SYS-CVS](SYS-CVS.md).
...
| **Active task** | `SYS-DEV-INVENTORY-001` (Not started) |
...
**In scope.** The life course as an organising principle: normal growth and development, adult health, nutrition across the life course, and care of the well patient (screening, immunisation, health maintenance by age).

**Out of scope.** Paediatric disease of a named organ system — that belongs to the organ system. Pregnancy belongs to SYS-OBS. Ageing-related organ pathology belongs to its system.

**Learner years.** Years 1–4; growth and development in Year 1–2, care of the well patient in Year 3–4

**Prerequisites.** SYS-FND (cell biology, genetics).

**Cross-system dependencies.** SYS-OBS (fetal and neonatal continuity), SYS-END (growth hormones, puberty), SYS-POP (screening and prevention overlap — resolve ownership per node in Phase 1), SYS-PSY (neurodevelopment).

**Runtime tree.** No runtime subject maps to SYS-DEV.
...
|---|---|---|---:|---:|---|
| `SYS-DEV-T01` | Growth and development | `TPL-CONCEPT` | 2 | 8 | Hub (confirm in inventory) |
| `SYS-DEV-T02` | Adult health | `TPL-CONDITION` | 2 | 7 | Hub (confirm in inventory) |
| `SYS-DEV-T03` | Nutrition across the life course | `TPL-CONDITION` | 2 | 6 | Hub (confirm in inventory) |
| `SYS-DEV-T04` | Care of the well patient | `TPL-CONDITION` | 3 | 11 | Hub (confirm in inventory) |

<details>
<summary>Full node tree (45 nodes)</summary>

- `SYS-DEV-T01` **Growth and development** *(TPL-CONCEPT)*
  - `SYS-DEV-T01-S01` **Infancy and early childhood** — `Growth` · `Milestones` · `Nutrition` · `Screening`
  - `SYS-DEV-T01-S02` **School age and adolescence** — `Puberty` · `Development` · `Risk behavior` · `Preventive care`
- `SYS-DEV-T02` **Adult health** *(TPL-CONDITION)*
  - `SYS-DEV-T02-S01` **Young and middle adulthood** — `Lifestyle` · `Screening` · `Occupational health`
  - `SYS-DEV-T02-S02` **Healthy aging** — `Normal aging` · `Frailty` · `Falls` · `Polypharmacy`
- `SYS-DEV-T03` **Nutrition across the life course** *(TPL-CONDITION)*
  - `SYS-DEV-T03-S01` **Macronutrients and micronutrients** — `Requirements` · `Deficiency` · `Excess`
  - `SYS-DEV-T03-S02` **Clinical nutrition** — `Malnutrition` · `Enteral nutrition` · `Parenteral nutrition`
- `SYS-DEV-T04` **Care of the well patient** *(TPL-CONDITION)*
  - `SYS-DEV-T04-S01` **Screening and anticipatory guidance** — `Risk assessment` · `Counseling` · `Vaccination`
  - `SYS-DEV-T04-S02` **Social and developmental context** — `Family` · `School` · `Work` · `Caregiving`
  - `SYS-DEV-T04-S03` **Disability and inclusive care** — `Function and participation` · `Communication and access needs` · `Reasonable adjustments` · `Caregiver and community support`
...
|---|---|---|
| `Human development` | `already covered under a different Synapse label` | Direct match to SYS-DEV. |
| `Family medicine` (Clinical knowledge, 21 child groups) | `secondary placement or cross-reference` | Synapse owns this as DIS-FCM. SYS-DEV-T04 is the life-course view of the same material. |
| `Pediatrics` (Clinical knowledge, 21 child groups) | `move of canonical home` — per node | AMBOSS keeps a large paediatric branch. Synapse deliberately distributes paediatric disease into organ systems and keeps only life-stage material here. Each of the 21 groups needs an individual disposition; do not adopt the branch. |
...
Candidate overlap to resolve, not to merge blind: SYS-DEV-T04 `Care of the well patient` against SYS-POP-T04 `Screening and prevention`. Proposal for Phase 1: SYS-POP owns the population-level rationale and evidence; SYS-DEV owns the age-banded individual encounter. Record as a cross-reference, not a duplicate.
...
No article and no concept has SYS-DEV as a canonical home. 9 articles and 111 concepts touch it as a secondary placement — overwhelmingly embryology (DIS-EMB, 129 concepts) reaching in. Phase 1 must decide per record whether developmental anatomy stays canonically in DIS-EMB (likely) with SYS-DEV secondary (likely).

**Measured baseline for `SYS-DEV`:**
...
`SYS-DEV-INVENTORY-001` must count how many of them belong to this system and
...
|---|---|---|
| Navigation-only hubs | 4–9 | topics are hubs by default; roughly half the subtopics stay hubs under LD-04 |
| Overview articles | 1–4 | only where an overview adds real teaching value beyond its children |
| Atomic articles | 22–42 | ≈0.7–1.3 per microtopic: some microtopics merge into one studiable unit, others split |
| **Total articles** | **23–46** | |

These are ranges derived from node counts, not commitments. `SYS-DEV-INVENTORY-001`
...
having):** SYS-OBS (fetal and neonatal continuity), SYS-END (growth hormones, puberty), SYS-POP (screening and prevention overlap — resolve ownership per node in Phase 1), SYS-PSY (neurodevelopment).
...
- Immunisation schedules — jurisdiction-specific and time-sensitive; must cite the current Egyptian national schedule with an access date, and be flagged `time_sensitive`.
- Nutrition recommendations in pregnancy and infancy — dosing-adjacent; faculty review.
...
- Growth charts and centile curves — the graph *is* the teaching object.
- Developmental-milestone comparison table by age band.
- Immunisation schedule table (Egyptian), versioned and dated.
- Tanner staging plate — requires careful rights and an appropriate, non-gratuitous illustration.
...
Moderate Qbank share; high OSCE relevance for developmental assessment and well-child/well-adult consultations. Communication-skill stations depend on this system.
...
|---:|---|---|---|---|
| 1 | `SYS-DEV-INVENTORY-001` | Classify every one of the 45 nodes; build the existing-content disposition ledger; count `BLK-09` records | Classification + ledger only. No content written | `GATE-TAX-001` |
| 2 | `SYS-DEV-SOURCE-001` | Build the source plan: local corpus records with state, authoritative web sources per topic, conflicts to resolve | Source plan only | task 1 |
| 3 | `SYS-DEV-CONCEPT-001..n` | Concept batches, one topic each, ≤60 concepts | One topic per batch | task 2 |
| 4 | `SYS-DEV-ARTICLE-001..n` | Article batches, one topic each, ≤10 articles | One topic per batch | task 3 |
| 5 | `SYS-DEV-RELATION-001..n` | Relation batches, one family each, ≤80 edges | One family per batch | task 4 |
| 6 | `SYS-DEV-ANNOT-001` | Statement annotations across the system's atomic articles | ≤50 annotations | task 4 |
| 7 | `SYS-DEV-IMAGE-001` | Image recommendation inventory | All recommendations, no media fulfilment | task 4 |
| 8 | `SYS-DEV-PUBLISH-001` | Apply publication gates; enumerate the faculty-review queue | No high-risk publication without a named reviewer | tasks 4–7, `OQ-02` |
| 9 | `GATE-SYS-DEV` | System completion | — | tasks 1–8 |
...
- Zero canonical content and no mapped corpus: this system is authored almost from scratch against web sources.
- Screening/prevention ownership ambiguity with SYS-POP will produce duplicate articles if not settled before authoring.

**Completion criteria (`GATE-SYS-DEV`).** All 45
...
> **`SYS-DEV-INVENTORY-001` — Classify every node and build the disposition ledger.**
...
- **Objective.** Assign every one of the 45 `SYS-DEV` nodes
...
- **Files to write.**
  `docs/medical-library-program/evidence/SYS-DEV-node-classification.json` and
  `docs/medical-library-program/evidence/SYS-DEV-disposition-ledger.json`
...
- **Stop condition.** Both evidence files exist and §2, §6 and §7 of this plan
  ...
  `SYS-DEV-INVENTORY-001` to `Done`, record the commit, and set the active task
  to `SYS-DEV-SOURCE-001`.
