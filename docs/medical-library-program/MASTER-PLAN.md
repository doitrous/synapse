...
| `BLK-09` | 62 articles and 736 concepts carried `subjectId: "medical"`, which is not one of the twenty valid subjects, so the student library could not group them | **Closed** 2026-08-13 — `server/scripts/repair-orphaned-subject-ids.mjs --commit`. Subject inferred from each record's reviewed canonical `SYS-*` placement via `CURRICULUM_SYSTEM_CROSSWALK`, never from a title. 62 articles and all 736 concepts repaired; 14 resource records deliberately untouched, since resources are not grouped by subject. Both documents versioned into `app_state_versions` first | Repair under a documented migration (LD-06). Each system's `INVENTORY-001` counts its share |
