<!--
  MU-MED102 - one citation per claim in evidence/MU-MED102-claims.md, each
  pointing at the resource record in evidence/MU-MED102-resources.md. The
  only MU-MED102 resource this dispatch has is the department-keyed exam
  itself (End Foundation 2 Batch 43 - Answers), not a department book, so
  every support_span here quotes the item's own stem/key as extracted by
  pagetext.mjs, not a textbook's explanatory prose; the broader teaching
  definition on each concept fills in standard curriculum knowledge around
  that terse exam key, consistent with each claim's own confidence and each
  concept's evidence_gaps note.
-->

# Item

## id
CIT-INF-940EB35DE56BEE
