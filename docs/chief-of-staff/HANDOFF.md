...
STILL NEEDS OMAR: `npm run medical:snapshot-live` with his super_admin+MFA token — gates still judge "live" against the Aug-12 fixture, which the DB write did NOT update. Glossary + academic were NOT in this import (admin-UI Bulk Import). AFTER: apply AU pending-live per `docs/Alexandria-Source-Imports/pending-live/INDEX.md` (now unblocked — its Kasr concept-file dependencies are live); then Omar bulk-tags and works Media Requests.
