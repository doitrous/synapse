...
const requiredMigrationId = '2026-08-11-medical-library-reader-quality-v7'
...
try {
  ...
  if (applied.length) {
    ...
    const snapshot = {
      ...
      scope: [...expectedStateKeys],
      tables: { app_state: [] },
      note: 'Reader-quality release recovery point; only the two state documents changed by this migration are included.',
    }
    const [snapshotStates] = await conn.query('SELECT k, v, updated_at FROM app_state WHERE k IN (?)', [[...expectedStateKeys]])
    snapshot.tables.app_state = snapshotStates
