...
-- ---------------------------------------------------------------------------
-- Back-compat for installs older than this file. Every column/index below is
-- already inline in the CREATE TABLE statements above, so on a fresh database
-- `CREATE TABLE IF NOT EXISTS` already created them and these are no-ops. The
-- production database predates several of these columns, and `CREATE TABLE IF
-- NOT EXISTS` cannot add a column to a table that already exists — these were
-- previously applied every boot as ad-hoc, information_schema-guarded ALTERs
-- in db.js migrate(); converted here to plain idempotent SQL so this file is
-- a no-op against that database. Two widenings could not be converted (MODIFY
-- COLUMN has no "IF" form) and remain as guarded JS steps in db.js migrate().
-- ---------------------------------------------------------------------------

ALTER TABLE user_access ADD COLUMN IF NOT EXISTS mfa_required BOOLEAN NOT NULL DEFAULT 0 AFTER status;
ALTER TABLE user_access ADD COLUMN IF NOT EXISTS content_scope JSON NULL AFTER mfa_required;

ALTER TABLE students ADD COLUMN IF NOT EXISTS phone VARCHAR(32) NULL AFTER email;
ALTER TABLE students ADD COLUMN IF NOT EXISTS nationality VARCHAR(64) NULL AFTER phone;
ALTER TABLE students ADD UNIQUE INDEX IF NOT EXISTS students_phone_unique (phone);

ALTER TABLE user_documents ADD COLUMN IF NOT EXISTS file_name VARCHAR(255) NULL AFTER media_type;
ALTER TABLE user_documents ADD COLUMN IF NOT EXISTS mime_type VARCHAR(128) NULL AFTER file_name;

ALTER TABLE device_tokens ADD COLUMN IF NOT EXISTS web_endpoint TEXT NULL;
ALTER TABLE device_tokens ADD COLUMN IF NOT EXISTS web_p256dh VARCHAR(255) NULL;
ALTER TABLE device_tokens ADD COLUMN IF NOT EXISTS web_auth VARCHAR(255) NULL;

ALTER TABLE study_party_members ADD COLUMN IF NOT EXISTS seat_desk VARCHAR(16) NULL;
ALTER TABLE study_party_members ADD COLUMN IF NOT EXISTS seat_device VARCHAR(16) NULL;
ALTER TABLE study_party_members ADD COLUMN IF NOT EXISTS seat_chair VARCHAR(16) NULL;
ALTER TABLE study_party_members ADD COLUMN IF NOT EXISTS seat_index TINYINT NULL;
ALTER TABLE study_party_members ADD COLUMN IF NOT EXISTS last_active_at DATETIME NULL;
ALTER TABLE study_party_members ADD COLUMN IF NOT EXISTS activity VARCHAR(16) NULL;
ALTER TABLE study_party_members ADD UNIQUE INDEX IF NOT EXISTS study_party_members_seat_unique (party_id, seat_index);
