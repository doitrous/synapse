// Shared render helpers for the HU-GIT-301 authoring lane (pathology, pharmacology,
// parasitology, biochemistry). Copied pattern from the manual's worked examples
// (02-concepts.md, 03-relationships.md, 04-library-articles.md) — not a generic
// importer, just the field order those manuals show, parameterised.
...
function block(key, value) {
  ...
  return `## ${key}\n${value}`.replace(/\n$/, m => m) // keep trailing content as-is
}
...
function fieldNotes(notes) {
  // notes: array of [camelCaseKey, reasonText]
...
export function renderConcept(c) {
  ...
  push('evidence_gaps', 'Evidence must be attached before publication: Helwan GIT-301 pathology-cluster sources are not yet in corpus-source-index.json, so no citation chain could be built this pass.')
  push('owner', 'Helwan Year-3 authoring lane')
  push('reviewer', 'Medical team, Helwan Pathology faculty')
  ...
  const notes = [
    ...
    ...(c.extraNotes || []),
    ['relationships', c.relationshipsNote || 'Same-chapter proximity only this pass; a live-sibling relationship discovery walk under DIS-PAT-T07 is deferred — see WANTED in the lane report.'],
  ]
...
export function renderConceptUpdate(u) {
  // Sparse overlay update: id + label (discriminator, verbatim) + only changed fields.
  const lines = []
  const push = (k, v) => lines.push(block(k, v))
  push('id', u.id)
  push('label', u.label)
  ...
  push('modules', u.modules || ['+HU-GIT-301'])
...
export function renderArticle(a) {
  ...
  push('module', a.module || ['HU-GIT-301'])
  push('module_subject', a.moduleSubject || [])
  push('university_notes', a.universityNotes || `hu: Restricted to HU-GIT-301 Year 3. Sourced from the department pathology MCQ bank triage (scripts/helwan/extract/HU-GIT-301/mcq-bank-pathology.json); no official past-paper key is involved.`)
