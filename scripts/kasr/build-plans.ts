 ...
 *
 *   node --experimental-strip-types scripts/kasr/build-plans.ts "102 INT"
 *
...
import { MODULES, mintConceptId, moduleOf, type Paper, type Seed } from './seeds/types.ts'
...
const STAGE = `scripts/kasr/extract/${slug}`

/** Every paper read for this module. Kept in step with `build-batches.ts`. */
const PAPERS: Record<string, string[]> = {
  '102 INT': [
    `${STAGE}/eoy-2025-199.json`,
    `${STAGE}/eoy-2024-198.json`,
    `${STAGE}/eom-2024-198.json`,
    `${STAGE}/eom-2023-197.json`,
    `${STAGE}/eom-2021-196.json`,
  ],
}

const papers: Paper[] = (PAPERS[module] ?? []).filter(existsSync).map(paperFromJson)
...
function existingArticles(): Map<string, string> {
  ...
  for (const half of ['biochemistry', 'physiology', 'anatomy', 'histology']) {
    const file = `docs/Kasr-Source-Imports/article/${slug}-${half}.md`
    if (!existsSync(file)) continue
...
function existingClaims(): Map<string, string> {
  const found = new Map<string, string>()
  const file = `docs/Kasr-Source-Imports/evidence/${slug}-claims.md`
...
for (const paper of papers) {
  for (const seed of paper.seeds) {
    ...
    byKey.set(seed.key, {
      seed,
      id: mintConceptId(moduleOf(paper.source).id, seed.subject, seed.key, seed.system),
