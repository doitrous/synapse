...
const ARTICLE_BATCHES = (slug: string) => [
  `docs/Kasr-Source-Imports/article/${slug}-biochemistry.md`,
  `docs/Kasr-Source-Imports/article/${slug}-physiology.md`,
  `docs/Kasr-Source-Imports/article/${slug}-anatomy.md`,
  `docs/Kasr-Source-Imports/article/${slug}-histology.md`,
]
...
export function unsupportedClaims(module: string): Set<string> {
  const slug = module.replace(/\s+/g, '-')
  const file = `scripts/kasr/extract/${slug}/claims-without-a-span.txt`
