#!/usr/bin/env python3
"""The 101 ISK end-of-module answers, recovered from the ink rather than the text.

Six end-of-module papers sit under `101 ISK/EOM`. Every one of them is a scan:
...
of the rest.
    ...
    python3 scripts/kasr/extract/eom-answerkey.py             # all six papers
    python3 scripts/kasr/extract/eom-answerkey.py --calibrate # threshold grids
    python3 scripts/kasr/extract/eom-answerkey.py --force     # ignore caches
"""
import argparse
import gzip
import json
...
CACHE = os.path.join(HERE, "eom-renders")
OUT = os.path.join(HERE, "eom-answers.json")
...
PAPERS = {
    ...
    "src_17bf088a37f1ab6540a3": {
        ...
        "sampledFrom": ("the band over option c of question 10 on page 2, modal "
                        "(252,240,241): hue 355, saturation 0.048, value 252; and "
                        "the blue band over option a of question 17 on the same "
                        "page, modal (157,182,220): hue 216, saturation 0.29"),
    },
    # 18/12/2021 sitting, unsolved. Filename says "Answers"; it carries none.
    ...
    # 10/12/2022 sitting, solved. Saturated pink, watermarked scan.
    "src_a54bbf7a625ba2b172fc": {
        "role": "solved",
        "dpi": 200,
        "sat": 0.15, "val": 200,
        "twin": "src_ce4292e31edea7517e7b",
        "markPath": "fill",
        "sampledFrom": ("the band over option a of question 11 on page 2, whose "
                        "pixels run hue 350-360 at saturation 0.20-0.26 and value "
                        "224-255"),
    },
    # 10/12/2022 sitting, unsolved. Filename says 2023; the cover says 10/12/2022.
    ...
    "src_16f747e1171423933757": {
        ...
        "sat": 0.05, "val": 180,
        "markPath": "stroke",
        "sampledFrom": ("nothing: at every threshold in the calibration grid this "
...
def sources():
    ...
    return [s for s in rows if s.get("moduleId") == MODULE
            and s.get("sourceCategory") == CATEGORY]
...
def main():
    ...
    doc = {"moduleId": MODULE, "universityId": "kau", "sourceCategory": CATEGORY,
           "generatedBy": "scripts/kasr/extract/eom-answerkey.py",
           "files": [], "answers": []}
