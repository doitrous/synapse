...
import {
  MODULES, mintConceptId, moduleOf, parseConceptIds, partsKey, resolveConceptId, subjectCollisions, subjectForPath,
 ...
 * not "does that id diverge from a fresh mint".** Most keys a hand-authored
 * file carries were themselves minted with `mintConceptId`, so the pinned id
 * and a fresh mint usually just agree, and that agreement is not this
 * function's business to notice: every caller compares the two itself and
 * only treats a genuine mismatch as a reason to skip minting. 101 ISK's own
 * `101-ISK-practical-concepts.md` shares six canonical_keys with
 * `101-ISK-mcq-concepts.md`, and every one of its ids already agrees with
 * `mintConceptId` — so this index finds all six, and every caller's own
 * comparison correctly still mints and emits the full record for each,
 * because there is nothing to reuse instead of.
 *
 * Cached per module: this build never writes a *hand-authored* file, so the
 * answer cannot change mid-run.
 */
const conceptIdIndexCache = new Map<string, Map<string, string>>()
function existingConceptIds(module: string): Map<string, string> {
  ...
  if (existsSync(dir)) {
    for (const name of readdirSync(dir).filter((one) => one.startsWith(`${fileSlug(module)}-`) && one.endsWith('.md'))) {
      ...
      // The parse itself is `parseConceptIds` in `seeds/types.ts` — pure and
      // file-system-free, which is what makes it a function a test can call
      // directly rather than one that only exists inside this script's own
      // directory-walk.
  ...
  conceptIdIndexCache.set(module, index)
  return index
}
...
function concepts(module: string, papers: Paper[]) {
  ...
  const blocks = [...byKey.values()].map(({ paper, seed, repeats }) => {
    ...
    // — see `existingConceptIds` — is not this build's to redefine. But most
    // keys a hand-authored file already carries were themselves minted with
    // mintConceptId, so the "pinned" id and a fresh mint usually just agree:
    // `resolveConceptId` (seeds/types.ts) treats that as the normal case, not
    // a divergence, and it must still produce the same full record it always
    // has. Only a genuine mismatch — an id mintConceptId could never
    // reproduce, minted by a different tool against a different scheme — is
    // the case this build must not paper over by minting a second, silently
    // rival id for the same key.
    const { id, reused } = resolveConceptId(pinned, module, seed.subject, seed.key, seed.system)
    ...
    if (reused) {
      return conceptUpdateBlock(id, seed.key, [occurrence(paper.source, seed), ...repeats], articleId,
        `canonical_key "${seed.key}" already has a pinned id in this module (${id}), and mintConceptId `
        + `would derive a different one (${mintConceptId(module, seed.subject, seed.key, seed.system)}) — `
        + 'minted outside mintConceptId and not this build\'s to re-mint; see existingConceptIds in '
        + 'build-batches.ts. This row records the paper\'s exam evidence and teaching article only; '
...
async function mcq(module: ModuleRef) {
  ...
  const conceptIdFor = (concept: McqLeafSeed['concepts'][number]) =>
    resolveConceptId(pinned, module.id, concept.subject, concept.key).id
  ...
  for (const { concept, signals, articleIds, stems } of byKey.values()) {
    ...
    // in seeds/types.ts): a pinned id that happens to already agree with a
    // fresh mint is not a divergence, and must still emit the full record it
    // always has. Only a genuine mismatch skips minting.
    const resolved = resolveConceptId(pinned, module.id, concept.subject, concept.key)
    if (resolved.reused) {
      conceptBlocks.push(conceptUpdateBlock(resolved.id, concept.key, [...signals], [...articleIds].join(' | '),
        `canonical_key "${concept.key}" already has a pinned id in this module (${resolved.id}), and `
        + `mintConceptId would derive a different one (${mintConceptId(module.id, concept.subject, concept.key)}) `
        + '— minted outside mintConceptId and not this build\'s to re-mint; see existingConceptIds in '
        + 'build-batches.ts. This row records the question books\' exam evidence and teaching article '
