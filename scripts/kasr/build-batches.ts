 ...
 */
/**
 * The line every batch this generator writes carries in its header, and the
 * only thing that tells its output apart from a hand-authored batch.
 */
...
function removeOrphans(module: string, written: Set<string>) {
  ...
  const orphans = readdirSync(dir)
    ...
    //
    // Scoping the sweep to one module stopped a 101 build deleting 104's and
    // 108's batches, but it cannot stop a *103* build deleting hand-authored
    // *103* files — and four exist, along with one of 108's. Nothing in a
    // filename or a registry distinguishes them: a batch a person wrote and a
    // batch this tool wrote are the same shape in the same directory.
    //
    // What does distinguish them is that every batch this tool writes says so
    // in its own header. So the sweep deletes only files carrying that line.
    // A file without it was written by hand, and "the current run did not
    // produce it" is then the expected state rather than evidence of an
    // orphan.
    //
    // Fails safe in the direction that matters: an unmarked file is never
    // deleted, so the worst case is an orphan surviving a rename — which
    // someone notices — rather than authored work disappearing, which three
    // times nobody did until after it was pushed.
    .filter((name) => readFileSync(`${dir}/${name}`, 'utf8').includes(GENERATED_BY))
