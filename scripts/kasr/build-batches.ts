...
import {
  MODULES, mintConceptId, moduleOf, parseConceptIds, parseConceptLabels, parseConceptModules, partsKey,
  resolveConceptId, subjectCollisions, subjectForPath,
  type BodySystem, type KasrSubject, type ModuleRef, type Paper, type Seed, type SourceRef,
} from './seeds/types.ts'
import {
  batchFile, conceptBlock, conceptUpdateBlock, mcqBlock, mcqConceptBlock, occurrence, writtenBlock,
} from './emit.ts'
...
/**
 * Concept ids a hand-authored file has already pinned, for one module.
 *
 ...
 * the only id a live record can carry. 103 BMS's hand-authored concept files
 * (confirmed across `103-BMS-anatomy-concepts.md`, `-histology-concepts.md`,
 * `-physiology-concepts.md` and `-mcq-carbohydrate-concepts.md`) were minted
 * by a different tool — `Instruction Manual for Content Creation/tools/
 * mint-concept-id.mjs`, which hashes the canonical_key alone, with no module
 * salt — and those ids are pinned: re-minting a live record is a content
 * decision for whoever owns it, never something a build script does on its
 * own. So before this build mints anything, it has to know what the module
 * already calls that key.
 *
 * **Scoped to hand-authored files only.** Anything already carrying
 * `GENERATED_BY` is this pipeline's own prior output — minted by
 * `mintConceptId` in the first place, so looking it up reproduces the exact
 * id `mintConceptId` would mint fresh, and finding it here would only demote
 * every one of a module's own previously-generated concepts to a sparse
 * "update" row on the very next run. That is not a correctness difference
 * (same id either way) but it is a *format* difference, and it would have
 * broken 101 ISK's byte-identity for nothing — 101's own `101-ISK-concepts.md`
 * and `101-ISK-mcq-concepts.md` are two of the five files this glob would
 * otherwise have read.
 *
 * **Scoped to the module a row actually belongs to**, via `module_subject`'s
 * first segment — not just the filename prefix — because the filename only
 * says which module's directory a file sits in, and a stray row filed under
 * the wrong module inside it should not silently donate its id to a key that
 * is not actually this module's.
 *
 * A block with no `## canonical_key` (normal on an update row — the manual's
 * own rule is that an omitted key is untouched, not empty) cannot be indexed
 * by key from this file alone and is skipped; the record that first
 * introduced the key is expected to still carry it somewhere in the module's
 * concept files, which is the only place this index can ever look.
 *
 * **The result here is "what id, if any, this module has already pinned" —
 * every hit is a reuse.** This used to also carry the caveat that a pinned id
 * agreeing with a fresh mint was not "really" a reuse, on the theory that
 * agreement meant there was nothing to protect against. That theory tracked
 * only the id and missed the record behind it: a key with a hand-authored
 * entry already has a full, reviewed concept — aliases, article links, a real
 * `resource_ids` — and a build that mints the id fresh still emits a **full**
 * record for it, which the importer's update mode uses to overwrite every
 * field the hand record carries that this pipeline does not itself populate.
 * Confirmed in 104 CPS: eleven canonical_keys shared with
 * `104-CPS-anatomy-concepts.md`/`-histology-concepts.md`/
 * `-physiology-concepts.md` all mint the *same* id fresh — because 104's hand
 * files were minted with `mintConceptId`, unlike 103 BMS's — and the old rule
 * let every one through as a full re-mint anyway, silently emptying their
 * `aliases`/`relatedArticleIds` and replacing `resource_ids`. `resolveConceptId`
 * (seeds/types.ts) now treats every hit here as a reuse, agreeing or not.
 * 101 ISK's own `101-ISK-practical-concepts.md` shares six canonical_keys with
 * `101-ISK-mcq-concepts.md` this way, and all six now emit as sparse update
 * rows in `101-ISK-mcq-concepts.md` too — a real, deliberate behaviour change
 * from before, not a regression: those six keys already had full hand-authored
 * records, and the generated file was quietly re-defining them.
 *
 * Cached per module: this build never writes a *hand-authored* file, so the
 * answer cannot change mid-run. Three maps share one directory walk and one
 * cache — `ids` (`canonical_key -> id`, what `resolveConceptId` needs),
 * `modules` (`canonical_key -> ## modules`, what a sparse update row needs to
 * decide whether `+modules` has anything new to add), and `labels`
 * (`canonical_key -> ## label`, what a sparse update row restates verbatim to
 * work around `conceptFromRow`'s label-blanking bug — see `parseConceptLabels`
 * in `seeds/types.ts`) — because they read the same rows for the same reason
 * and disagreeing about which files or which rows qualify would be its own
 * bug.
 */
const conceptIdIndexCache = new Map<string, ConceptIdIndex>()
interface ConceptIdIndex { ids: Map<string, string>, modules: Map<string, string[]>, labels: Map<string, string> }
function existingConceptIds(module: string): ConceptIdIndex {
  const cached = conceptIdIndexCache.get(module)
  if (cached) return cached
  const dir = `${OUT}/concept`
  const ids = new Map<string, string>()
  const modules = new Map<string, string[]>()
  const labels = new Map<string, string>()
  if (existsSync(dir)) {
    for (const name of readdirSync(dir).filter((one) => one.startsWith(`${fileSlug(module)}-`) && one.endsWith('.md'))) {
      const text = readFileSync(`${dir}/${name}`, 'utf8')
      if (text.includes(GENERATED_BY)) continue // this pipeline's own output — see above.
      // The parse itself is `parseConceptIds`/`parseConceptModules`/
      // `parseConceptLabels` in `seeds/types.ts` — pure and file-system-free,
      // which is what makes them functions a test can call directly rather
      // than only existing inside this script's own directory-walk.
      for (const [key, id] of parseConceptIds(text, module)) {
        if (!ids.has(key)) ids.set(key, id)
      }
      for (const [key, mods] of parseConceptModules(text, module)) {
        if (!modules.has(key)) modules.set(key, mods)
      }
      for (const [key, label] of parseConceptLabels(text, module)) {
        if (!labels.has(key)) labels.set(key, label)
      }
    }
  }
  const result = { ids, modules, labels }
  conceptIdIndexCache.set(module, result)
  return result
}

/**
 * `field_notes` for a sparse update row, and whether this build's module
 * belongs on `## modules` too.
 *
 * Two shapes, because `resolveConceptId` no longer needs its pinned id to
 * disagree with a fresh mint to call it a reuse: the id can genuinely agree
 * (104 CPS, minted with `mintConceptId` throughout) or genuinely diverge
 * (103 BMS, minted by a different tool). Both are real reuses of an existing
 * hand-authored record and both get a sparse row — only the reason differs,
 * and a reader of the batch should be able to tell which one they are looking
 * at without re-deriving the mint themselves.
 */
function reuseNote(
  key: string, id: string, module: string, subject: KasrSubject, system: BodySystem | undefined, source: string,
  pinned: ConceptIdIndex,
): { note: string, newModule: string | undefined, label: string | undefined } {
  const freshMint = mintConceptId(module, subject, key, system)
  const reason = freshMint === id
    ? `mintConceptId derives the same id fresh (${freshMint}) — the hand-authored record behind it still `
      + 'carries fields (aliases, article links, evidence) this pipeline does not populate, and re-emitting '
      + 'a full record would overwrite every one of them with nothing'
    : `mintConceptId would derive a different one (${freshMint}) — minted outside mintConceptId and not this `
      + 'build\'s to re-mint'
  const note = `canonical_key "${key}" already has a pinned id in this module (${id}); ${reason}; see `
    + `existingConceptIds in build-batches.ts. This row records ${source} evidence and teaching article `
    + 'only; every other field is untouched.'
  // `module_subject`'s first segment already had to match `module` for this
  // key to be found pinned at all — see `parseConceptIds` — so `## modules`
  // is only genuinely new when a hand author's `## modules` list itself
  // fell out of sync with its own `## module_subject`. Real, but rare.
  const newModule = pinned.modules.get(key)?.includes(module) ? undefined : module
  // Restated verbatim, not redefined — see `parseConceptLabels` for why this
  // row cannot simply omit `## label` the way it omits every other
  // descriptive field.
  const label = pinned.labels.get(key)
  return { note, newModule, label }
}
...
function concepts(module: string, papers: Paper[]) {
  ...
  const pinned = existingConceptIds(module)
  ...
  const blocks = [...byKey.values()].map(({ paper, seed, repeats }) => {
    // A key this module has already pinned an id for, outside mintConceptId
    // — see `existingConceptIds` — is not this build's to redefine, whether
    // or not the pinned id happens to also be what mintConceptId would derive
    // fresh: either way, the hand-authored record behind it already carries
    // fields (aliases, article links, evidence) this build does not, and a
    // full re-mint would overwrite them. `resolveConceptId` (seeds/types.ts)
    // now treats every pinned hit as a reuse for exactly that reason.
    const { id, reused } = resolveConceptId(pinned.ids, module, seed.subject, seed.key, seed.system)
    ...
    if (reused) {
      const { note, newModule, label } = reuseNote(
        seed.key, id, module, seed.subject, seed.system, 'the paper\'s exam', pinned)
      return conceptUpdateBlock(id, seed.key, [occurrence(paper.source, seed), ...repeats], articleId,
        note, newModule, label)
    }
...
async function mcq(module: ModuleRef) {
  ...
  // A key this module has already pinned an id for (outside `mintConceptId`
  // — see `existingConceptIds`) is not this build's to redefine, and it is
  // resolved once here rather than separately for the concept block and each
  // question's `main_concept`: those two must never disagree about which id
  // one canonical_key means.
  const pinned = existingConceptIds(module.id)
  const conceptIdFor = (concept: McqLeafSeed['concepts'][number]) =>
    resolveConceptId(pinned.ids, module.id, concept.subject, concept.key).id
  ...
  for (const leaf of leaves) {
    ...
    for (const authored of live) {
      ...
      questionBlocks.push(mcqBlock(row, authored, leaf, module, conceptIdFor(concept)))
    }
  }
  ...
  for (const { concept, signals, articleIds, stems } of byKey.values()) {
    // Same rule as the paper route, and the same function (`resolveConceptId`
    // in seeds/types.ts): every pinned hit is a reuse now, agreeing with a
    // fresh mint or not — the hand-authored record behind it is not this
    // build's to overwrite either way.
    const resolved = resolveConceptId(pinned.ids, module.id, concept.subject, concept.key)
    if (resolved.reused) {
      const { note, newModule, label } = reuseNote(
        concept.key, resolved.id, module.id, concept.subject, undefined, 'the question books\' exam', pinned)
      conceptBlocks.push(conceptUpdateBlock(
        resolved.id, concept.key, [...signals], [...articleIds].join(' | '), note, newModule, label))
      continue
    }
 ...
 */
/**
 * What this run produced goes to stdout; what it skipped, left or removed goes
 * to stderr.
 *
 * `sync.sh` runs the builds with `>/dev/null`, which is right for the routine
 * "261 concepts -> …" lines and was silently swallowing every warning beside
 * them. The orphan report was the worst case, because the argument for leaving
 * orphans in place rather than deleting them rests entirely on the report being
 * seen — through `sync.sh` it was neither deleted nor reported, which is safe
 * and useless. Found by the lane that found the two residual cases before it.
 */
