...
/** The filename prefix this module owns, and the only one the sweep may touch. */
const MODULE_PREFIX = '101-ISK'

const slug = (paper: Paper) =>
  `${MODULE_PREFIX}-${TIER_PREFIX[paper.source.tier]}-${paper.source.sittingYear}`
...
function removeOrphans(written: Set<string>) {
  ...
  const orphans = readdirSync(dir)
    ...
    //
    // `written/` holds other modules' batches now, and this generator only
    // knows about 101 ISK papers — so every 104 CPS and 108 INT file in the
    ...
    // time I pushed the deletion. A sweep that cannot tell "no longer
    // generated" from "generated by someone else" must not be allowed to
    // delete anything.
    //
    // Scoped by the same constant the filenames are built from, so the sweep
    // and the slug cannot drift apart — the registry has been incomplete three
