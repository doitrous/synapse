...
async function mcq() {
   ...
   */
  const byKey = new Map<string, { concept: McqLeafSeed['concepts'][number]; signals: string[]; articleId: string }>()
  ...
  for (const leaf of leaves) {
    ...
    for (const concept of leaf.concepts) {
      ...
      if (found) found.signals.push(...signals)
      else byKey.set(concept.key, { concept, signals: [...signals], articleId: leaf.articleId })
    }
  ...
  for (const { concept, signals, articleId } of byKey.values()) {
    conceptBlocks.push(mcqConceptBlock(concept, [...new Set(signals)], articleId))
  }
