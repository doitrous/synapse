...
const ROOT = process.env.ASU_TOOLCHAIN_OUT ?? 'docs/Ain-Shams-Source-Imports'
const MANIFEST_DIR = process.env.ASU_TOOLCHAIN_MANIFEST_DIR ?? 'docs/Ain-Shams-Source-Imports/manifest'

// Every asu-y<N>-sources.json manifest, merged — Ain Shams has one per year,
// unlike Kasr's single kasr-y1-sources.json.
const manifestFiles = existsSync(MANIFEST_DIR)
  ? readdirSync(MANIFEST_DIR).filter((name) => /^asu-y\d+-sources\.json$/.test(name)).sort()
  : []
const known = new Set<string>(manifestFiles.flatMap((name) =>
  (JSON.parse(readFileSync(`${MANIFEST_DIR}/${name}`, 'utf8')).sources as { sourceId: string }[])
...
for (const path of walk(ROOT)) {
  // The manifests define the IDs; checking one against itself proves nothing.
  if (relative('.', path).startsWith(relative('.', MANIFEST_DIR))) continue
...
against the manifests under ${MANIFEST_DIR}, or regenerate one if the file is genuinely new.`)
  process.exit(1)
}
