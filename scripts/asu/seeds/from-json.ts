/**
 * Copied from `scripts/kasr/seeds/from-json.ts` — this worktree's copy is
 * already the fixed version the shared manual's "Two live defects on main
 * right now" describes: `main`'s 120-line version declares
 * `matchingOptions`/`matchingPrompts` and never turns them into the
 * structured `options`/`matches` pair `emit.ts` reads, so a matching block
 * emitted through it silently loses its options and prompts. This 201-line
 * copy (with `matchingBlock` below) already does the conversion, so nothing
 * to relay here.
 *
...
import {
  MODULES, type BodySystem, type AsuSubject, type Paper, type Scheme, type Seed, type SourceRef,
} from './types.ts'
...
function knownPaths(module: string): Set<string> {
  ...
  // Overridable so the fixture proof's chapter files can live under
  // scripts/asu/fixtures/ instead of the real scripts/asu/extract/ tree.
  const stage = process.env.ASU_TOOLCHAIN_EXTRACT_DIR ?? 'scripts/asu/extract'
  for (const half of ['biochem', 'physio']) {
    const file = `${stage}/${slug}/${half}-chapters.json`
...
export function paperFromJson(file: string): LoadedPaper {
  ...
  const seeds: Seed[] = json.seeds.map((seed) => {
    ...
    return {
      ...seed,
      subject: seed.subject as AsuSubject,
