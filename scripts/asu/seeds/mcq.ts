/**
 * Copied from `scripts/kasr/seeds/mcq.ts` — type definitions only, no seeded
 * data lives here, so the only changes are the subject type name and the
 * paths in the comments below.
 *
 ...
 * `scripts/asu/extract/mcq-bank.json` and is never edited by hand. An author
 ...
 */
import type { AsuSubject } from './types.ts'
...
export interface McqLeafSeed {
  /** The subject-tree leaf, exactly as `ASU-Y1-IMM-structure.md` spells it. */
  leaf: string
  modulePath: string
