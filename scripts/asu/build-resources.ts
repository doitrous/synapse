 ...
 * saying the evidence chain cannot be built because the Ain Shams sources are not in
 ...
 *
 * Copied from `scripts/kasr/build-resources.ts`, retrofitted for `--module`:
 * that file hardcodes `101 ISK` in its manifest filter and its output
 * filename, and reads one manifest file. This version takes the module as
 * `process.argv[2]`, reads every `asu-y<N>-sources.json` manifest present,
 * and writes `<slug>-resources.md`.
 */
...
if (!module || !MODULES[module]) {
  throw new Error(`name the module — one of ${Object.keys(MODULES).join(', ')}`)
}
...
const OUT = `${OUT_ROOT}/evidence/${slug}-resources.md`
...
if (!manifestFiles.length) {
  throw new Error(`no "asu-y<N>-sources.json" manifest found under ${MANIFEST_DIR}.`)
}
const manifest = { sources: manifestFiles.flatMap((name) =>
  (JSON.parse(readFileSync(`${MANIFEST_DIR}/${name}`, 'utf8')) as { sources: ManifestSource[] }).sources) }
// This module only. The manifest indexes the whole university, and a batch
// that quietly carried resources for other modules would be claiming to have
// read them.
const rows = manifest.sources.filter((source) =>
  source.moduleId === module && !source.oldSystemExcluded)
...
## collection_id
asu-${module.toLowerCase().replace(/[^a-z0-9]+/g, '-')}
...
const header = `Every ${module} source as a resource record.

${sources.length} sources, generated from ../manifest/asu-y<N>-sources.json by
scripts/asu/build-resources.ts. Nothing here is typed; rerun it and the batch
is today's manifest.
...
built because the Ain Shams sources are not in the corpus source index — true, and
...
mkdirSync('docs/Ain-Shams-Source-Imports/evidence', { recursive: true })
