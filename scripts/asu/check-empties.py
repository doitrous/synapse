...
"""Whether a deliberately-empty column says so in the form its parser reads.
    ...
    python3 scripts/asu/check-empties.py
    python3 scripts/asu/check-empties.py --self-test
...
IMPORTS = os.environ.get("ASU_TOOLCHAIN_OUT") or os.path.join(REPO, "docs/Ain-Shams-Source-Imports")
# Copied from `scripts/kasr/extract/108-INT/check-empties.py`, which scans
# only that module's own prefix. Ain Shams has many modules across one
# import tree, so this defaults to matching every batch rather than one
# module's — pass --prefix "<module-slug>" (case-insensitive) to scope it,
# the way the 108 INT lane scoped its own copy.
def _prefix_arg():
    argv = sys.argv[1:]
...
MODULE_PREFIX = _prefix_arg().lower()
