...
Copied from `scripts/kasr/extract/pagetext.py` — the shared manual names it
"the worked example and is already correct", so only the manifest lookup
changed (Ain Shams reads one manifest per year, via `asu_module.py`, rather
than Kasr's single file). It takes manifest source IDs, decides native-vs-OCR
from the manifest's own `textLayer` (falling back to whether pdftotext
actually returned anything), and writes `pagetext/<sourceId>.json`.

    python3 scripts/asu/extract/pagetext.py <sourceId> [<sourceId> ...]
    python3 scripts/asu/extract/pagetext.py --module "ASU-CVS" [--tier-max 5]
    python3 scripts/asu/extract/pagetext.py --reprobe        # audit what is already cached
...
def sources():
    return {s["sourceId"]: s for s in asu_module.manifest_sources()}
