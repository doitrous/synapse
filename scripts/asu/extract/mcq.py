#!/usr/bin/env python3
"""Extract MCQs from an Ain Shams module's question books.

Copied from `scripts/kasr/extract/mcq.py`. `--module` is required — Ain
Shams has no default module and no unprefixed legacy path to fall back to,
unlike Kasr's `101 ISK`. Every module writes into extract/<module-slug>/,
caches included, from the first run.

    python3 scripts/asu/extract/mcq.py --module "ASU-CVS" [--category NAME ...]

Which manifest categories hold a module's question books is per-module data
(MODULE_CATEGORIES); `--category` overrides the table for one run.
`MODULE_CATEGORIES`, `MODULE_TARGETS`, `MODULE_ANSWER_PAIRS`,
`MODULE_TOPIC_RULES` and the sibling per-module tables below are still keyed
by Kasr's own module ids (`"101 ISK"`, `"104 CPS"`) — left in place rather
than stripped, because they are inert for any Ain Shams module (a `.get(module,
default)` lookup that never matches) and stripping them would cost more
review time than leaving working code that no Ain Shams run ever reaches.
Add an Ain Shams entry the same way once a module needs one; do not reuse a
Kasr module's key.
...
from asu_module import module_sources, out_dir, out_path, \
    parse_module, report_textlayer_fallback  # noqa: E402

OCR_PAGE_CAP = 40
DPI = 120

# Set from --module in main(). Left as None at import time rather than
# calling out_path() with a placeholder — Kasr's copy safely defaults these
# to `101 ISK`'s real paths because that module is the fallback; Ain Shams
# has no such fallback, and out_path() has the side effect of creating the
# directory it names, so a placeholder here would create a spurious
# directory on every import. Real values are set in main() before anything
# reads them.
MODULE = None
PARTS = None
TEXTCACHE = None
...
def merge_and_write():
    ...
    doc = {
        "generatedFrom": "Kasr Al Ainy corpus, module " + MODULE +
                         ...
                         " (paths resolved via docs/Ain-Shams-Source-Imports/manifest/asu-y<N>-sources.json (all years, merged))",
