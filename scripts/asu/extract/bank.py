...
"""Deduplicate mcq.json into a question bank + write the human report.
    ...
    python3 scripts/asu/extract/bank.py [--module "104 CPS"]
...
def main(argv=()):
    ...
    MODULE, rest = asu_module.parse_module(list(argv))
    if rest:
        raise SystemExit("unexpected arguments: %s" % " ".join(rest))
    SRC = asu_module.out_path(MODULE, "mcq.json")
    BANK = asu_module.out_path(MODULE, "mcq-bank.json")
    REPORT = asu_module.out_path(MODULE, "mcq-report.md")
...
def write_report(doc, out, bank, conflicts, blanks, noopt, match_bank=(), prose=()):
    ...
    L.append("Generated by `scripts/asu/extract/bank.py` from `mcq.json`. "
    ...
    L.append("Leaf assignment is keyword-based against "
             "`docs/Ain-Shams-Source-Imports/academic/%s-structure.md` and is approximate.\n"
