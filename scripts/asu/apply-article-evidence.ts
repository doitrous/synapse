 ...
 *
 *   node --experimental-strip-types scripts/asu/apply-article-evidence.ts "102 INT"
 *
...
const mapPath = `${EXTRACT}/${slug}/article-evidence.json`
...
/**
 * Every article batch for this module, discovered rather than named.
 *
 * Kasr's original hardcodes its four subject-half names
 * (`biochemistry`/`physiology`/`anatomy`/`histology`) — the split of Kasr's
 * own department books. Ain Shams has no fixed subject list per module (the
 * curriculum catalogue carries twenty subjects), so this scans the article
 * directory for every file the module's own concept/written batches already
 * use as a prefix, rather than guessing a subject-half name that may not
 * exist for this module.
 */
const ARTICLE_DIR = process.env.ASU_TOOLCHAIN_OUT ? `${process.env.ASU_TOOLCHAIN_OUT}/article` : 'docs/Ain-Shams-Source-Imports/article'
const BATCHES = (existsSync(ARTICLE_DIR) ? readdirSync(ARTICLE_DIR) : [])
