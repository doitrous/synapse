 ...
 *
 *   node --experimental-strip-types scripts/asu/check-column-parsers.ts \
 *     docs/Ain-Shams-Source-Imports/concept/ASU-CVS-concepts.md
 *
...
const PROBE = 'ASUPROBEALPHA | ASUPROBEBETA'
...
for (const field of CONCEPT_IMPORT_FIELDS) {
  ...
  const value = Object.values(stored).find((one) =>
    (Array.isArray(one) && one.some((item) => String(item).includes('ASUPROBE')))
    || (typeof one === 'string' && one.includes('ASUPROBE')))
