...
function expectedTriageState(readiness, triage) {
  ...
  const remainingChecksum = '434ab87810c8663cd9baf000a487141a2dbd70d766f8ab35a01f01b06389150f'
...
function runAuditLabelStaticTest() {
  ...
  if (parsed.triageCheckpointRemainingExtractionDebt.emptyTextRows !== 11 || parsed.triageCheckpointRemainingExtractionDebt.sparseTextRows !== 4 || parsed.triageCheckpointRemainingExtractionDebt.substantiveTextRows !== 0) throw new Error('Audit-label static test: triage debt drift')
  console.log('audit-label-static-test=pass forbidden-temporal-text-labels=absent audit_sample_labels=present triage_debt=0-substantive/11-empty/4-sparse')
}
...
if (remainingHashes.length !== 15 || processedHashes.length + remainingHashes.length !== selectedHashes.length) throw new Error('Processed and remaining hashes do not reconcile to 106')
...
const provenance = {
  ...
  triageCheckpointRemainingExtractionDebt: { substantiveTextRows: 0, emptyTextRows: 11, sparseTextRows: 4, source: 'pinned triage/readiness checkpoint; not replaced by audit-sample status counts' },
