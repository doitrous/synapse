...
function expectedTriageState(readiness, triage) {
  ...
  const remainingChecksum = '05371a25dda0d2f9b6b6ae508b59d8784053537827fdeb2ae56b84ca5f764c69'
...
function runAuditLabelStaticTest() {
  ...
  if (parsed.triageCheckpointRemainingExtractionDebt.emptyTextRows !== 37 || parsed.triageCheckpointRemainingExtractionDebt.sparseTextRows !== 6 || parsed.triageCheckpointRemainingExtractionDebt.substantiveTextRows !== 13) throw new Error('Audit-label static test: triage debt drift')
  console.log('audit-label-static-test=pass forbidden-temporal-text-labels=absent audit_sample_labels=present triage_debt=13-substantive/37-empty/6-sparse')
}
...
if (remainingHashes.length !== 54 || processedHashes.length + remainingHashes.length !== selectedHashes.length) throw new Error('Processed and remaining hashes do not reconcile to 106')
...
const provenance = {
  ...
  triageCheckpointRemainingExtractionDebt: { substantiveTextRows: 13, emptyTextRows: 37, sparseTextRows: 6, source: 'pinned triage/readiness checkpoint; not replaced by audit-sample status counts' },
