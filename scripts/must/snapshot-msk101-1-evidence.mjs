...
const REMAINING_CHECKSUM = '40e20d3a619018b1ecc7318536ee00734aebcc27b79def4069242511fe33efb2'
...
function runAuditLabelStaticTest() {
  ...
  const expected = { 'audit-extract-failed': 3, 'audit-not-found': 14, 'empty-text': 14, 'sparse-text': 8, 'substantive-text': 44 }
  ...
  console.log('audit-label-static-test=pass remaining_audit_debt=44-substantive/8-sparse/14-empty/14-not-found/3-extract-failed')
}
...
for (const value of [...PROCESSED_FAMILY_HASHES, SELECTED_CHECKSUM, REMAINING_CHECKSUM, '1368', '1298', '27']) {
...
if (processedHashes.length !== 20 || remainingHashes.length !== 81 || processedHashes.length + remainingHashes.length !== 101) throw new Error('Processed/remaining reconciliation drift')
...
const expectedRemainingDebt = { 'audit-extract-failed': 3, 'audit-not-found': 14, 'empty-text': 14, 'sparse-text': 8, 'substantive-text': 44 }
...
const provenance = {
  ...
  triageCumulative: { printedPromptObservations: 1368, printedKeyObservations: 1298, namedConceptsAssigned: 27, liveHits: 0, pendingHits: 1, newConcepts: 26 },
