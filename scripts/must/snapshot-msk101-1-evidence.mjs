...
const REMAINING_CHECKSUM = '0ca647ac43a558bbbeb18bbdd8ade773ef780c4734fc046949f1a3b142754be4'
...
function runAuditLabelStaticTest() {
  ...
  const expected = { 'audit-extract-failed': 3, 'audit-not-found': 14, 'empty-text': 19, 'sparse-text': 8, 'substantive-text': 55 }
  ...
  console.log('audit-label-static-test=pass remaining_audit_debt=55-substantive/8-sparse/19-empty/14-not-found/3-extract-failed')
}
...
for (const value of [...PROCESSED_FAMILY_HASHES, SELECTED_CHECKSUM, REMAINING_CHECKSUM, '261', '251', '14']) {
...
if (processedHashes.length !== 4 || remainingHashes.length !== 97 || processedHashes.length + remainingHashes.length !== 101) throw new Error('Processed/remaining reconciliation drift')
...
const expectedRemainingDebt = { 'audit-extract-failed': 3, 'audit-not-found': 14, 'empty-text': 19, 'sparse-text': 8, 'substantive-text': 55 }
...
const provenance = {
  ...
  triageCumulative: { printedPromptObservations: 261, printedKeyObservations: 251, namedConceptsAssigned: 14, liveHits: 0, pendingHits: 1, newConcepts: 13 },
