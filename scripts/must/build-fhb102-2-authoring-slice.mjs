...
| Kind | Created | Status |
|---|---:|---|
| Evidence resources | 6 | local-only source records |
| Claims | 6 | verified against local curriculum citations; independent review still owed |
| Citations | 9 | local curriculum |
| Article spans | 6 | linked to the clean-key claims |
| Concepts | 6 | under review / needs evidence |
| Articles | 6 | Draft |
| Questions | 6 | Draft |
| Question key-conflict holds | 4 | no student-facing record authored |

The first slice authored Q1 with printed key D unchanged. The second slice authored Q5 with printed key C unchanged. The third slice authored Q7 with printed key B unchanged. The fourth slice authored Q8 with printed key A unchanged. The fifth slice authored Q9 with printed key C unchanged. The sixth slice authors Q10 with printed key A unchanged: the MUST mosquito-control lecture directly defines paratransgenesis through symbiont bacteria. No answer override was made in any question.
...
## Exact backlog after six slices

- Governed prompt observations: 5,444 total; 6 authored; **5,438 raw prompt observations remain**, including the 4 explicit holds.
- Governed answer observations: 5,211 total; 6 clean source-keyed prompts authored; **5,205 raw answer observations remain**, including the 4 held printed-key observations.
- Opening 32-prompt normalized family: 6 authored, 4 held, and **22 not yet assessed for authoring**; therefore **26 prompts remain in the authoring backlog**, of which Q32 is source-absent.
- Record-level backlog is not asserted as 5,438 unique records: repeated and near-repeated prompts must still be deduplicated during authoring, per the one-question-one-record rule.

## Upload state

No MUST content has been uploaded or imported. All six student-facing articles and questions remain Draft; all six concepts use the concept schema's non-published \`under review\` state and \`publication_status: needs_evidence\`. Q2, Q3, Q4 and Q6 exist only as authoring-ledger holds.
`
}
