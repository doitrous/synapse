...
const REMAINING_CHECKSUM = '2057487d48492ce0efaeb4ed5089bfd96f4dfaa40ebfb1fb81916afcab202e2d'
const FIRST_SOURCE = {
...
function runAuditLabelStaticTest() {
  ...
  const expected = { 'audit-extract-failed': 1, 'audit-not-found': 11, 'empty-text': 10, 'sparse-text': 15, 'substantive-text': 24 }
  ...
  console.log('audit-label-static-test=pass remaining_audit_debt=24-substantive/15-sparse/10-empty/11-not-found/1-extract-failed')
}
...
if (processedHashes.length !== 33 || remainingHashes.length !== 61 || processedHashes.length + remainingHashes.length !== 94) throw new Error('Processed/remaining reconciliation drift')
...
const expectedRemainingDebt = { 'audit-extract-failed': 1, 'audit-not-found': 11, 'empty-text': 10, 'sparse-text': 15, 'substantive-text': 24 }
...
const provenance = {
  ...
  triageCumulative: { printedPromptObservations: 2727, printedKeyObservations: 2571, namedConceptsAssigned: 62, liveHits: 0, pendingHits: 0, newConcepts: 62 },
