...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.equal(items(questions).length, 116)
  assert.equal(items(articles).length, 19)
  assert.equal(items(concepts).length, 78)
  assert.equal(items(claims).length, 116)
  assert.equal(items(citations).length, 116)
  assert.equal(items(spans).length, 116)
  ...
  assert.match(coverage, /\| Questions \| 219 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 236 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 219 \|/)
  assert.match(coverage, /\| Citations \| 224 \|/)
  assert.match(coverage, /\| Article spans \| 219 \|/)
  assert.match(coverage, /\| Concepts \| 156 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 219 authored; \*\*5,225 raw prompt observations remain\*\*, including 236 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 219 clean source-keyed prompts authored; \*\*4,992 raw answer observations remain\*\*, including the 236 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 219 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 236 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 219 authored; \*\*5,225 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 219 clean source-keyed prompts authored; \*\*4,992 raw answer observations remain\*\*/)
})
...
test('Mucize Parasitology core Q31-Q60 emits sixteen governed Draft questions and fourteen exact holds', () => {
  ...
  assert.equal(items(concepts).length, 78)
  assert.equal(items(articles).length, 19)
  assert.match(sources, /Physical pages 6–19 were rendered and visually re-read/)
...
test('Mucize Parasitology core Q61-Q90 emits eighteen governed Draft questions and twelve exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 219 authored; \*\*5,225 raw prompt observations remain\*\*, including 236 explicit holds and two source-absent prompts/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 219 clean source-keyed prompts authored; \*\*4,992 raw answer observations remain\*\*, including the 236 held printed-key observations/)
  assert.match(coverage, /\| Evidence resources \| 29 \|/)
  assert.match(coverage, /\| Claims \| 219 \|/)
  assert.match(coverage, /\| Citations \| 224 \|/)
...
test('Mucize Parasitology core Q91-Q95 emits four governed Draft questions and one exact hold', () => {
  ...
  assert.match(coverage, /exact next boundary is Mucize case-based learning Case 1 on physical page 20/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 219 authored; \*\*5,225 raw prompt observations remain\*\*, including 236 explicit holds and two source-absent prompts/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 219 clean source-keyed prompts authored; \*\*4,992 raw answer observations remain\*\*, including the 236 held printed-key observations/)
  assert.match(coverage, /\| Evidence resources \| 29 \|/)
  assert.match(coverage, /\| Claims \| 219 \|/)
