...
test('the approved Absalam introduction and two Arthropoda slices emit thirty clean Draft questions and thirty ledger holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 47 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 74 \| no student-facing record authored \|/)
  ...
  assert.match(coverage, /global Q61–Q90 are fully dispositioned/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.equal(items(questions).length, 9)
  assert.equal(items(articles).length, 2)
  assert.equal(items(concepts).length, 8)
  assert.equal(items(claims).length, 9)
  assert.equal(items(citations).length, 9)
  assert.equal(items(spans).length, 9)
  ...
  assert.equal((questions.match(/^## status\nDraft$/gm) || []).length, 9)
  assert.equal((articles.match(/^## status\nDraft$/gm) || []).length, 2)
  assert.equal((concepts.match(/^## status\nunder review$/gm) || []).length, 8)
  assert.equal((concepts.match(/^## publication_status\nneeds_evidence$/gm) || []).length, 8)
  ...
  assert.match(coverage, /\| Claims \| 47 \|/)
  assert.match(coverage, /\| Citations \| 52 \|/)
  assert.match(coverage, /\| Article spans \| 47 \|/)
  assert.match(coverage, /\| Concepts \| 43 \|/)
