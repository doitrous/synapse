...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.equal(items(questions).length, 126)
  assert.equal(items(articles).length, 19)
  assert.equal(items(concepts).length, 85)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
  assert.equal(items(spans).length, 126)
  ...
  assert.match(coverage, /\| Questions \| 229 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 281 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 229 \|/)
  assert.match(coverage, /\| Citations \| 234 \|/)
  assert.match(coverage, /\| Article spans \| 229 \|/)
  assert.match(coverage, /\| Concepts \| 163 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 229 authored; \*\*5,215 raw prompt observations remain\*\*, including 281 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 229 clean source-keyed prompts authored; \*\*4,982 raw answer observations remain\*\*, including the 281 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 229 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 281 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 229 authored; \*\*5,215 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 229 clean source-keyed prompts authored; \*\*4,982 raw answer observations remain\*\*/)
})
...
test('Mucize Parasitology core Q31-Q60 emits sixteen governed Draft questions and fourteen exact holds', () => {
  ...
  assert.equal(items(concepts).length, 85)
  assert.equal(items(articles).length, 19)
  assert.match(sources, /Physical pages 6–28 were rendered and visually re-read/)
...
test('Mucize Parasitology core Q61-Q90 emits eighteen governed Draft questions and twelve exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 229 authored; \*\*5,215 raw prompt observations remain\*\*, including 281 explicit holds and two source-absent prompts/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 229 clean source-keyed prompts authored; \*\*4,982 raw answer observations remain\*\*, including the 281 held printed-key observations/)
  assert.match(coverage, /\| Evidence resources \| 29 \|/)
  assert.match(coverage, /\| Claims \| 229 \|/)
  assert.match(coverage, /\| Citations \| 234 \|/)
...
test('Mucize case-based learning Case 2 records all three repeated identities as explicit holds', () => {
  ...
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
  assert.equal(items(claims).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
  assert.equal(items(citations).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
  assert.equal(items(spans).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
...
test('Mucize Advanced MCQ Q23 remains an exact governed-identity and prior-family duplicate hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q23\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q23\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q23\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q23\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q23: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q23 B, They undergo complete metamorphosis with egg, larva, pupa, and adult stages/)
  assert.match(coverage, /CON-INF-FABEF04D8E14A7/)
  assert.match(coverage, /Advanced Q1–Q23 cumulative: \*\*4 authored \/ 19 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q24 on physical page 27/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q24 remains an exact governed-identity and prior-family duplicate hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q24\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q24\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q24\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q24\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q24: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q24 B, Introducing natural predators like Gambusia fish/)
  assert.match(coverage, /CON-INF-863856AB7138F8/)
  assert.match(coverage, /Advanced Q1–Q24 cumulative: \*\*4 authored \/ 20 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q25 on physical page 27/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q25 remains an exact governed-identity and same-source duplicate hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q25\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q25\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q25\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q25\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q25: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q25 B, Only female mosquitoes feed on blood, while males feed on plant nectar/)
  assert.match(coverage, /CON-INF-2D0A5960736100/)
  assert.match(coverage, /Advanced Q1–Q25 cumulative: \*\*4 authored \/ 21 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q26 on physical page 27/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q26 authors the non-volatile-oil chemical control identity', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.match(questions, /QST-MUST-FHB1022-PARA-MUCIZEADV-Q26/)
  assert.match(questions, /What is the primary method of chemical control for mosquito larvae\?/)
  assert.match(questions, /Spraying non-volatile oils on water surfaces to suffocate larvae/)
  assert.match(questions, /## correct_answer\nB/)
  assert.match(questions, /## main_concept\nCON-INF-C9A3D62F58B417/)
  assert.match(concepts, /CON-INF-C9A3D62F58B417/)
  assert.match(concepts, /Non-volatile oils suffocate mosquito aquatic stages as chemical control/)
  assert.match(articles, /ART-INF-MUST-FHB1022-MOSQUITO-CONTROL/)
  assert.match(claims, /CLM-INF-MUST-FHB1022-MUCIZEADV-Q26-01/)
  assert.match(citations, /CIT-INF-MUST-FHB1022-MUCIZEADV-Q26-01/)
  assert.match(spans, /SPN-INF-MUST-FHB1022-MUCIZEADV-Q26-01/)
  assert.match(coverage, /Mucize Advanced MCQ Q26: \*\*1 authored \/ 0 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Advanced Q1–Q26 cumulative: \*\*5 authored \/ 21 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q27 on physical page 27/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q27 remains an exact governed-identity and same-source duplicate hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q27\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q27\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q27\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q27\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q27: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q27 B, Mosquitoes typically fly up to 2 kilometers, but wind can extend this to 10 kilometers/)
  assert.match(coverage, /CON-INF-E7CE29355ADA44/)
  assert.match(coverage, /Advanced Q1–Q27 cumulative: \*\*5 authored \/ 22 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q28 on physical page 27/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q28 authors the nets-and-repellents personal-protection identity', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.match(questions, /QST-MUST-FHB1022-PARA-MUCIZEADV-Q28/)
  assert.match(questions, /What is the primary method of preventing mosquito-borne diseases in endemic areas\?/)
  assert.match(questions, /Using mosquito nets and insect repellents/)
  assert.match(questions, /## correct_answer\nB/)
  assert.match(questions, /## main_concept\nCON-INF-D74B6E15A2C938/)
  assert.match(concepts, /CON-INF-D74B6E15A2C938/)
  assert.match(concepts, /Mosquito nets and insect repellents are personal-protection measures in endemic areas/)
  assert.match(articles, /ART-INF-MUST-FHB1022-MOSQUITO-CONTROL/)
  assert.match(claims, /CLM-INF-MUST-FHB1022-MUCIZEADV-Q28-01/)
  assert.match(citations, /CIT-INF-MUST-FHB1022-MUCIZEADV-Q28-01/)
  assert.match(spans, /SPN-INF-MUST-FHB1022-MUCIZEADV-Q28-01/)
  assert.match(coverage, /Mucize Advanced MCQ Q28: \*\*1 authored \/ 0 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Advanced Q1–Q28 cumulative: \*\*6 authored \/ 22 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q29 on physical page 27/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q29 authors the Aedes small-container breeding identity', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.match(questions, /QST-MUST-FHB1022-PARA-MUCIZEADV-Q29/)
  assert.match(questions, /Which of the following is a key feature of the breeding habits of Aedes mosquitoes\?/)
  assert.match(questions, /They can breed in small water collections, such as flower vases or buckets/)
  assert.match(questions, /## correct_answer\nB/)
  assert.match(questions, /## main_concept\nCON-INF-F16A82C4E9D375/)
  assert.match(concepts, /CON-INF-F16A82C4E9D375/)
  assert.match(concepts, /Aedes and related mosquitoes breed in small artificial water collections/)
  assert.match(articles, /ART-INF-MUST-FHB1022-MOSQUITO-BIOLOGY/)
  assert.match(claims, /CLM-INF-MUST-FHB1022-MUCIZEADV-Q29-01/)
  assert.match(citations, /CIT-INF-MUST-FHB1022-MUCIZEADV-Q29-01/)
  assert.match(spans, /SPN-INF-MUST-FHB1022-MUCIZEADV-Q29-01/)
  assert.match(coverage, /Mucize Advanced MCQ Q29: \*\*1 authored \/ 0 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Advanced Q1–Q29 cumulative: \*\*7 authored \/ 22 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q30 on physical page 28/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q30 remains an established cyclopropagative dependency-chain hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q30\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q30\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q30\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q30\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q30: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q30 B, They allow pathogens to develop or multiply within them before transmission/)
  assert.match(coverage, /\*\*Mucize Advanced MCQ Q30 is held as an established cyclopropagative\/dependency-chain hold/)
  assert.match(coverage, /Advanced Q1–Q30 cumulative: \*\*7 authored \/ 23 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q31 on physical page 28/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q31 remains an exact governed-identity and same-source duplicate hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q31\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q31\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q31\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q31\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q31: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q31 B, Only female sandflies feed on blood, while males feed on plant nectar/)
  assert.match(coverage, /CON-INF-93E9B181184A6B/)
  assert.match(coverage, /Advanced Q1–Q31 cumulative: \*\*7 authored \/ 24 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q32 on physical page 28/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q32 remains an exact governed-identity and prior-family duplicate hold', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q32\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q32\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q32\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q32\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q32: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q32 B, They are weak fliers and tend to hop short distances/)
  assert.match(coverage, /CON-INF-509BF395C5EBF9/)
  assert.match(coverage, /Advanced Q1–Q32 cumulative: \*\*7 authored \/ 25 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /exact next boundary is Mucize Advanced MCQ Q33 on physical page 28/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
...
test('Mucize Advanced MCQ Q33 remains an exact governed-identity and same-source duplicate hold, closing the Advanced family', () => {
  execFileSync(process.execPath, [generator], { cwd: root, stdio: 'pipe' })
  ...
  assert.doesNotMatch(questions, /MUCIZEADV(?:ANCED)?-Q33\b/)
  assert.doesNotMatch(claims, /MUCIZEADV(?:ANCED)?-Q33\b/)
  assert.doesNotMatch(citations, /MUCIZEADV(?:ANCED)?-Q33\b/)
  assert.doesNotMatch(spans, /MUCIZEADV(?:ANCED)?-Q33\b/)
  assert.match(coverage, /Mucize Advanced MCQ Q33: \*\*0 authored \/ 1 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Q33 B, They are transmitted through the bite of infected sandflies/)
  assert.match(coverage, /CON-INF-F25D45A3C5F542/)
  assert.match(coverage, /Advanced Q1–Q33 cumulative: \*\*7 authored \/ 26 held \/ 0 source-absent \/ 0 unassessed\*\*/)
  assert.match(coverage, /Advanced MCQs Q1–Q33 are now fully dispositioned; the next source boundary is Mucize Pharmacology — Antibacterial \(1\), Q1, physical page 29, pending a fresh audit/)
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q33 authoring audit/)
  assert.match(sources, /Advanced MCQs Q1–Q33 are now fully dispositioned/)
  assert.equal(items(questions).length, 126)
  assert.equal(items(claims).length, 126)
  assert.equal(items(citations).length, 126)
  assert.equal(items(spans).length, 126)
