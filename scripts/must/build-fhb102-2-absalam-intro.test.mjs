...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.match(coverage, /\| Questions \| 151 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 180 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.equal(items(questions).length, 72)
  assert.equal(items(articles).length, 14)
  assert.equal(items(concepts).length, 60)
  assert.equal(items(claims).length, 72)
  assert.equal(items(citations).length, 72)
  assert.equal(items(spans).length, 72)
  ...
  assert.equal((questions.match(/^## status\nDraft$/gm) || []).length, 72)
  assert.equal((articles.match(/^## status\nDraft$/gm) || []).length, 14)
  assert.equal((concepts.match(/^## status\nunder review$/gm) || []).length, 60)
  assert.equal((concepts.match(/^## publication_status\nneeds_evidence$/gm) || []).length, 60)
  ...
  assert.match(coverage, /\| Claims \| 151 \|/)
  assert.match(coverage, /\| Citations \| 156 \|/)
  assert.match(coverage, /\| Article spans \| 151 \|/)
  assert.match(coverage, /\| Concepts \| 129 \|/)
  assert.match(coverage, /\| Articles \| 41 \|/)
  assert.match(coverage, /\| Questions \| 151 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 180 \| no student-facing record authored \|/)
...
test('global Q91-Q120 emits only the ten approved Chapter 6 Draft questions and preserves all twenty holds', () => {
  ...
  assert.equal(sourceIndex.count, 27)
...
test('global Q121-Q150 emits exactly nineteen approved Pharmacology Draft questions and preserves eleven holds', () => {
  ...
  assert.match(coverage, /\| Evidence resources \| 27 \|/)
  assert.match(coverage, /\| Claims \| 151 \|/)
  assert.match(coverage, /\| Citations \| 156 \|/)
  assert.match(coverage, /\| Article spans \| 151 \|/)
  assert.match(coverage, /\| Concepts \| 129 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 151 authored; \*\*5,293 raw prompt observations remain\*\*, including 180 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 151 clean source-keyed prompts authored; \*\*5,060 raw answer observations remain\*\*, including the 180 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 151 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 180 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 151 authored; \*\*5,293 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 151 clean source-keyed prompts authored; \*\*5,060 raw answer observations remain\*\*/)
})
