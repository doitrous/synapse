...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.equal(items(questions).length, 78)
  assert.equal(items(articles).length, 19)
  assert.equal(items(concepts).length, 61)
  assert.equal(items(claims).length, 78)
  assert.equal(items(citations).length, 78)
  assert.equal(items(spans).length, 78)
  ...
  assert.match(coverage, /\| Questions \| 181 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 209 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 181 \|/)
  assert.match(coverage, /\| Citations \| 186 \|/)
  assert.match(coverage, /\| Article spans \| 181 \|/)
  assert.match(coverage, /\| Concepts \| 139 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 181 authored; \*\*5,263 raw prompt observations remain\*\*, including 209 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 181 clean source-keyed prompts authored; \*\*5,030 raw answer observations remain\*\*, including the 209 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 181 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 209 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 181 authored; \*\*5,263 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 181 clean source-keyed prompts authored; \*\*5,030 raw answer observations remain\*\*/)
})
...
test('Mucize Parasitology core Q1-Q30 emits eight exact-key Draft questions, twenty-one holds and one source-absent prompt', () => {
  ...
  assert.equal(items(questions).filter((item) => /MUCIZE-Q/.test(item)).length, 8)
  assert.equal(items(claims).filter((item) => /MUCIZE-Q/.test(item)).length, 8)
  assert.equal(items(citations).filter((item) => /MUCIZE-Q/.test(item)).length, 8)
  assert.equal(items(spans).filter((item) => /MUCIZE-Q/.test(item)).length, 8)
  assert.equal(items(concepts).length, 61)
  assert.equal(items(articles).length, 19)
