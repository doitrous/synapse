...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.equal(items(questions).length, 112)
  assert.equal(items(articles).length, 19)
  assert.equal(items(concepts).length, 75)
  assert.equal(items(claims).length, 112)
  assert.equal(items(citations).length, 112)
  assert.equal(items(spans).length, 112)
  ...
  assert.match(coverage, /\| Questions \| 215 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 235 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 215 \|/)
  assert.match(coverage, /\| Citations \| 220 \|/)
  assert.match(coverage, /\| Article spans \| 215 \|/)
  assert.match(coverage, /\| Concepts \| 153 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 215 authored; \*\*5,229 raw prompt observations remain\*\*, including 235 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 215 clean source-keyed prompts authored; \*\*4,996 raw answer observations remain\*\*, including the 235 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 215 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 235 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 215 authored; \*\*5,229 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 215 clean source-keyed prompts authored; \*\*4,996 raw answer observations remain\*\*/)
})
...
test('Mucize Parasitology core Q1-Q30 emits eight exact-key Draft questions, twenty-one holds and one source-absent prompt', () => {
  ...
  assert.equal(items(questions).filter((item) => /MUCIZE-Q/.test(item)).length, 42)
  assert.equal(items(claims).filter((item) => /MUCIZE-Q/.test(item)).length, 42)
  assert.equal(items(citations).filter((item) => /MUCIZE-Q/.test(item)).length, 42)
  assert.equal(items(spans).filter((item) => /MUCIZE-Q/.test(item)).length, 42)
  assert.equal(items(concepts).length, 75)
  assert.equal(items(articles).length, 19)
...
test('Mucize Parasitology core Q31-Q60 emits sixteen governed Draft questions and fourteen exact holds', () => {
  ...
  assert.equal(items(concepts).length, 75)
  assert.equal(items(articles).length, 19)
  assert.match(sources, /Physical pages 6–18 were rendered and visually re-read/)
...
test('Mucize Parasitology core Q61-Q90 emits eighteen governed Draft questions and twelve exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 215 authored; \*\*5,229 raw prompt observations remain\*\*, including 235 explicit holds and two source-absent prompts/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 215 clean source-keyed prompts authored; \*\*4,996 raw answer observations remain\*\*, including the 235 held printed-key observations/)
  assert.match(coverage, /\| Evidence resources \| 29 \|/)
  assert.match(coverage, /\| Claims \| 215 \|/)
  assert.match(coverage, /\| Citations \| 220 \|/)
