...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.match(coverage, /\| Questions \| 197 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 223 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 197 \|/)
  assert.match(coverage, /\| Citations \| 202 \|/)
  assert.match(coverage, /\| Article spans \| 197 \|/)
  assert.match(coverage, /\| Concepts \| 148 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 197 authored; \*\*5,247 raw prompt observations remain\*\*, including 223 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 197 clean source-keyed prompts authored; \*\*5,014 raw answer observations remain\*\*, including the 223 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 197 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 223 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 197 authored; \*\*5,247 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 197 clean source-keyed prompts authored; \*\*5,014 raw answer observations remain\*\*/)
})
...
test('Mucize Parasitology core Q1-Q30 emits eight exact-key Draft questions, twenty-one holds and one source-absent prompt', () => {
  ...
  assert.equal(items(questions).filter((item) => /MUCIZE-Q/.test(item)).length, 24)
  assert.equal(items(claims).filter((item) => /MUCIZE-Q/.test(item)).length, 24)
  assert.equal(items(citations).filter((item) => /MUCIZE-Q/.test(item)).length, 24)
  assert.equal(items(spans).filter((item) => /MUCIZE-Q/.test(item)).length, 24)
  assert.equal(items(concepts).length, 70)
  assert.equal(items(articles).length, 19)
...
test('Mucize Parasitology core Q31-Q60 emits sixteen governed Draft questions and fourteen exact holds', () => {
  ...
  assert.equal(items(concepts).length, 70)
  assert.equal(items(articles).length, 19)
  assert.match(sources, /Physical pages 6–14 were rendered and visually re-read/)
