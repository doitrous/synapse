...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.equal(items(questions).length, 120)
  assert.equal(items(articles).length, 19)
  assert.equal(items(concepts).length, 79)
  assert.equal(items(claims).length, 120)
  assert.equal(items(citations).length, 120)
  assert.equal(items(spans).length, 120)
  ...
  assert.match(coverage, /\| Questions \| 223 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 266 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 223 \|/)
  assert.match(coverage, /\| Citations \| 228 \|/)
  assert.match(coverage, /\| Article spans \| 223 \|/)
  assert.match(coverage, /\| Concepts \| 157 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 223 authored; \*\*5,221 raw prompt observations remain\*\*, including 266 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 223 clean source-keyed prompts authored; \*\*4,988 raw answer observations remain\*\*, including the 266 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 223 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 266 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 223 authored; \*\*5,221 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 223 clean source-keyed prompts authored; \*\*4,988 raw answer observations remain\*\*/)
})
...
test('Mucize Parasitology core Q31-Q60 emits sixteen governed Draft questions and fourteen exact holds', () => {
  ...
  assert.equal(items(concepts).length, 79)
  assert.equal(items(articles).length, 19)
  assert.match(sources, /Physical pages 6–25 and 28 were rendered and visually re-read/)
...
test('Mucize Parasitology core Q61-Q90 emits eighteen governed Draft questions and twelve exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 223 authored; \*\*5,221 raw prompt observations remain\*\*, including 266 explicit holds and two source-absent prompts/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 223 clean source-keyed prompts authored; \*\*4,988 raw answer observations remain\*\*, including the 266 held printed-key observations/)
  assert.match(coverage, /\| Evidence resources \| 29 \|/)
  assert.match(coverage, /\| Claims \| 223 \|/)
  assert.match(coverage, /\| Citations \| 228 \|/)
...
test('Mucize case-based learning Case 2 records all three repeated identities as explicit holds', () => {
  ...
  assert.match(sources, /case-based learning Cases 1–8 and Advanced MCQs Q1–Q12 authoring audit/)
  assert.equal(items(questions).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
  assert.equal(items(claims).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
  assert.equal(items(citations).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
  assert.equal(items(spans).filter((item) => /MUCIZE-Q/.test(item)).length, 46)
