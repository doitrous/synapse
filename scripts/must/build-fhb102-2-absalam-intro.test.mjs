...
test('the approved Absalam introduction slices emit fourteen clean Draft questions and sixteen ledger holds', () => {
  ...
  assert.equal(items(questions).length, 14)
  assert.equal(items(articles).length, 5)
  assert.equal(items(concepts).length, 12)
  assert.equal(items(claims).length, 14)
  assert.equal(items(citations).length, 14)
  assert.equal(items(spans).length, 14)
  ...
  for (const held of ['Q1', 'Q3', 'Q6', 'Q7', 'Q8', 'Q10', 'Q12', 'Q13', 'Q16', 'Q17', 'Q20', 'Q26', 'Q27', 'Q28', 'Q29', 'Q30']) {
  ...
  assert.match(coverage, /\| Questions \| 22 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 39 \| no student-facing record authored \|/)
  ...
  for (const item of items(articles)) {
    ...
    for (const sibling of related.split('\n').map((line) => line.split(':')[0]).filter(Boolean)) {
      ...
      assert.ok(siblingItem, `${ownId} must link only to a complete article in the batch`)
