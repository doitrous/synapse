...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.match(coverage, /\| Questions \| 173 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 188 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.match(coverage, /\| Claims \| 173 \|/)
  assert.match(coverage, /\| Citations \| 178 \|/)
  assert.match(coverage, /\| Article spans \| 173 \|/)
...
test('global Q91-Q120 emits only the ten approved Chapter 6 Draft questions and preserves all twenty holds', () => {
  ...
  assert.equal(sourceIndex.count, 28)
...
test('global Q121-Q150 emits exactly nineteen approved Pharmacology Draft questions and preserves eleven holds', () => {
  ...
  assert.match(coverage, /\| Evidence resources \| 28 \|/)
  assert.match(coverage, /\| Claims \| 173 \|/)
  assert.match(coverage, /\| Citations \| 178 \|/)
  assert.match(coverage, /\| Article spans \| 173 \|/)
  assert.match(coverage, /\| Concepts \| 139 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 173 authored; \*\*5,271 raw prompt observations remain\*\*, including 188 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 173 clean source-keyed prompts authored; \*\*5,038 raw answer observations remain\*\*, including the 188 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /\| Questions \| 173 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 188 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 173 authored; \*\*5,271 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 173 clean source-keyed prompts authored; \*\*5,038 raw answer observations remain\*\*/)
})
