...
test('the approved Absalam introduction, Arthropoda, Mosquitoes and Sandfly slices preserve their exact Draft and hold boundaries', () => {
  ...
  assert.equal(items(questions).length, 70)
  assert.equal(items(articles).length, 19)
  assert.equal(items(concepts).length, 61)
  assert.equal(items(claims).length, 70)
  assert.equal(items(citations).length, 70)
  assert.equal(items(spans).length, 70)
  ...
  assert.match(coverage, /\| Questions \| 132 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 169 \| no student-facing record authored \|/)
...
test('global Q61-Q75 emits only the six approved microbiology Draft questions and preserves all nine holds', () => {
  ...
  assert.equal(items(questions).length, 53)
  assert.equal(items(articles).length, 11)
  assert.equal(items(concepts).length, 47)
  assert.equal(items(claims).length, 53)
  assert.equal(items(citations).length, 53)
  assert.equal(items(spans).length, 53)
  ...
  assert.match(coverage, /\| Claims \| 132 \|/)
  assert.match(coverage, /\| Citations \| 137 \|/)
  assert.match(coverage, /\| Article spans \| 132 \|/)
  assert.match(coverage, /\| Concepts \| 116 \|/)
  assert.match(coverage, /\| Articles \| 38 \|/)
  assert.match(coverage, /\| Questions \| 132 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 169 \| no student-facing record authored \|/)
...
test('global Q91-Q120 emits only the ten approved Chapter 6 Draft questions and preserves all twenty holds', () => {
  ...
  assert.equal(sourceIndex.count, 26)
...
test('global Q121-Q150 emits exactly nineteen approved Pharmacology Draft questions and preserves eleven holds', () => {
  ...
  assert.match(coverage, /\| Evidence resources \| 26 \|/)
  assert.match(coverage, /\| Claims \| 132 \|/)
  assert.match(coverage, /\| Citations \| 137 \|/)
  assert.match(coverage, /\| Article spans \| 132 \|/)
  assert.match(coverage, /\| Concepts \| 116 \|/)
...
test('Absalam Part 2 Sandfly Q1-Q30 emits only nineteen safe Draft questions and eleven exact holds', () => {
  ...
  assert.match(coverage, /Governed prompt observations: 5,444 total; 132 authored; \*\*5,312 raw prompt observations remain\*\*, including 169 explicit holds/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 132 clean source-keyed prompts authored; \*\*5,079 raw answer observations remain\*\*, including the 169 held printed-key observations/)
})
...
test('Absalam Part 2 Virology Q1-Q30 emits only three safe Draft questions and twenty-seven exact holds', () => {
  ...
  assert.match(coverage, /exact next boundary is Part 2 Microbiology Chapter 10 Q1–Q30 on pages 39–47/)
  assert.match(coverage, /\| Evidence resources \| 26 \|/)
  assert.match(coverage, /\| Claims \| 132 \|/)
  assert.match(coverage, /\| Citations \| 137 \|/)
  assert.match(coverage, /\| Article spans \| 132 \|/)
  ...
  assert.match(coverage, /\| Questions \| 132 \| Draft \|/)
  assert.match(coverage, /\| Question authoring holds \| 169 \| no student-facing record authored \|/)
  assert.match(coverage, /Governed prompt observations: 5,444 total; 132 authored; \*\*5,312 raw prompt observations remain\*\*/)
  assert.match(coverage, /Governed answer observations: 5,211 total; 132 clean source-keyed prompts authored; \*\*5,079 raw answer observations remain\*\*/)
})
