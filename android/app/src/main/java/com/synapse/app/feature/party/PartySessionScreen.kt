            ...
            LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                when {
                    session.state == "scheduled" -> item {
                        Text("This session has not started yet.", style = MaterialTheme.typography.bodyLarge)
                    }
                    ...
                    session.isClosed || allDone -> item {
                        ...
                        Text("${session.myAnswers.size + 1} / ${session.itemRefs.size}", style = MaterialTheme.typography.labelLarge)
                        if (current.kind == "question") {
                            val question = questionsById[current.id]
                            if (question == null) {
                                Text("This question is no longer available.", style = MaterialTheme.typography.bodyMedium)
                            } else {
                ...
                item { Text("Who is in", style = MaterialTheme.typography.titleMedium) }
...
private fun SelfCheckCard(item: PartySessionItemRefDto, onDone: () -> Unit) {
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text(if (item.kind == "practical") "Practical item" else "Essay question", style = MaterialTheme.typography.titleMedium)
            Text(
                "This item's authored content isn't previewable on Android yet — work through it on the web app, then mark it done here. It is self-checked, not scored by the server.",
                style = MaterialTheme.typography.bodySmall,
            )
            Button(onClick = { busy = true; onDone() }, enabled = !busy, modifier = Modifier.fillMaxWidth()) { Text("I've done this") }
        }
    }
}
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (marked.isNotEmpty()) {
                Text("${marked.count { it.correct == true }} of ${marked.size} correct", style = MaterialTheme.typography.headlineSmall)
            } else {
                Text("No questions were marked in this session.", style = MaterialTheme.typography.bodyMedium)
            }
            if (practised > 0) {
                Text("$practised item${if (practised == 1) "" else "s"} self-checked", style = MaterialTheme.typography.bodyMedium)
            }
            Text(
...
private fun MemberTallyRow(member: PartySessionMemberDto) {
    Row(
        ...
        Column(horizontalAlignment = Alignment.End) {
            Text(member.tally.marked?.let { "${it.correct} / ${it.of}" } ?: "Not marked yet", style = MaterialTheme.typography.labelLarge)
            if (member.tally.practised > 0) {
                Text("${member.tally.practised} self-checked", style = MaterialTheme.typography.bodySmall)
            }
        }
    }
}
