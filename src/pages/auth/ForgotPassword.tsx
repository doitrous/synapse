...
import { authMessage, recover } from '@/lib/auth/client'
...
export function ForgotPassword() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!API_MODE) return setMessage('This deployment is not connected to its account service yet. No reset email was sent.')
    setLoading(true)
    // The server answers this the same way whether or not the address exists,
    // so the only failure worth reporting is one that never reached it.
    try {
      await recover(email)
      setMessage('If an account exists for that address, a reset message has been sent.')
    } catch (error) {
      setMessage(authMessage(error, 'The reset message could not be requested. Wait a moment and try again.'))
    }
    setLoading(false)
  }
