...
export function PricingPage({ content, pricing }: { content: LandingContent; pricing: PricingContent }) {
  ...
  usePageMeta({
    ...
    jsonLd: [
      ...
      // asked "can you cancel Connect Cortex any time?" should find the sentence
  ...
  return (
    <MarketingShell c={c} otherHref={p.otherPath} homeHref={home}>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {p.trust.map((point) => (
            <div key={point.label} className="rounded-xl border border-line bg-surface p-4 shadow-panel">
              <Icon icon={TRUST_ICON[point.icon]} size={17} className="text-primary-strong" />
      ...
      {/* ---- Close ---- */}
      <section className="mt-20 overflow-hidden rounded-2xl border border-primary-strong/25 bg-primary px-6 py-14 text-center text-on-primary sm:px-10 sm:py-16">
        ...
        <p className="mx-auto mt-4 max-w-xl text-[15.5px] leading-relaxed text-on-primary/85">{p.closingSub}</p>
