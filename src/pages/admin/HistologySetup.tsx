...
import { Microscope, ChevronRight, Network, GraduationCap } from 'lucide-react'
...
/**
 * Histology Setup = a left "Master Histology" navigator (all slides →
 * per-university → per-year) wrapped around the full histology catalogue, matching
 * Questions Setup and Resources & Media. Systems/topics are edited in Subjects & Topics.
 */
export function HistologySetup() {
    ...
    <div className="flex min-h-[calc(100dvh-4rem)] flex-col lg:flex-row">
      ...
      <div className="min-w-0 flex-1">
        <div className="border-b border-line bg-surface px-5 py-2.5 text-[12.5px] text-ink-2">
          ...
          {isMaster
            ? 'Master Histology — every slide'
            : `${universities.find((u) => u.id === selection.universityId)?.short ?? ''}${selection.year ? ` · ${selection.year}` : ' · all years'}`}
        </div>
        <ControlDashboard key={`${selection.universityId ?? 'all'}-${selection.year ?? 'all'}`} initialKind="histology" lockedKind scope={scope} />
      </div>
    </div>
  )
}
