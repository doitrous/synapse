...
export function NotificationCampaigns() {
  ...
  const [campaigns, setCampaigns] = usePersistentState<NotificationCampaign[]>(NOTIFICATION_STORAGE_KEY, initialNotificationCampaigns)
    ...
    <PageContainer>
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_25rem]">
        <Panel className="overflow-hidden">
          ...
          <Table>
            <thead><tr><Th className="pl-4">Notification</Th><Th>Delivery</Th><Th>Audience</Th><Th>When</Th><Th>Status</Th><Th align="right" className="pr-4">Actions</Th></tr></thead>
            <tbody>{campaigns.map((campaign) => {
              ...
              return <Tr key={campaign.id} hover><Td className="max-w-sm pl-4"><p className="line-clamp-1 font-semibold text-ink">{campaign.title}</p><p className="mt-0.5 line-clamp-1 text-[11.5px] text-ink-3">{campaign.message}</p></Td><Td><Badge tone={campaign.delivery === 'Automated' ? 'accent' : campaign.delivery === 'Scheduled' ? 'warning' : 'neutral'}>{campaign.delivery}</Badge></Td><Td className="text-[12px] text-ink-2">{audienceRules ? `${audienceRules} targeting rules` : 'All students'}</Td><Td className="whitespace-nowrap text-[12px] text-ink-2">{campaign.delivery === 'Automated' ? campaign.automation : formatDateTime(new Date(campaign.scheduledAt))}</Td><Td><Badge tone={campaign.active ? 'success' : 'neutral'}>{campaign.active ? 'Active' : 'Paused'}</Badge></Td><Td align="right" className="pr-4"><div className="inline-flex gap-1"><IconButton icon={Pencil} label={`Edit ${campaign.title}`} size="sm" onClick={() => setEditing({ ...campaign })} /><IconButton icon={Trash2} label={`Delete ${campaign.title}`} size="sm" className="text-danger" onClick={() => setCampaigns((current) => current.filter((item) => item.id !== campaign.id))} /></div></Td></Tr>
            })}</tbody>
          </Table>
        </Panel>
