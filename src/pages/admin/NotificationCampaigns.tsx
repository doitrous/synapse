        ...
        <Panel className="overflow-hidden">
          ...
          <Table>
            ...
            <tbody>{campaigns.map((campaign) => {
              ...
              return <Tr key={campaign.id} hover><Td className="max-w-sm pl-4"><p className="line-clamp-1 font-semibold text-ink">{campaign.title}</p><p className="mt-0.5 line-clamp-1 text-[11.5px] text-ink-3">{campaign.message}</p></Td><Td><Badge tone={campaign.delivery === 'Automated' ? 'primary' : campaign.delivery === 'Scheduled' ? 'warning' : 'neutral'}>{campaign.delivery}</Badge></Td><Td className="text-[12px] text-ink-2">{audienceRules ? `${audienceRules} targeting rules` : 'All students'}</Td><Td className="whitespace-nowrap text-[12px] text-ink-2">{campaign.delivery === 'Automated' ? campaign.automation : formatDateTime(new Date(campaign.scheduledAt))}</Td><Td><Badge tone={campaign.active ? 'success' : 'neutral'}>{campaign.active ? 'Active' : 'Paused'}</Badge></Td><Td align="right" className="pr-4"><div className="inline-flex gap-1"><IconButton icon={Pencil} label={`Edit ${campaign.title}`} size="sm" onClick={() => setEditing({ ...campaign })} /><IconButton icon={Trash2} label={`Delete ${campaign.title}`} size="sm" className="text-danger" onClick={() => setCampaigns((current) => current.filter((item) => item.id !== campaign.id))} /></div></Td></Tr>
            })}</tbody>
          </Table>
        </Panel>
