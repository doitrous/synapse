        ...
        <div className="space-y-4">
          <Panel className="p-2">
            {([['all', 'All mail', Mail], ['inbox', 'Inbox', Inbox], ['outbox', 'Outbox', Send]] as const).map(([val, label, icon]) => (
              <button key={val} type="button" onClick={() => setFolder(val)} className={cn('flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-start text-[13.5px] font-medium', folder === val ? 'bg-accent-tint text-accent-strong' : 'text-ink-2 hover:bg-inset')}>
        ...
        <div className="min-w-0">
          {composing ? (
            ...
            <Panel className="overflow-hidden">
              ...
              {loading ? (
                <div className="flex items-center justify-center gap-2 py-16 text-ink-3"><Loader2 size={16} className="animate-spin" />Loading…</div>
              ) : rows.length === 0 ? (
...
function MailboxPanel({ box, mailboxes, onSelect, onCreated, onError }: { box: string; mailboxes: Mailbox[]; onSelect: (b: string) => void; onCreated: (m: Mailbox) => void; onError: (s: string) => void }) {
  ...
  return (
    <Panel className="overflow-hidden">
      ...
      <div className="p-2">
        <button type="button" onClick={() => onSelect('')} className={cn('flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-start text-[12.5px]', box === '' ? 'bg-accent-tint font-medium text-accent-strong' : 'text-ink-2 hover:bg-inset')}>All addresses</button>
        <button type="button" onClick={() => onSelect(DEFAULT_FROM)} className={cn('flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-start text-[12.5px]', box === DEFAULT_FROM ? 'bg-accent-tint font-medium text-accent-strong' : 'text-ink-2 hover:bg-inset')}>{DEFAULT_FROM}</button>
        {mailboxes.filter((m) => m.address !== DEFAULT_FROM).map((m) => (
          <button key={m.address} type="button" onClick={() => onSelect(m.address)} className={cn('flex w-full items-center gap-2 truncate rounded-md px-2.5 py-1.5 text-start text-[12.5px]', box === m.address ? 'bg-accent-tint font-medium text-accent-strong' : 'text-ink-2 hover:bg-inset')}>{m.address}</button>
        ))}
        {adding && (
