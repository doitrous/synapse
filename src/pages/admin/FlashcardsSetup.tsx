...
import { Layers, ChevronRight, Network, GraduationCap } from 'lucide-react'
...
/**
 * Flashcards Setup = a left "Master Flashcards" navigator (all decks →
 * per-university → per-year) wrapped around the full deck catalogue, matching
 * Practical Setup, Questions Setup, and Resources & Media.
 */
    ...
    <div className="flex min-h-[calc(100dvh-4rem)] flex-col lg:flex-row">
      ...
      <div className="min-w-0 flex-1">
        <div className="border-b border-line bg-surface px-5 py-2.5 text-[12.5px] text-ink-2">
          ...
          {isMaster
            ? 'Master Flashcards — every deck'
            : `${universities.find((u) => u.id === selection.universityId)?.short ?? ''}${selection.year ? ` · ${selection.year}` : ' · all years'}`}
        </div>
        <ControlDashboard key={`${selection.universityId ?? 'all'}-${selection.year ?? 'all'}`} initialKind="deck" lockedKind scope={scope} />
      </div>
    </div>
  )
}
