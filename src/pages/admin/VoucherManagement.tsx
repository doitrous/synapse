...
import { initialVouchers, VOUCHER_STORAGE_KEY, type Voucher, type VoucherDiscountType } from '@/data/vouchers'
...
export function VoucherManagement() {
  ...
  function save() {
    if (!editing || !editing.code.trim() || !editing.name.trim() || editing.amount <= 0) return
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_23rem]">
        ...
        <Panel className="overflow-hidden xl:sticky xl:top-20">
          ...
          {editing ? <div className="space-y-4 p-4">
            ...
            <div className="grid grid-cols-2 gap-3"><Field label="Starts"><TextInput type="date" value={dateInput(editing.startsAt)} onChange={(event) => setEditing({ ...editing, startsAt: new Date(`${event.target.value}T00:00:00`).toISOString() })} /></Field><Field label="Expires"><TextInput type="date" value={dateInput(editing.expiresAt)} onChange={(event) => setEditing({ ...editing, expiresAt: new Date(`${event.target.value}T23:59:59`).toISOString() })} /></Field></div>
            ...
            <Button variant="primary" className="w-full" onClick={save} disabled={!editing.code.trim() || !editing.name.trim() || editing.amount <= 0}>Save voucher</Button>
