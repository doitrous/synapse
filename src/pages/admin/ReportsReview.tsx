      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(20rem,0.9fr)_minmax(26rem,1.1fr)]">
        <Panel className="overflow-hidden">
          ...
          <div className="max-h-[42rem] overflow-y-auto">
            {rows.map((report) => (
              <button key={report.id} type="button" onClick={() => openReport(report)} className={cn('flex w-full items-start gap-3 border-b border-line px-4 py-3 text-left transition-colors last:border-b-0 hover:bg-inset/60', selected?.id === report.id && 'bg-primary-tint/45')}>
