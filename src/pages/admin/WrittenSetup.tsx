...
import { PenLine, ChevronRight, Network, GraduationCap } from 'lucide-react'
...
/**
 * Written Setup = a left "Master Written" navigator (all written questions →
 * per-university → per-year) wrapped around the full essay catalogue, matching
 * Practical Setup and Questions Setup. This is the only way to author a written
 * question by hand — everything else falls through to bulk import.
 */
export function WrittenSetup() {
    ...
    <div className="flex min-h-[calc(100dvh-4rem)] flex-col lg:flex-row">
      ...
      <div className="min-w-0 flex-1">
        <div className="border-b border-line bg-surface px-5 py-2.5 text-[12.5px] text-ink-2">
          ...
          {isMaster
            ? 'Master Written — every written question'
            : `${universities.find((u) => u.id === selection.universityId)?.short ?? ''}${selection.year ? ` · ${selection.year}` : ' · all years'}`}
        </div>
        <ControlDashboard key={`${selection.universityId ?? 'all'}-${selection.year ?? 'all'}`} initialKind="essay" lockedKind scope={scope} />
      </div>
    </div>
  )
}
