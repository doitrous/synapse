  ...
  return (
    <button
      ...
      className={cn(
        ...
        'relative h-6 w-11 shrink-0 rounded-full border transition-[background-color,border-color,box-shadow] duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary',
        "after:absolute after:-inset-y-2.5 after:inset-x-0 after:content-[''] sm:after:hidden",
        !checked && 'border-line-2 bg-surface-2',
        checked && !tint && 'border-primary-hover bg-primary shadow-[inset_0_0_0_1px_rgba(255,255,255,0.08)]',
      )}
      style={checked && tint ? { backgroundColor: tint, borderColor: tint } : undefined}
    >
      <span
        className="absolute top-1/2 size-4 -translate-y-1/2 rounded-full border border-black/5 bg-white shadow-[0_1px_2px_rgba(22,25,32,0.24)] transition-[left] duration-200 ease-[var(--ease-out-quint)]"
