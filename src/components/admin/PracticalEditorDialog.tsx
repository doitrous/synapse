...
export function PracticalEditorDialog({ open, item, onClose, onSave }: { open: boolean; item: ManagedContentItem | null; onClose: () => void; onSave: (item: ManagedContentItem) => void }) {
...
const idList = (value: string) => value.split(/\r?\n|\||;/).map((entry) => entry.trim()).filter(Boolean)
 ...
 * request.
 */
function TaggingPanel({ data, onChange }: { data: PracticalAuthoringData; onChange: (data: PracticalAuthoringData) => void }) {
  const tags = data.conceptTags
  const setTags = (patch: Partial<PracticalConceptTags>) => onChange({ ...data, conceptTags: { ...tags, ...patch } })
  return <div className="space-y-4">
    <EditorShell title="What this teaches" hint="Concept IDs, one per line. Contextual concepts are mentioned but not assessed, and receive no mastery evidence.">
      <div className="grid gap-4 sm:grid-cols-3">
        <Field label="Main concept(s)" htmlFor="practical-main-concept"><Textarea id="practical-main-concept" className="min-h-20 font-mono text-[12px]" value={tags.mainConceptIds.join('\n')} onChange={(event) => setTags({ mainConceptIds: idList(event.target.value) })} placeholder="CON-CVS-…" /></Field>
        <Field label="Also assessed" htmlFor="practical-concepts"><Textarea id="practical-concepts" className="min-h-20 font-mono text-[12px]" value={tags.conceptIds.join('\n')} onChange={(event) => setTags({ conceptIds: idList(event.target.value) })} placeholder="CON-CVS-…" /></Field>
        <Field label="Mentioned only" htmlFor="practical-contextual"><Textarea id="practical-contextual" className="min-h-20 font-mono text-[12px]" value={tags.contextualConceptIds.join('\n')} onChange={(event) => setTags({ contextualConceptIds: idList(event.target.value) })} placeholder="CON-CVS-…" /></Field>
      </div>
...
function QuestionTagFields({ conceptId, secondaryConceptIds, difficulty, onChange, idPrefix }: { conceptId?: string; secondaryConceptIds?: string[]; difficulty?: PracticalDifficulty; onChange: (patch: { conceptId?: string; secondaryConceptIds?: string[]; difficulty?: PracticalDifficulty }) => void; idPrefix: string }) {
  return <div className="mt-4 grid gap-4 sm:grid-cols-[1fr_1fr_10rem]">
    <Field label="Concept taught" htmlFor={`${idPrefix}-concept`}><TextInput id={`${idPrefix}-concept`} className="font-mono text-[12px]" value={conceptId ?? ''} onChange={(event) => onChange({ conceptId: event.target.value })} placeholder="CON-CVS-…" /></Field>
    <Field label="Also assessed" htmlFor={`${idPrefix}-secondary`}><TextInput id={`${idPrefix}-secondary`} className="font-mono text-[12px]" value={(secondaryConceptIds ?? []).join(' | ')} onChange={(event) => onChange({ secondaryConceptIds: idList(event.target.value) })} placeholder="CON-CVS-… | CON-CVS-…" /></Field>
...
function CaseEditor({ data, index, onIndex, onChange }: { data: CaseAuthoringData; index: number; onIndex: (index: number) => void; onChange: (data: CaseAuthoringData) => void }) {
