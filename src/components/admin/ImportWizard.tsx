...
export function ImportWizard(props: ImportWizardProps) {
  ...
  // What to call a row in the preview. The fixed chain below covers the catalogues
  // whose naming column happens to be one of these; the glossary names its column
  // `term`, so every row of a valid batch read "Untitled" and the preview could not
  // be reviewed at all. Falling back to the schema's first required field fixes that
  // for this importer and any later one.
  const nameKey = fields.find((f) => f.required)?.key
  const rowTitle = (values: Record<string, string>) =>
    values.title || values.label || values.module || values.name || (nameKey ? values[nameKey] : '') || 'Untitled'
