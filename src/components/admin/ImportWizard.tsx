...
import { AlertTriangle, ArrowLeft, ArrowRight, Check, CheckCircle2, FileSpreadsheet, FileText, RotateCcw, Upload, XCircle } from 'lucide-react'
...
export interface ImportWizardProps {
  ...
  /**
   * Called once a file is accepted. A page whose `commit`/`validateRow` needs a
   * large deferred document (the concept graph, the evidence store) starts
   * loading it here, so the download overlaps the mapping steps instead of
   * blocking first paint.
   */
  onBegin?: () => void
  /**
   * When false, the final Import button is disabled: the heavy data `commit`
   * reads has not finished loading yet, so importing now would compute against a
   * seed. Default true (no deferred data to wait for).
   */
  ready?: boolean
}
