  ...
  if (debrief) {
    return (
      <div>
        ...
        <Panel className="mt-4 p-4"><h3 className="text-[13px] font-semibold text-ink">Read around it</h3><ul className="mt-2 space-y-2">{(detail.references ?? ['Relevant clinical guideline']).map((reference) => <li key={reference} className="flex items-center gap-2 text-[12.5px] text-accent-strong"><Icon icon={BookOpen} size={14} />{reference}</li>)}</ul><Button className="mt-4" variant="primary" onClick={onExit}>Finish case</Button></Panel>
      </div>
    )
  }
