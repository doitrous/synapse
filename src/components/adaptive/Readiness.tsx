...
function ReadinessTrend({ series }: { series: ReadinessResult[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} role="img" aria-label={t('Your readiness range over time')}>
  ...
  if (session) {
    ...
    if (!item) {
      return (
        <Panel>
          <EmptyState
            icon={ClipboardCheck}
            title={t('This assessment could not be restored')}
            description={t('The questions it referred to are no longer in scope.')}
            action={<Button onClick={discard}>{t('Discard it')}</Button>}
          />
        </Panel>
      )
    }
    ...
    return (
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
        <Panel>
          <PanelHeader
            title={t('Question {index} of {total}').replace('{index}', String(index + 1)).replace('{total}', String(session.items.length))}
            icon={Timer}
            hint={t('Feedback comes at the end — this is a measurement, not practice')}
          />
          <div className="p-5">
        ...
        <div className="min-w-0 space-y-4">
          <Panel>
            <PanelHeader title={t('Progress')} icon={ClipboardCheck} />
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[1.35fr_1fr]">
      <div className="min-w-0 space-y-5">
        <Panel>
          <PanelHeader
            title={t('Your readiness range over time')}
            icon={TrendingUp}
            hint={t('Held-out questions only — feedback at the end, never during')}
          />
          <div className="p-5 pt-4">
            {series.length >= 2 ? (
              <>
                ...
                <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-[11px] text-ink-3">
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-[3px] w-4 shrink-0 rounded-full bg-primary" /> {t('estimate')}
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2.5 w-4 shrink-0 rounded-sm bg-primary-tint" /> {t('uncertainty band — it narrows as you answer more')}
                  </span>
                </div>
              </>
            ) : (
              <EmptyState
                icon={TrendingUp}
                title={t('Not enough assessments yet')}
                description={t('Your range appears here as a trend once you have taken at least two readiness assessments.')}
              />
            )}
          </div>
        </Panel>
        ...
        {latest && latest.groups.length > 0 && (
          <Panel>
            <PanelHeader title={t('By blueprint area')} icon={ClipboardCheck} hint={t('Most recent assessment')} />
            <Table>
              <thead>
                <tr>
                  <Th>{t('Blueprint area')}</Th>
                  <Th align="end">{t('Answered')}</Th>
                  <Th align="end">{t('Correct')}</Th>
                  <Th>{t('Range')}</Th>
                </tr>
              </thead>
              <tbody>
                {latest.groups.map((group) => (
                  <Tr key={group.groupId}>
                    ...
                    <Td>
                      {group.insufficient ? (
                        <span className="text-[12px] text-ink-3">{t('Too few questions to report')}</span>
                      ) : (
            ...
            {latest.underRepresented.length > 0 && (
              <div className="border-t border-line p-5">
                <Caveat>
                  {latest.underRepresented.length}{' '}
                  {latest.underRepresented.length === 1
                    ? t('blueprint area could not be fully represented:')
                    : t('blueprint areas could not be fully represented:')}{' '}
                  {latest.underRepresented.map((entry) => entry.groupLabel).join(', ')}
                  {t('. Treat this result as provisional.')}
                </Caveat>
              </div>
            )}
          </Panel>
      ...
      <div className="min-w-0 space-y-5">
        ...
        <Panel className="overflow-hidden border-accent-line">
          <div className="bg-gradient-to-b from-accent-tint to-transparent p-5">
            ...
            <h3 className="mt-1.5 font-serif text-[19px] font-semibold leading-snug tracking-[-0.01em] text-ink">
              {t('{n} held-out questions, marked at the end').replace('{n}', String(study.config.readiness.assessmentSize))}
            </h3>
            <p className="mt-2 text-[12.5px] leading-relaxed text-ink-2">{t(MEASUREMENT_SEPARATION.readiness.body)}</p>
            <p className="mt-2 text-[12.5px] leading-relaxed text-ink-2">{readinessSentence(latest, t)}</p>
        ...
        <Panel>
          <PanelHeader
            title={t('History')}
            icon={History}
            hint={`${history.length} ${history.length === 1 ? t('assessment') : t('assessments')}`}
          />
          {history.length === 0 ? (
            <EmptyState
              icon={History}
              title={t('No assessments yet')}
              description={t('Your practice accuracy is deliberately not shown here as a stand-in. Adaptive blocks oversample your weak areas, so it would read lower than your real standing.')}
            />
          ) : (
            <Table>
              <thead>
                <tr>
                  <Th>{t('Date')}</Th>
                  <Th align="end">{t('Answered')}</Th>
                  <Th align="end">{t('Omitted')}</Th>
                  <Th>{t('Range')}</Th>
                  <Th align="end">{t('Model')}</Th>
                </tr>
              </thead>
              <tbody>
                {history.map((result) => (
        ...
        <Panel className="p-4">
          <p className="text-[12px] leading-relaxed text-ink-2">
            <span className="font-semibold text-ink">{t('Why a range, not a number?')}</span>{' '}
            {t('A single percentage implies a precision the evidence does not have. The band narrows as you answer more held-out questions — watching it narrow, and rise, is the goal.')}
          </p>
        </Panel>
      </div>
    </div>
