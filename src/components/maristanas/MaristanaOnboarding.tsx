  ...
  return (
    <div className="grid-chart-major relative flex min-h-[290px] flex-col overflow-hidden bg-surface-2/45 sm:min-h-[360px] lg:min-h-full">
      <div className="flex items-center justify-between gap-3 px-4 pt-4 sm:px-5 sm:pt-5">
        <div>
          <p className="text-[10.5px] font-bold uppercase tracking-[0.09em] text-primary-strong">Construction study</p>
          <p className="mt-0.5 text-[11.5px] text-ink-3">One hospital · 25 visible stages</p>
        </div>
        {!reducedMotion && (
          <IconButton
            icon={RotateCcw}
            label="Replay build animation"
            variant="surface"
            onClick={replay}
      ...
      <div className="px-4 pb-4 sm:px-5 sm:pb-5">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="tnum font-mono text-[12px] font-semibold text-ink">Stage {String(frame).padStart(2, '0')} / 25</p>
            <p className="mt-0.5 text-[10.5px] text-ink-3">{frame === 25 ? 'Ready to serve' : assetsReady ? 'Building from your learning' : 'Preparing the construction study'}</p>
          </div>
          {frame === 25 && (
            <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-success">
              <Icon icon={Check} size={14} /> Complete
            </span>
          )}
        </div>
      ...
      <p className="sr-only">An illustration shows a Maristana being built from stage {frame} to stage 25.</p>
    </div>
  )
}
        ...
        {HOW_IT_WORKS.map((item, index) => (
          <li key={item.title} className={cn('flex gap-3.5', compact ? 'py-3' : 'py-4')}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-pretty text-[11.5px] leading-relaxed text-ink-2">{item.description}</p>
            </div>
          </li>
        ))}
      </ol>
      <div className="mt-4 rounded-lg border border-line bg-surface-2/55 p-3.5">
        <p className="text-[11px] font-bold uppercase tracking-[0.07em] text-ink-3">Then it builds itself</p>
...
export function MaristanaOnboarding({ onStart }: { onStart: () => void }) {
  ...
  return (
    <Dialog onClose={DO_NOT_DISMISS} label="Welcome to Build Maristanas" size="xl" className="overflow-hidden p-0">
        ...
        <div className="flex min-h-[430px] flex-col p-5 sm:p-6 lg:p-7">
          <div key={view} className="animate-pop">
            {view === 'introduction' ? (
              <>
                ...
                <p className="mt-5 text-[10.5px] font-bold uppercase tracking-[0.09em] text-primary-strong">Build Maristanas</p>
                <h1 className="mt-2 text-balance font-serif text-[30px] font-semibold leading-[1.08] tracking-[-0.03em] text-ink sm:text-[36px]">
                  Build a place of healing.
                </h1>
                ...
                <dl className="mt-6 grid grid-cols-3 gap-3 border-y border-line py-4">
                  <div><dt className="tnum font-mono text-[17px] font-semibold text-ink">25</dt><dd className="mt-1 text-[10.5px] leading-snug text-ink-3">visible stages</dd></div>
                  <div><dt className="font-mono text-[17px] font-semibold text-ink">3</dt><dd className="mt-1 text-[10.5px] leading-snug text-ink-3">learning signals</dd></div>
                  <div><dt className="font-mono text-[17px] font-semibold text-ink">Auto</dt><dd className="mt-1 text-[10.5px] leading-snug text-ink-3">construction</dd></div>
                </dl>
              </>
            ) : (
              <>
                <p className="text-[10.5px] font-bold uppercase tracking-[0.09em] text-primary-strong">How it works</p>
                <h1 className="mt-2 text-balance font-serif text-[29px] font-semibold leading-tight tracking-[-0.025em] text-ink">Learning is the building material.</h1>
                <p className="mt-2 text-pretty text-[12.5px] leading-relaxed text-ink-2">There is nothing extra to log and no arbitrary daily claim. The construction ledger reads the study work you already do.</p>
          ...
          <div className="mt-auto pt-6">
            <div className={cn('flex flex-col gap-2', view === 'how' && 'sm:flex-row')}>
              ...
              >
                {view === 'introduction' ? 'How it works' : 'Back'}
              </Button>
            </div>
            <p className="mt-3 text-center text-[10.5px] leading-relaxed text-ink-3">Progress is based on active study and server-scored performance.</p>
          </div>
        </div>
      </div>
    </Dialog>
...
export function MaristanaHowItWorksDialog({ onClose }: { onClose: () => void }) {
  return (
    <Dialog onClose={onClose} label="How Build Maristanas works" size="lg" className="overflow-hidden p-0">
      <div className="flex items-start justify-between gap-4 border-b border-line px-5 py-4 sm:px-6">
        <div>
          <p className="text-[10.5px] font-bold uppercase tracking-[0.09em] text-primary-strong">Build Maristanas</p>
          <h2 className="mt-1.5 font-serif text-[25px] font-semibold tracking-[-0.025em] text-ink">How construction works</h2>
          <p className="mt-1 text-[12px] text-ink-2">Your existing study activity moves the build forward automatically.</p>
        </div>
        <IconButton icon={X} label="Close how it works" onClick={onClose} />
      </div>
      <div className="px-5 py-2 sm:px-6"><HowItWorksSteps /></div>
      <div className="flex justify-end border-t border-line px-5 py-4 sm:px-6">
        <Button variant="primary" onClick={onClose} className="active:scale-[0.96]">I understand</Button>
      </div>
    </Dialog>
  )
}
