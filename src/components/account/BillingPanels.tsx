...
/**
 * What the student is actually subscribed to — the Billing tab of Account.
 *
 ...
 *
 * Moved out of a page of its own: billing is one of the four things a student
 * manages about their account, not a separate destination, so it renders inside
 * `Account` and `/app/billing` redirects to `?tab=billing`.
 */
        ...
        <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.8fr)] lg:items-center">
          ...
          {redemption ? (
            ...
            <div>
              ...
              <p role="status" className={cn('mt-2 text-[12px] text-ink-2', !message && 'sr-only')}>{message}</p>
            </div>
          )}
        </div>
      </Panel>
