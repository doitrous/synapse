...
export function RoomLobby({ onEnter }: { onEnter: (room: RoomAddress) => void }) {
  ...
  return <div className="room-discovery">
    <div className="room-discovery-main">
      ...
      <section aria-label={t('University rooms')}><PanelHeader title={t('University rooms')} icon={Users} hint={t('Your university and year')}/>{loadingOpen?<LoadingRegion><RoomCardsSkeleton /></LoadingRegion>:university.length?<div className="room-live-cards">{university.map(room=><RoomCard key={room.id} {...room} busy={busy} onEnter={()=>API_MODE?void joinAndEnter(room.code):onEnter(room)}/>)}</div>:<p className="py-4 text-sm text-ink-2">{t('No open rooms in your year right now.')}</p>}</section>
    ...
    <aside className="room-discovery-friends" aria-label={t('Friends')}>
      <Button className="w-full" variant="primary" iconLeft={Plus} onClick={()=>setCreating(true)}>{t('Create a new room')}</Button>
