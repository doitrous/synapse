    ...
    <div className="min-h-full bg-paper px-4 py-8 sm:px-7 lg:px-10 lg:py-12">
      <div className="mx-auto max-w-[76rem]">
        <div className="max-w-[46rem]">
          <div className="inline-flex items-center gap-2 rounded-full border border-primary-line bg-primary-tint/45 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.07em] text-primary-strong"><Icon icon={BookOpen} size={13} />Medical library</div>
        ...
        {searching ? (
          ...
          <div className="mt-9 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {MEDICAL_LIBRARY_VIEWS.map((view) => {
              ...
              return (
                <button key={view.id} type="button" onClick={() => onOpenView(view.id)} className="group flex min-h-44 flex-col rounded-xl border border-line bg-surface p-5 text-start shadow-panel transition-[border-color,background-color,transform] hover:-translate-y-0.5 hover:border-primary-line hover:bg-primary-tint/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">
                  <span className="grid size-10 place-items-center rounded-lg border border-line bg-surface-2 text-ink-2 transition-colors group-hover:border-primary-line group-hover:bg-primary-tint group-hover:text-primary-strong"><Icon icon={IconComponent} size={18} /></span>
                  ...
                  <span className="mt-4 inline-flex items-center gap-1 text-[12px] font-medium text-primary-strong">{t('Open')}<Icon icon={ChevronRight} size={14} className="transition-transform group-hover:translate-x-0.5" /></span>
                </button>
              )
            })}
          </div>
...
function TaxonomyBranch({ node, index, open, selectedNodeId, articleCounts, onToggle, onSelect }: {
  ...
  return (
    <li className="[content-visibility:auto]">
      <div className={cn('group flex min-h-9 items-center gap-1 rounded-md pe-1', selectedNodeId === node.id && 'bg-primary-tint/55')}>
        {children.length > 0 ? <button type="button" onClick={() => onToggle(node.id)} className="grid size-8 shrink-0 place-items-center rounded text-ink-3 hover:bg-inset hover:text-ink" aria-label={`${expanded ? 'Collapse' : 'Expand'} ${node.title}`} aria-expanded={expanded}><Icon icon={ChevronRight} size={13} className={cn('chevron-turn')} open={expanded} /></button> : <span className="ms-3 me-2 size-1.5 shrink-0 rounded-full bg-line-2" />}
        <button type="button" onClick={() => onSelect(node.id)} className="min-w-0 flex-1 py-2 text-start"><span className={cn('block truncate text-[12px]', selectedNodeId === node.id ? 'font-semibold text-primary-strong' : 'text-ink-2 group-hover:text-ink')}>{node.title}</span></button>
    ...
    >
      <button
        ...
        className={cn(
          ...
          view === 'home' ? 'bg-primary-tint text-primary-strong' : 'text-ink-3 hover:bg-inset hover:text-ink',
        )}
      >
      ...
      {MEDICAL_LIBRARY_VIEWS.map((item) => (
        <button
          ...
          className={cn(
            'inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg px-2.5 text-[12px] font-medium transition-colors',
            view === item.id ? 'bg-primary-tint text-primary-strong' : 'text-ink-2 hover:bg-inset hover:text-ink',
          )}
        >
...
export function TaxonomyNodeOverview({ node, taxonomy, articles, onOpenArticle, onSelectNode }: { node?: MedicalTaxonomyNode; taxonomy: MedicalTaxonomyNode[]; articles: AtlasArticle[]; onOpenArticle: (articleId: string) => void; onSelectNode?: (nodeId: string) => void }) {
  ...
  if (!node) return <div className="grid min-h-[60vh] place-items-center px-6 text-center"><div className="max-w-md"><span className="mx-auto grid size-11 place-items-center rounded-xl bg-primary-tint text-primary-strong"><Icon icon={BookOpen} size={20} /></span><h1 className="mt-4 font-serif text-[22px] font-semibold text-ink">{t('Choose a subject or topic')}</h1><p className="mt-2 text-[13px] leading-relaxed text-ink-3">{t('Pick a branch on the left to see what there is to read.')}</p></div></div>
        ...
        {ancestors.map((item) => (
          <span key={item.id} className="inline-flex items-center gap-1.5">
            <button
              ...
              className="rounded text-ink-3 transition-colors hover:text-primary-strong hover:underline disabled:no-underline disabled:hover:text-ink-3"
            >
              {item.title}
            </button>
            ...
            {related.map((article) => (
              <li key={article.id}>
                <button
                  ...
                  className="group flex w-full items-start gap-3 px-4 py-3.5 text-start transition-colors hover:bg-primary-tint/15"
                >
                  <span className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-md bg-primary-tint text-primary-strong"><Icon icon={BookOpen} size={13} /></span>
