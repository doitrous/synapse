  ...
  return (
    <span
      className={cn(
        ...
        faculty ? 'border-primary-line bg-primary-tint text-primary-strong' : 'border-line-2 bg-surface-2 text-ink-2',
        className,
      )}
    >
...
function ThenRow({ item }: { item: UpcomingItem }) {
  ...
  return (
    <li className="flex items-center gap-2.5 text-[12px] text-ink-2">
      ...
      <span
        className={cn('size-1.5 shrink-0 rounded-full', item.source === 'faculty' ? 'bg-primary' : 'bg-ink-3')}
    ...
    <Panel className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex flex-1 flex-col items-center justify-center p-4 text-center">
        ...
        <div className="mt-2.5 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[12.5px] text-ink-2">
          ...
          <Badge tone={next.isExam ? 'danger' : 'primary'} className="text-[11.5px]">{t(next.kind)}</Badge>
        </div>
