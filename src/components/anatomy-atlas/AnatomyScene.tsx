...
interface Props {atlas:Atlas;assetBase:string;state:SceneState;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({atlas,assetBase,state,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),reportProgress=useRef(onProgress),reportError=useRef(onError);
 latest.current=state;select.current=onSelect;reportProgress.current=onProgress;reportError.current=onError;
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{reportError.current('This browser could not start the 3D viewer. Please try a browser with WebGL enabled.');return;}
  ...
  const ground=new T.Mesh(new T.CircleGeometry(30,96),new T.MeshStandardMaterial({color:0xd5d9dc,roughness:1,envMap:env.texture}));ground.rotation.x=-Math.PI/2;ground.position.y=-.019;scene.add(ground);
  const platform=new T.Mesh(new T.CylinderGeometry(.68,.7,.028,100),new T.MeshStandardMaterial({color:0xeeeeec,metalness:.12,roughness:.67,envMap:env.texture}));platform.position.y=-.016;scene.add(platform);
  ...
  // Theme changes repaint the surrounding stage without reloading the model,
  // resetting the camera, or altering anatomical materials and illumination.
  const updateSurroundings=()=>{
   const style=getComputedStyle(el);
   const color=(name:string)=>style.getPropertyValue(`--atlas-${name}`).trim();
   renderer.setClearColor(color('background'));
   ground.material.color.set(color('ground'));
   platform.material.color.set(color('platform'));
   ground.material.envMapIntensity=Number(color('stage-environment'));
   platform.material.envMapIntensity=Number(color('stage-environment'));
   ring.material.color.set(color('ring'));
   innerRing.material.color.set(color('inner-ring'));
   markerMaterial.color.set(color('marker'));
   dirty=true;
  };
  updateSurroundings();
  const themeObserver=new MutationObserver(updateSurroundings);
  ...
  const loadChunk=async(ci:number)=>{
   const chunk=atlas.chunks[ci],compressed=!!chunk.gzip&&typeof DecompressionStream!=='undefined';const path=compressed?chunk.gzip!:chunk.url;const response=await fetch(path.replace('/models/',`${assetBase}/`),{signal:abort.signal});const buffer=await decodeModelResponse(response,chunk.bytes,compressed);if(disposed)return;
   ...
   lastState=null;loaded++;reportProgress.current(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)reportError.current(e instanceof Error?e.message:'Could not load the anatomy.');}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();reportError.current('The 3D session was paused by your device. Reload to continue.');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
 ...
 return <div className="anatomy-atlas-scene" ref={host}/>;
}
