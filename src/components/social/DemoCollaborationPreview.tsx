...
export function DemoFriendsPreview() {
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
      ...
      <Panel><PanelHeader title={t('Discoverability')} icon={Bell} /><div className="p-5"><Badge tone="outline">{t('Off by default')}</Badge><p className="mt-3 text-[13px] leading-relaxed text-ink-2">{t('Students opt in before they can appear here or browse classmates in the same university and year.')}</p></div></Panel>
    </div>
  )
}
