...
function SharedBoardImage({ image, shareId }: { image: ReturnType<typeof imagesOf>[number]; shareId?: string }) {
  ...
  return source ? <img src={source} alt={image.alt} draggable={false} className="size-full object-contain" /> : <span className="grid size-full place-items-center p-2 text-center text-[11px] text-ink-3">{image.alt}</span>
}
