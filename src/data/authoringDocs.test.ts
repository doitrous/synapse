...
import { IMPORT_CONTRACTS } from './importContract.ts'
...
test('every article import field is documented in the article manual', () => {
  check(IMPORT_CONTRACTS.article.manualFiles, IMPORT_CONTRACTS.article.fields.map((field) => field.key))
})
...
test('every question import field is documented in the question manual', () => {
  ...
  const keys = IMPORT_CONTRACTS.question.fields
    .map((field) => field.key)
    .filter((key) => !/^(answer|explanation)_[a-f]$/.test(key))
  check(IMPORT_CONTRACTS.question.manualFiles, keys)
})

test('every practical import field is documented across the five format manuals', () => {
  check(IMPORT_CONTRACTS.practical.manualFiles, IMPORT_CONTRACTS.practical.fields.map((field) => field.key))
})

test('every concept import field is documented in the concept manual', () => {
  check(IMPORT_CONTRACTS.concept.manualFiles, IMPORT_CONTRACTS.concept.fields.map((field) => field.key))
})

test('every relation import field is documented in the relationships manual', () => {
  check(IMPORT_CONTRACTS.relation.manualFiles, IMPORT_CONTRACTS.relation.fields.map((field) => field.key))
})
...
test('every evidence import field is documented in the relationships manual', () => {
  ...
  const keys = [...IMPORT_CONTRACTS.claim.fields, ...IMPORT_CONTRACTS.citation.fields].map((field) => field.key)
  check(IMPORT_CONTRACTS.claim.manualFiles, keys)
})

test('every catalogue-resource import field is documented in the resource manual', () => {
  check(IMPORT_CONTRACTS['catalogue-resource'].manualFiles, IMPORT_CONTRACTS['catalogue-resource'].fields.map((field) => field.key))
})
...
test('every evidence-source import field is documented in the resource manual', () => {
  ...
  check(IMPORT_CONTRACTS.resource.manualFiles, IMPORT_CONTRACTS.resource.fields.map((field) => field.key))
})
