...
test('no body carries the sign-off, which now sits outside the card', () => {
  each((automation) => assert.doesNotMatch(automation.body, /The Nishany team/, automation.id))
})
...
test('every automation renders the same wrapper, down to the rose rule', () => {
  each((automation) => {
    ...
    assert.match(html, /— The Nishany team/, automation.id)
...
const legacy = {
  ...
  note: undefined,
  details: undefined,
  body: '<p>Hello Maya,</p><p>Use the link below to choose a new password. It expires in one hour.</p><p><a href="{{actionUrl}}">Reset my password</a></p><p>If you did not ask for this, no change has been made.</p><p>— The Nishany team</p>',
}
...
test('the sign-off is dropped, because the wrapper now supplies it outside the card', () => {
  const migrated = normaliseAutomation(legacy)
  assert.doesNotMatch(migrated.body, /The Nishany team/)
  assert.match(renderAutomation(migrated).html, /— The Nishany team/)
})
