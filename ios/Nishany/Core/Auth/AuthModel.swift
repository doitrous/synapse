...
/// and the Nishany API verifies it against the project's JWKS and decides what
...
final class AuthModel {
    ...
    private(set) var api: NishanyAPI!
        ...
        if let url = AppConfig.supabaseURL, let key = AppConfig.supabaseAnonKey {
            ...
            let readToken: @Sendable () async -> String? = { [weak client] in
                ...
                do {
                    ...
                    #if DEBUG
                    NishanyAPI.lastTokenError = String(describing: error)
                    #endif
                    return nil
                }
            }
            self.readToken = readToken
            self.api = NishanyAPI(token: readToken)
        } else {
            self.client = nil
            self.readToken = { nil }
            self.api = NishanyAPI { nil }
        }
    }
    ...
    private func confirmWithServer(explainFailure: Bool) async {
        do {
            if let user = try await api.session() {
                ...
                // Not an error: Supabase knows this address and Nishany has no
                // account for it yet.
                userCache.writeUser(nil)
                state = .signedOut
                if explainFailure {
                    message = "Signed in, but Nishany has no account for this address yet."
                }
            }
            ...
            if explainFailure {
                ...
                message = "Your password was accepted, but Nishany rejected the session. "
                    + "Please try again, or contact support if it keeps happening."
                #if DEBUG
                if let detail = NishanyAPI.lastDiagnostic { message! += "\n\n[\(detail)]" }
                if let tokenError = NishanyAPI.lastTokenError { message! += "\n[token error: \(tokenError.prefix(180))]" }
                #endif
            }
        } catch {
