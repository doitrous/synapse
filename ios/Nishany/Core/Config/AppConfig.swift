...
enum AppConfig {
    ...
    /// The Nishany API, e.g. `https://synapse.doitrous.com/api`.
    static let apiBaseURL: URL = {
        let host = string(for: "NishanyAPIHost") ?? "synapse.doitrous.com"
        ...
        guard let url = URL(string: "\(scheme)://\(host)/api") else {
            preconditionFailure("NishanyAPIHost is not a usable host: \(host)")
        }
        return url
    }()
